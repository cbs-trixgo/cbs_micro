 ...
 * TOOLS
 */
const { checkParametersModel, checkPointDotsAndRoundingNumberForPagingChatting } = require('./utils');
 ...
 */
 exports.calculateCurrentPage = async function ({ totalRecord, totalCurrentPage, perPage }) {
	...
	let currentPage 	 	 = 0;
	let checkParamsTotalPage = checkParametersModel([ {totalCurrentPage}, {totalRecord} ], 'number');
	...
	if(!checkParamsTotalPage.error){
		...
		if(valueCurrentPage > 0){
			currentPage = checkPointDotsAndRoundingNumberForPagingChatting(valueCurrentPage, perPage);
		} else{
			currentPage = 1;
		}

		// Tính toán total page
		totalRecord = checkPointDotsAndRoundingNumberForPagingChatting(totalRecord, perPage);
	} else{
		// Nếu không phải là số ---> các giá trị trả về là 1
		currentPage = checkPointDotsAndRoundingNumberForPagingChatting(0, perPage);
		totalRecord  	= checkPointDotsAndRoundingNumberForPagingChatting(0, perPage);
	}
...
exports.getCurrentPage = function ({ totalRecord, totalCurrentPage, limit }){
	...
	let valueCurrentPage = totalRecord - totalCurrentPage;
	if(valueCurrentPage > 0){
		currentPage = checkPointDotsAndRoundingNumberForPagingChatting(valueCurrentPage, limit);
	} else{
		currentPage = 1;
	}
	return currentPage;
