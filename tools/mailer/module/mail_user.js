...
let { templateRecoverPass } = require('./recover-pass')
let { templateNewAccount } = require('./new-account')

exports.verifyEmailUser = function (userName, email, link, lang) {
  let emailContent =
    getHeaderEmail(
      util.format(language.getLanguage('welcome_to_website', lang), [
        'EXT GROUP',
      ])
    ) +
    getContentVerifyEMail(
      language.getLanguage(
        'xac_minh_dia_cho_email_cua_ban_click_vao_ben_duoi',
        lang
      ),
      link,
      lang
  ...
  mailer(
    email,
    util.format(language.getLanguage('vui_long_xac_thuc_email_cua_ban', lang), [
      userName,
    ]),
    emailContent,
    function (callback) {}
...
exports.resetPassAccount = function (email, userName, link, type) {
  let emailContent =
    getHeaderEmail(type === 1 ? 'RESET PASSWORD' : 'RESET SECOND PASSWORD') +
    getContentEmailResetPass(userName, baseUrl + link, type)
  ;+getFooterEmail()

  mailer(
    email,
    `${webName}: Reset password`,
    emailContent,
    function (callback) {}
  )
}
...
function getHeaderEmail(title) {
  return (
    ...
    '">' +
    '</a>' +
    '<h1 style="color: #2e3192; font-size: 30px; margin-bottom: 40px; text-align: center">' +
    title +
    '</h1>' +
    '</td>' +
    '</tr>' +
...
function getContentVerifyEMail(message, link, lang) {
  return (
    ...
    '<br><b style="font-size: 16px; text-align: center; display: block;">' +
    message +
    '</b>' +
    '<br>' +
    '<br><p style="text-align: center;"><a style="text-decoration: none; background: #2e3291; color: #fff; padding: 7px 25px; font-size: 15px; border-radius: 5px;" href="' +
    link +
    '">' +
    language.getLanguage('xac_nhan_dia_chi_email', lang) +
    '</a></p>' +
    '<br>' +
    '</div>' +
    '</td></tr></tbody>'
...
function getContentEmailResetPass(userName, link, type) {
  return (
    ...
    '<p class="title" style="line-height: 1.7; font-size: 19px;">' +
    util.format(
      language.getLanguage(
        type === 1 ? 'reset_password_mail' : 'reset_second_password_mail',
        defaultLanguage
      ),
      userName,
      link
...
function getFooterEmail() {
  return (
    '</table> <table style="line-height:1.5;font-size:12px;font-family:Arial,' +
...
exports.sendMailForgetPassword = function (email, fullName, codeReset) {
  let emailContent = templateRecoverPass(fullName, codeReset)
  mailer(
    email,
    ...
    'TRIXGO <noreply.trixgo@gmail.com>'
  )
}

exports.sendMailNewAccount = function (email, fullName, pwd) {
  let emailContent = templateNewAccount(fullName || email, pwd)
  mailer(
    email,
    `MẬT KHẨU MỚI/NEW PASSWORD - TRIXGO.COM`,
    emailContent,
    function (cb) {
      console.log(cb)
    }
...
exports.sendMailTestWithAttachments = function (email, fullName) {
  let emailContent = templateNewAccount(fullName || email)
  mailer(
    email,
    `EXAMPLE SEND MAIL WITH ATTACHMENTS - TRIXGO.COM`,
    emailContent,
    function (cb) {
      console.log(cb)
    }
