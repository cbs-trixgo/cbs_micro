    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (goods && Number(unitprice) == 0) {
          // console.log('===========================')
          let infoWoods = await ITEM__GOODS_COLL.findById(goods).select(
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (listDataUp && listDataUp.length) {
          dataBudget.quantity =
            Number(dataBudget.quantity) + Number(listDataUp[0].quantity)
          dataBudget.amount =
        ...
        if (listDataDown && listDataDown.length) {
          dataBudget.quantity =
            Number(dataBudget.quantity) - Number(listDataDown[0].quantity)
          dataBudget.amount =
            Number(dataBudget.amount) - Number(listDataDown[0].amount)
        }
  ...
  getList({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoDataAfterGet = await FINANCIAL_GENERAL_JOURNAL_COLL.find(
          conditionObj
        )
          .limit(limit + 1)
          .sort(sortBy)
            ...
            if (fundaID && checkObjectIDs(fundaID)) {
              ...
              if (arrCompanyIDs && checkObjectIDs(arrCompanyIDs)) {
                ...
                conditionDebitObj.company = {
                  $in: arrCompanyIDs.map((item) => ObjectID(item)),
                }
        ...
        if (!option) {
          ...
          if (option == 1) {
            ...
            if (listDataO[0]._debit.length > 0) {
              arrCustomers = [
                ...arrCustomers,
                ...listDataO[0]._debit.map((item) => item._id.contact),
              ]
            }
            if (listDataO[0]._credit.length > 0) {
              arrCustomers = [
                ...arrCustomers,
                ...listDataO[0]._credit.map((item) => item._id.contact),
              ]
            }
            if (listDataIn[0]._debit.length > 0) {
              arrCustomers = [
                ...arrCustomers,
                ...listDataIn[0]._debit.map((item) => item._id.contact),
              ]
            }
            if (listDataIn[0]._credit.length > 0) {
              arrCustomers = [
                ...arrCustomers,
                ...listDataIn[0]._credit.map((item) => item._id.contact),
              ]
            }
                ...
                if (!optionCal || Number(optionCal) == 1) {
                  ...
                  for (const subitem of item.nestedChilds) {
                    if (arrDebitO[`${subitem}`]) {
                      kcNoDauKySub =
                        kcNoDauKySub + Number(arrDebitO[subitem][0].amount)
                    }
                    if (arrCreditO[`${subitem}`]) {
                      kcCoDauKySub =
                        kcCoDauKySub + Number(arrCreditO[subitem][0].amount)
                    }
                    if (arrDebitOF[`${subitem}`]) {
                      noDauKySub =
                        noDauKySub + Number(arrDebitOF[subitem][0].amount)
                    }
                    if (arrCreditOF[`${subitem}`]) {
                      coDauKySub =
                        coDauKySub + Number(arrCreditOF[subitem][0].amount)
                    }
                    if (arrDebitFT[`${subitem}`]) {
                      noTrongKySub =
                        noTrongKySub + Number(arrDebitFT[subitem][0].amount)
                    }
                    if (arrCreditFT[`${subitem}`]) {
                      coTrongKySub =
                        coTrongKySub + Number(arrCreditFT[subitem][0].amount)
                    }
                  }
                } else {
            ...
            listAccounts.forEach((item) => {
              ...
              if ([1, 2, 6, 8].includes(Number(item.name.charAt(0)))) {
                listDataO[0]._debit.forEach((element) => {
                  let key = element._id.account
                  if (
                    item?._id.toString() === key.toString() ||
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (['152', '156'].includes(infoAccount.name.toString())) {
          ...
          let listVouchers = await FINANCIAL_VOUCHER_COLL.find(conditionOb)
            .sort({ date: 1 })
          ...
          return resolve({
            error: true,
            message: 'Tài khoản không hợp lệ: chỉ 152, 156 được phép',
            status: 400,
          })
        }
      } catch (error) {
  ...
  exportExcelByFilter({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        const listData = await FINANCIAL_GENERAL_JOURNAL_COLL.find(conditionObj)
          ...
          if (type == 1) {
            ...
            data?.forEach((item, index) => {
              ...
              workbook
                ...
                .cell(3)
                .value(
                  item?.voucher?.type && item?.voucher?.type >= 1
                    ? VOUCHER_TYPES[Number(item?.voucher?.type - 1)].text
                    : ''
                )
            ...
            data?.forEach((item, index) => {
              if (item.debit._id && checkObjectIDs(item.debit._id)) {
                ...
                workbook
                  ...
                  .row(i)
                  .cell(7)
                  .value(`${item?.debit?.name}-${item?.debit?.description}`)
                workbook
                  .sheet('Data1')
                  .row(i)
                  .cell(8)
              ...
              if (item.credit._id && checkObjectIDs(item.credit._id)) {
                ...
                workbook
                  ...
                  .row(i)
                  .cell(7)
                  .value(`${item?.credit?.name}-${item?.credit?.description}`)
            ...
            workbook
              ...
              .row(3)
              .cell(1)
              .value(`Ngày: ${moment(infoVoucher.date).format('DD/MM/YYYY')}`)
