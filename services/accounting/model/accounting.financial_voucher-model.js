...
class Model extends BaseModel {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (subtypeID && checkObjectIDs(subtypeID)) {
          ...
          let infoSubtype =
            await ITEM__DOCTYPE_COLL.findById(subtypeID).select('parent')
        ...
        if (selectedJournals && selectedJournals.length > 0) {
          ...
          for (const subitem of selectedJournals) {
            ...
            const dataInsert = {
              ...
              unitprice:
                subitem.quantity != 0
                  ? Number(Number(subitem.amount / subitem.quantity))
                  : 0,
              fcuExRate,
              fcuAmount:
                fcuExRate != 0 ? Number(subitem.amount / fcuExRate) : 0,
              note: subitem.note,
              ctx,
            }
  ...
  convertVoucher({ companyID, docID, userID, ctx }) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        let infoVoucher = await ACCOUNTING__FINANCIAL_VOUCHER_COLL.findById(
          docID
        ).populate({
          path: 'linkFunda',
        ...
        if (
          ...
          let listAccount = await ITEM__ACCOUNT_MODEL.getListRecursive({
            companyID,
            arrAccNames: ['152', '156'],
          })
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (lastestID && checkObjectIDs(lastestID)) {
          let infoData =
            await ACCOUNTING__FINANCIAL_VOUCHER_COLL.findById(lastestID)
          if (!infoData)
            return resolve({
              error: true,
        ...
        let infoDataAfterGet = await ACCOUNTING__FINANCIAL_VOUCHER_COLL.find(
          conditionObj
        )
          .limit(limit + 1)
          .sort(sortBy)
        ...
        let totalRecord =
          await ACCOUNTING__FINANCIAL_VOUCHER_COLL.count(conditionObjOrg)
