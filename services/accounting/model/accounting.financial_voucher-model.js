 ...
 * MODELS
 */
const ACCOUNTING__FINANCIAL_GENERAL_JOURNAL_MODEL =
    require('./accounting.financial_general_journal-model').MODEL
...
class Model extends BaseModel {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (
                    ...
                    return resolve({
                        error: true,
                        message: 'date|name|type|fundaID|customerID invalid',
                        keyError: KEY_ERROR.PARAMS_INVALID,
                        status: 400,
                    })

                //_________Tự động lấy số hiệu phiếu
                let number = await ACCOUNTING__FINANCIAL_VOUCHER_COLL.count({
                    funda: fundaID,
                    type: type,
                })
                ...
                if (voucherSign && voucherSign != '') {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(voucherSign)
                }
                if (signInvoice && signInvoice != '') {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(signInvoice)
                }
                if (receiver && receiver != '') {
                    convertStr =
                        convertStr + ' ' + stringUtils.removeAccents(receiver)
                }
                if (note && note != '') {
                    convertStr =
                ...
                if (subtypeID && checkObjectIDs(subtypeID)) {
                    ...
                    let infoSubtype =
                        await ITEM__DOCTYPE_COLL.findById(subtypeID).select(
                            'parent'
                        )
                ...
                if (selectedJournals && selectedJournals.length > 0) {
                    ...
                    for (const subitem of selectedJournals) {
                        ...
                        const dataInsert = {
                            ...
                            pricePolicy,
                            updown:
                                !isNaN(subitem.updown) &&
                                [1, -1].includes(Number(subitem.updown))
                                    ? Number(subitem.updown)
                                    : 1,
                            quantity: subitem.quantity
                                ? Number(subitem.quantity)
                                : 0,
                            ...
                            unitprice:
                                subitem.quantity != 0
                                    ? Number(
                                          Number(
                                              subitem.amount / subitem.quantity
                                          )
                                      )
                                    : 0,
                            fcuExRate,
                            fcuAmount:
                                fcuExRate != 0
                                    ? Number(subitem.amount / fcuExRate)
                                    : 0,
                            note: subitem.note,
                            ctx,
                        }
                         ...
                         */
                        if (
                            subitem.customerDebit &&
                            _isValid(subitem.customerDebit)
                        ) {
                        ...
                        if (
                            subitem.customerCredit &&
                            _isValid(subitem.customerCredit)
                        ) {
                        ...
                        if (
                            subitem.subtype &&
                            checkObjectIDs(subitem.subtype)
                        ) {
                            let infoSubtype = await ITEM__DOCTYPE_COLL.findById(
                                subitem.subtype
                            ).select('parent')
                            if (
                                infoSubtype.parent &&
                                _isValid(infoSubtype.parent)
                            ) {
                                parentSubtypeID = infoSubtype.parent
                            } else {
                                parentSubtypeID = subtypeID
                        ...
                        let infoJournal =
                            await ACCOUNTING__FINANCIAL_GENERAL_JOURNAL_MODEL.insert(
                                dataInsert
                            )
                ...
                if (type == 5 && linkFundaID && checkObjectIDs(linkFundaID)) {
                    await this.convertVoucher({
                        companyID,
                        docID: infoAfterInsert._id,
                        userID: authorID,
                        ctx,
                    })
                }
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let info =
                    await ACCOUNTING__FINANCIAL_VOUCHER_COLL.findById(voucherID)
                if (!info)
                    return resolve({
                        error: true,
                        message: 'cannot_get_info_voucher',
                        status: 400,
                    })
                ...
                if (sign && sign != '') {
                    let infoVoucher =
                        await ACCOUNTING__FINANCIAL_VOUCHER_COLL.findOne({
                            funda: fundaID,
                            sign: sign,
                            _id: {
                                $ne: voucherID,
                ...
                const infoAfterUpdate =
                    await ACCOUNTING__FINANCIAL_VOUCHER_COLL.findByIdAndUpdate(
                        voucherID,
                        dataUpdateVoucher,
                        { new: true }
                    )
                    ...
                    for (const subitem of selectedJournals) {
                        ...
                        if (!subitem.journalID) {
                            ...
                            const dataInsert = {
                                ...
                                unitprice:
                                    subitem.quantity != 0
                                        ...
                                        : 0, // Đơn giá tính theo thành tiền/khối lượng
                                fcuExRate,
                                fcuAmount:
                                    fcuExRate != 0
                                        ? Number(subitem.amount / fcuExRate)
                            ...
                            if (
                                ...
                                let infoSubtype =
                                    await ITEM__DOCTYPE_COLL.findById(
                                        subitem.subtype
                                    ).select('parent')
                                if (
                                    infoSubtype.parent &&
                        ...
                        // Cập nhật
                            ...
                            //_________Tiến hành tạo chi tiết_________//
                            let infoJournal =
                                await ACCOUNTING__FINANCIAL_GENERAL_JOURNAL_MODEL.update(
                                    dataUpdate
                                )
    ...
    updateValue({ voucherID }) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let listData =
                    await ACCOUNTING__FINANCIAL_GENERAL_JOURNAL_COLL.aggregate([
                        {
                            $match: {
                ...
                if (!resultUpdateVoucher)
                    return resolve({
                        erorr: true,
                        message: 'update_voucher_fail',
                        data: resultUpdateVoucher,
                        status: 400,
                    })

                return resolve({
                    erorr: false,
                    message: 'successfully_update_voucher',
                    data: resultUpdateVoucher,
                    status: 200,
                })
            } catch (error) {
                console.log(
                    `ERROR [ACCOUNTING][UPDATE_VOUCHER_WITH_NEW_JOURNAL]: ${error}`
                )
                return resolve({
                    erorr: true,
                    message: 'update_voucher_fail',
    ...
    convertVoucher({ companyID, docID, userID, ctx }) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                let infoVoucher =
                    await ACCOUNTING__FINANCIAL_VOUCHER_COLL.findById(
                        docID
                    ).populate({
                        path: 'linkFunda',
                ...
                if (
                    infoVoucher.type == 5 &&
                    infoVoucher.linkFunda &&
                    checkObjectIDs(infoVoucher.linkFunda._id)
                ) {
                    // console.log(infoVoucher)

                    let listAccount =
                        await ITEM__ACCOUNT_MODEL.getListRecursive({
                            companyID,
                            arrAccNames: ['152', '156'],
                        })
                    listAccount = listAccount.data.map((item) =>
                        item._id.toString()
                    )
                    // console.log(listAccount)

                    // Phải trả nội bộ
                    let info336 = await ITEM__ACCOUNT_COLL.findOne({
                        name: '336',
                    })
                    // console.log(info336)

                    let listJournals =
                        await ACCOUNTING__FINANCIAL_GENERAL_JOURNAL_COLL.find({
                            voucher: docID,
                        })
                    // console.log(listJournals)

                    let infoFunda = await ITEM_FUNDA_COLL.findById(
                        infoVoucher.funda
                    )
                    ...
                    for (const item of listJournals) {
                        if (listAccount.includes(item.credit.toString())) {
                            let dataInsert = {
                                ...
                                debit: item.credit,
                                credit: info336._id,
                                customerDebit: infoVoucher.linkFunda?.contact
                                    ? infoVoucher.linkFunda?.contact
                                    : item.customerDebit,
                                customerDebit: infoVoucher.linkFunda?.contact
                                    ? infoVoucher.linkFunda?.contact
                                    : item.customerDebit,
                                goods: item.goods,
                                warehouse: infoVoucher.linkFunda?.warehouse
                                    ? infoVoucher.linkFunda?.warehouse
                                    : item.warehouse,
                    ...
                    let infoNewVoucher = await that.insert({
                        ...
                        customerID: infoVoucher.linkFunda?.contact
                            ? infoVoucher.linkFunda?.contact
                            : infoVoucher.customer,
                        contractID: infoVoucher.contract,
                        storageID: infoVoucher.storage,
                        subtypeID: infoVoucher.subtype,
                        warehouseID: infoVoucher.linkFunda?.warehouse
                            ? infoVoucher.linkFunda?.warehouse
                            : infoVoucher.warehouse,
    ...
    updateFromVoucherToJournal({ companyID, userID }) {
        return new Promise(async (resolve, reject) => {
            try {
                let listData = await ACCOUNTING__FINANCIAL_VOUCHER_COLL.find({
                    company: companyID,
                })
                ...
                for (const item of listData) {
                    let infoAfterUpdate =
                        await ACCOUNTING__FINANCIAL_GENERAL_JOURNAL_COLL.updateMany(
                            { voucher: ObjectID(item._id) },
                            {
                                $set: {
                                    date: item.date,
                                    forward: !isNaN(item.forward)
                                        ? item.forward
                                        : 0,
                                    forwardIs: !isNaN(item.forwardIs)
                                        ? item.forwardIs
                                        : 0,
                                    returning: !isNaN(item.returning)
                                        ? item.returning
                                        : 0,
                                    advancePayment: !isNaN(item.advancePayment)
                                        ? item.advancePayment
                                        : 0,
                                    cancel: !isNaN(item.cancel)
                                        ? item.cancel
                                        : 0,
                                    type: !isNaN(item.type) ? item.type : 1,
                                    vat: !isNaN(item.vat) ? item.vat : 0,
                                    source: !isNaN(item.source)
                                        ? item.source
                                        : 1,
                                    orderNew: !isNaN(item.orderNew)
                                        ? item.orderNew
                                        : 1,
                                    pricePolicy: !isNaN(item.pricePolicy)
                                        ? item.pricePolicy
                                        : 1,
                                },
                            }
                ...
                return resolve({
                    erorr: false,
                    message: 'convert_successfull',
                    totalVoucher: listData.length,
                    status: 200,
                })
            } catch (error) {
                return resolve({
    ...
    updateFromJournalToVoucher({ companyID, userID }) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                let listVoucher = await ACCOUNTING__FINANCIAL_VOUCHER_COLL.find(
                    { company: companyID }
                ).select('_id name')
    ...
    getInfo({ voucherID, select, populates, ctx }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let info = await ACCOUNTING__FINANCIAL_VOUCHER_COLL.findById(
                    voucherID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet =
                    await ACCOUNTING__FINANCIAL_VOUCHER_COLL.find(conditionObj)
                        .limit(limit + 1)
                        .sort(sortBy)
                        .select(select)
                        .populate(populates)
                ...
                let totalRecord =
                    await ACCOUNTING__FINANCIAL_VOUCHER_COLL.count(
                        conditionObjOrg
                    )
