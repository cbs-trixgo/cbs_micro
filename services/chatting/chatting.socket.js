...
exports.CHATTING_SOCKET = {
        ...
        async function (data, ack) {
            ...
            if (!infoConversation) {
                return sendDataToMultilSocketsOfListUser({
                    ...
                    event: [
                        EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_NEW_CONVERSATION,
                    ],
                    io,
                })
            }
        ...
        async function (data, ack) {
            try {
                ...
                const arrReceiver = membersID.filter((memberID) => {
                    return (
                        memberID.toString() !== userID.toString() &&
                        !listMemberIDMuteConversation.includes(
                            memberID.toString()
                        )
                    )
                })
    ...
    [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_SEND_MESSAGE]: async function (
        ...
        try {
            ...
            switch (+type) {
                ...
                case 5:
                    infoMessageAfterInsert =
                        await CHATTING__MESSAGE_POLL_MODEL.insert({
                            conversationID,
                            name,
                            options,
                            authorID: userID,
            ...
            if (infoMessageAfterInsert.error) {
                return sendDataToMultilSocketsOfListUser({
                    ...
                    event: [
                        EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_RECEIVE_MESSAGE,
                    ],
                    io,
                })
            }
            ...
            listMemberWithAmountMissMessage.map(
                ({ memberID, amountMissMessage }) => {
                    // SEND SOCKET
                    sendDataToMultilSocketsOfListUser({
                        listUserConnected: listUserActive,
            ...
            console.info(
                'EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_SEND_MESSAGE'
            )
            console.error(error)
        }
    },
     ...
     * Date: 25/08/2021
     */
    [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_SHARE_MESSAGE]:
        async function (data, ack) {
            let socket = this
            let listUserActive = socket.$service.listUserConnected
            let io = socket.$service.io
            ...
            if (
                ...
                return sendDataToMultilSocketsOfListUser({
                    ...
                    event: [
                        EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_SHARE_MESSAGE,
                    ],
                    io,
                })
            }
     ...
     * Date: 11/01/2022
     */
    [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_REVOKE_MESSAGE]:
        async function (data) {
            let socket = this
            let listUserActive = socket.$service.listUserConnected
            let io = socket.$service.io
            ...
            if (!infoConversation || !messagesID || !messagesID.length) {
                return sendDataToMultilSocketsOfListUser({
                    ...
                    event: [
                        EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_REVOKE_MESSAGE,
                    ],
                    io,
                })
            }
    ...
    [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_PIN_MESSAGE]: async function (
        ...
        if (!infoConversation) {
            return sendDataToMultilSocketsOfListUser({
                ...
                event: [
                    EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_PIN_MESSAGE,
                ],
                io,
            })
        }
     ...
     * Date: 06/09/2021
     */
    [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_UNPIN_MESSAGE]:
        async function (data, ack) {
            let socket = this
            let broker = socket.$service.broker
            let listUserActive = socket.$service.listUserConnected
            ...
            if (!infoConversation) {
                return sendDataToMultilSocketsOfListUser({
                    ...
                    event: [
                        EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_UNPIN_MESSAGE,
                    ],
                    io,
                })
            }
    ...
    [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_FIRE_MESSAGE]: async function (
        ...
        if (!infoConversation) {
            return sendDataToMultilSocketsOfListUser({
                ...
                data: { error: true, message: "Can't get info conversation" },
                event: [
                    EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_FIRE_MESSAGE,
                ],
                io,
            })
        }
