 ...
 */
const {
  EVENT_SOCKET_CONSTANT_CHATTING,
} = require('./helper/chatting.events-socket-constant')
...
const CHATTING__MESSAGE_POLL_MODEL =
  require('./model/chatting.message_poll-model').MODEL
...
const CHATTING__REACTION_MESSAGE_MODEL =
  require('./model/chatting.message_reaction-model').MODEL
const {
...
exports.CHATTING_SOCKET = {
   ...
   * Date: ...
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_NEW_CONVERSATION]:
    async function (data, ack) {
      let socket = this
      let listUserActive = socket.$service.listUserConnected
      let userID = socket.client.user
      let io = socket.$service.io
      ...
      const infoConversation = await CHATTING__CONVERSATION_COLL.findById(
        conversationID
      )
        .populate({
          path: 'authors',
      ...
      if (!infoConversation) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_NEW_CONVERSATION],
          io,
        })
      }

      const { authors, members } = infoConversation
      const arrReceiver = members.filter(
   ...
   * Date: ...
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_UPDATE_INFO_CONVERSATION]:
    async function (data, ack) {
      let socket = this
      let listUserActive = socket.$service.listUserConnected
      let io = socket.$service.io
      ...
      if (!infoConversation) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [
            EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_UPDATE_INFO_CONVERSATION,
          ],
          io,
        })
      }

      const infoConversationResponse =
        await CHATTING__CONVERSATION_COLL.findById(conversationID)
   ...
   * Date: ...
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_ADD_MEMBER_CONVERSATION]:
    async function (data, ack) {
      let socket = this
      ...
      if (!infoConversation) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [
            EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_ADD_MEMBER_CONVERSATION,
          ],
          io,
        })
      }

      const infoSender = await broker.call(
      ...
      const addedMembers = await broker.call(
        `${CF_DOMAIN_SERVICES.AUTH}.${CF_ACTIONS_AUTH.GET_LIST_USER_WITH_CONDITION}`,
        {
      ...
      const listMemberIDMuteConversation = usersMute.map((userID) =>
        userID.toString()
      )
      ...
      const arrReceiver = members.filter((memberID) => {
        return (
          ...
          !listMemberIDMuteConversation.includes(memberID.toString())
        )
      })
   ...
   * Date: ...
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_REMOVE_MEMBER_CONVERSATION]:
    async function (data, ack) {
      let socket = this
      let broker = socket.$service.broker
      let listUserActive = socket.$service.listUserConnected
      ...
      const infoConversation =
        await CHATTING__CONVERSATION_COLL.findById(conversationID).lean()
      ...
      if (!infoConversation) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [
            EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_REMOVE_MEMBER_CONVERSATION,
          ],
          io,
        })
      }
      ...
      const removedMember = await broker.call(
        `${CF_DOMAIN_SERVICES.AUTH}.${CF_ACTIONS_AUTH.GET_LIST_USER_WITH_CONDITION}`,
        {
   ...
   * Date: 11/08/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_UPDATE_ADMIN_CONVERSATION]:
    async function (data, ack) {
      let socket = this
      let broker = socket.$service.broker
      let listUserActive = socket.$service.listUserConnected
      ...
      if (!infoConversation) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [
            EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_UPDATE_ADMIN_CONVERSATION,
          ],
          io,
        })
      }
      ...
      const infoMember = await broker.call(
        `${CF_DOMAIN_SERVICES.AUTH}.${CF_ACTIONS_AUTH.GET_INFO_BY_ID}`,
        {
          userID: memberID,
          select: '_id bizfullname fullname image',
   ...
   * Date: 15/08/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_MEMBER_LEAVE_CONVERSATION]:
    async function (data, ack) {
      try {
        let socket = this
        let broker = socket.$service.broker
        ...
        if (!infoConversation) {
          return sendDataToMultilSocketsOfListUser({
            ...
            event: [
              EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_MEMBER_LEAVE_CONVERSATION,
            ],
            io,
          })
        }
   ...
   * Date: 15/08/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_REMOVE_CONVERSATION]:
    async function (data, ack) {
      let socket = this
      let listUserActive = socket.$service.listUserConnected
      let io = socket.$service.io
      ...
      sendDataToMultilSocketsOfListUser({
        ...
        event: [
          EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_REMOVE_CONVERSATION,
        ],
        io,
      })
    },
   ...
   * Date: 17/08/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_SEND_MESSAGE]: async function (
    data
  ) {
    try {
      let socket = this
      ...
      if (infoMessageAfterInsert.error) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_RECEIVE_MESSAGE],
          io,
        })
      }
      ...
      const infoConversation =
        await CHATTING__CONVERSATION_COLL.findById(conversationID).lean()
      const { members, usersMute } = infoConversation
      const listMemberIDMuteConversation =
        usersMute?.map((userID) => userID.toString()) || []
      ...
      if (infoMessageAfterInsert.data) {
        infoMessage = await CHATTING__MESSAGE_COLL.findById(
          infoMessageAfterInsert.data?._id,
      ...
      for (const memberID of members) {
        ...
        listMemberWithAmountMissMessage[
          listMemberWithAmountMissMessage.length
        ] = {
          memberID,
          amountMissMessage,
        }
      ...
      let infoSender = await broker.call(
        `${CF_DOMAIN_SERVICES.AUTH}.${CF_ACTIONS_AUTH.GET_INFO_BY_ID}`,
        {
          userID,
   ...
   * Date: 20/02/2022
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_UPDATE_SEEN_MESSAGE]:
    async function (data, ack) {
      let socket = this
      let broker = socket.$service.broker
      let listUserActive = socket.$service.listUserConnected
      ...
      if (!infoConversation) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [
            EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_UPDATE_SEEN_MESSAGE,
          ],
          io,
        })
      }

      const infoUserSeen = await broker.call(
        `${CF_DOMAIN_SERVICES.AUTH}.${CF_ACTIONS_AUTH.GET_INFO_BY_ID}`,
        {
          userID,
          select: '_id bizfullname fullname image',
   ...
   * Date: 25/08/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_SHARE_MESSAGE]: async function (
    data,
    ack
  ) {
    let socket = this
    ...
    if (
      ...
      return sendDataToMultilSocketsOfListUser({
        ...
        data: {
          error: true,
          message: "Can't get info conversation or message",
        },
        event: [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_SHARE_MESSAGE],
        io,
      })
    }
    ...
    for (const conversationID of conversationsID) {
      ...
      const listMessagesShared = await CHATTING__MESSAGE_COLL.find({
        _id: { $in: messagesID },
      })
        .populate({
   ...
   * Date: 11/01/2022
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_REVOKE_MESSAGE]: async function (
    data
  ) {
    let socket = this
    let listUserActive = socket.$service.listUserConnected
    ...
    if (!infoConversation || !messagesID || !messagesID.length) {
      return sendDataToMultilSocketsOfListUser({
        ...
        event: [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_REVOKE_MESSAGE],
        io,
      })
    }
   ...
   * Date: 04/09/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_UPDATE_USER_JOIN_REMINDER]:
    async function (data, ack) {
      let socket = this
      let broker = socket.$service.broker
      let listUserActive = socket.$service.listUserConnected
      ...
      if (!infoConversation) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [
            EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_UPDATE_USER_JOIN_REMINDER,
          ],
          io,
        })
      }
   ...
   * Date: 06/09/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_UPDATE_MESSAGE_REMINDER]:
    async function (data, ack) {
      let socket = this
      let broker = socket.$service.broker
      let listUserActive = socket.$service.listUserConnected
      ...
      const infoConversation =
        await CHATTING__CONVERSATION_COLL.findById(conversationID)
      const infoMessage = await CHATTING__MESSAGE_COLL.findById(messageID)
        .populate('reminder')
        .populate({
          path: 'sender',
          select: '_id bizfullname fullname image',
      ...
      if (!infoConversation || !infoMessage) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [
            EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_UPDATE_MESSAGE_REMINDER,
          ],
          io,
        })
      }
   ...
   * Date: 06/09/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_REMOVE_MESSAGE_REMINDER]:
    async function (data, ack) {
      let socket = this
      let broker = socket.$service.broker
      let listUserActive = socket.$service.listUserConnected
      ...
      if (!infoConversation || !infoMessage) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [
            EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_REMOVE_MESSAGE_REMINDER,
          ],
          io,
        })
      }
   ...
   * Date: 06/09/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_UPDATE_MESSAGE_POLL]:
    async function (data, ack) {
      let socket = this
      let broker = socket.$service.broker
      let listUserActive = socket.$service.listUserConnected
      ...
      if (!infoConversation || !infoMessage) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [
            EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_UPDATE_MESSAGE_POLL,
          ],
          io,
        })
      }
   ...
   * Date: 04/06/2022
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_UPDATE_MESSAGE_NPS]:
    async function (data, ack) {
      let socket = this
      let broker = socket.$service.broker
      let listUserActive = socket.$service.listUserConnected
      ...
      if (!infoConversation || !infoMessage) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [
            EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_UPDATE_MESSAGE_NPS,
          ],
          io,
        })
      }
      ...
      sendDataToMultilSocketsOfListUser({
        ...
        event: [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_UPDATE_MESSAGE_NPS],
        io,
      })
    },
   ...
   * Date: 06/09/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_PIN_MESSAGE]: async function (
    data,
    ack
  ) {
    let socket = this
    ...
    if (!infoConversation) {
      return sendDataToMultilSocketsOfListUser({
        ...
        event: [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_PIN_MESSAGE],
        io,
      })
    }
   ...
   * Date: 06/09/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_UNPIN_MESSAGE]: async function (
    data,
    ack
  ) {
    let socket = this
    ...
    if (!infoConversation) {
      return sendDataToMultilSocketsOfListUser({
        ...
        event: [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_UNPIN_MESSAGE],
        io,
      })
    }
   ...
   * Date: 13/09/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_NOTIFICATION_REMINDER]:
    async function (data, ack) {
      let socket = this
      let broker = socket.$service.broker
      // let listUserActive       					= socket.$service.listUserConnected;
      ...
      const infoConversation =
        await CHATTING__CONVERSATION_COLL.findById(conversationID).select(
          'members'
        )
      const infoMessage = await CHATTING__MESSAGE_COLL.findById(messageID)
        .populate('reminder')
      ...
      const listMembersIDConversation = members.map((memberID) =>
        memberID.toString()
      )
      const arrReceiver = accepts.filter((memberID) =>
        listMembersIDConversation.includes(memberID.toString())
      )
   ...
   * Date: 26/10/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_REACTION_MESSAGE]:
    async function (data) {
      try {
        let socket = this
        let userID = socket.client.user
        ...
        if (!infoMessage) {
          return sendDataToMultilSocketsOfListUser({
            ...
            data: {
              error: true,
              message: "Can't get info message",
            },
            event: [
              EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_REACTION_MESSAGE,
            ],
            io,
          })
        }
        ...
        sendDataToMultilSocketsOfListUser({
          ...
          event: [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_REACTION_MESSAGE],
          io,
        })
      } catch (error) {
        console.info(
   ...
   * Date: 04/01/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_GET_INFO_CONVERSATION]:
    async function (data) {
      try {
        let socket = this
        let listUserActive = socket.$service.listUserConnected
        ...
        if (populates && typeof populates === 'string') {
          if (!IsJsonString(populates)) {
            return sendDataToMultilSocketsOfListUser({
              ...
              event: [
                EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_GET_INFO_CONVERSATION,
              ],
              io,
            })
          }
        ...
        const membersConversation = await CHATTING__CONVERSATION_COLL.findById(
          conversationID
        )
          .select('members')
          .lean()
   ...
   * Date: 16/01/2021
   */
  [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_CSS_FIRE_MESSAGE]: async function (
    data
  ) {
    let socket = this
    let userID = socket.client.user
    ...
    if (!infoConversation) {
      return sendDataToMultilSocketsOfListUser({
        ...
        arrReceiver: [userID],
        data: { error: true, message: "Can't get info conversation" },
        event: [EVENT_SOCKET_CONSTANT_CHATTING.CHATTING_SSC_FIRE_MESSAGE],
        io,
      })
    }
