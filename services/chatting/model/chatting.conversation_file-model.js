        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await CHATTING__CONVERSATION_FILE_COLL.findById(
                            lastestID
                        )
                    if (!infoData)
                        return resolve({
                ...
                let listFileConversation =
                    await CHATTING__CONVERSATION_FILE_COLL.find(conditionObj)
                        .limit(limit + 1)
                        .sort(sortBy)
                        .select(select)
                        .populate(populates)
                ...
                // GET TOTAL RECORD
                let totalRecord =
                    await CHATTING__CONVERSATION_FILE_COLL.count(
                        conditionObjOrg
                    )
                let nextCursor = null
