    ...
    return new Promise(async (resolve) => {
      try {
        ...
        const infoAfterUpdate =
          await CHATTING__CONVERSATION_MEMBER_COLL.updateMany(
            {
              member: userID,
            },
            {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (lastestID && checkObjectIDs(lastestID)) {
          let infoData =
            await CHATTING__CONVERSATION_MEMBER_COLL.findById(lastestID)
          if (!infoData)
            return resolve({
              error: true,
        ...
        let infoDataAfterGet = await CHATTING__CONVERSATION_MEMBER_COLL.find(
          conditionObj
        )
          .limit(limit + 1)
          .sort(sortBy)
        ...
        let totalRecord =
          await CHATTING__CONVERSATION_MEMBER_COLL.count(conditionObjOrg)
