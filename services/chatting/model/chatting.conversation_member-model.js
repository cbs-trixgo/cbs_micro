        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await CHATTING__CONVERSATION_MEMBER_COLL.findById(
                            lastestID
                        )
                    if (!infoData)
                        return resolve({
                ...
                let infoDataAfterGet =
                    await CHATTING__CONVERSATION_MEMBER_COLL.find(conditionObj)
                        .limit(limit + 1)
                        .sort(sortBy)
                        .select(select)
                        .populate(populates)
                ...
                let totalRecord =
                    await CHATTING__CONVERSATION_MEMBER_COLL.count(
                        conditionObjOrg
                    )
