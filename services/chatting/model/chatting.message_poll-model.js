                ...
                if (
                    ...
                    if (!listAuthorsID.includes(authorID)) {
                        return resolve({
                            ...
                            keyEror:
                                'you_do_not_have_permission_to_create_poll',
                            status: 403,
                        })
                    }
                }
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoMessagePoll = await CHATTING__MESSAGE_COLL.findById(
                    messageID
                )
                    .select('_id poll')
                    .populate('poll')
                ...
                if (newOptions && newOptions.length) {
                    let listOptionsName = newOptions.map(
                        (option) => option.title
                    )
                ...
                let infoMessagePollAfterUpdate =
