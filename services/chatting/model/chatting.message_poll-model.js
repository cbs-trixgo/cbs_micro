...
class Model extends BaseModel {
	...
	insert({ conversationID, name, options, authorID }) {
		return new Promise(async (resolve) => {
            try {
                if (!checkObjectIDs([authorID, conversationID]) || !name || !options.length) {
					return resolve({
						error: true,
				...
				if(checkExistConversation.config && checkExistConversation.config.configCreatePoll === 2){
				...
				// INSERT MESSAGE POLL
				let infoMessagePoll = await CHATTING__MESSAGE_MODEL.insertData(dataInsertMessage);
                ...
                await CHATTING__CONVERSATION_COLL.findByIdAndUpdate(conversationID, {
                    lastMessage: infoMessagePoll._id, modifyAt: Date.now()
                });
	 ...
	 * Date: 28/08/2021
	 */
	update({ conversationID, messageID, newVotes, votesCancel, newOptions, authorID }) {
		return new Promise(async (resolve) => {
			try {
				...
				let infoMessagePoll = await CHATTING__MESSAGE_COLL
					.findById(messageID)
					.select('_id poll')
					.populate('poll');
				...
				if(votesCancel && votesCancel.length && checkObjectIDs(votesCancel)){
					await CHATTING__MESSAGE_POLL_COLL.findByIdAndUpdate(poll._id, {
						$pull: {
				...
				if((votesCancel.length || newVotes.length || newOptions.length) && !isFirstTimeVote){
					lastMessage = await CHATTING__MESSAGE_MODEL.insertData({
						...dataInsertMessage,
				...
				let infoMessagePollAfterUpdate = await CHATTING__MESSAGE_COLL
					.findById(messageID)
					.populate({
						path: 'poll',
						populate: {
				...
				if(lastMessage){
					...
					await CHATTING__MESSAGE_MODEL.updateUserMissMessage({ conversationID, userID: authorID, numberMissMessage });
				}
	...
	closePoll({ conversationID, messageID, authorID }) {
		return new Promise(async resolve => {
			try {
				...
				const infoPoll = await CHATTING__MESSAGE_POLL_COLL.findByIdAndUpdate(poll._id, {
					$set: {
						status: 2
					}
				}, { new: true });
	 ...
	 * Date: 30/08/2021
	 */
	getList({ conversationID, lastestID, authorID, limit = 10, filter = {}, select = {} }){
		return new Promise(async (resolve) => {
			try {
				...
				let	listMessagesPoll = await CHATTING__MESSAGE_COLL
					.find(conditionObj, filter)
