...
const {
    REMINDER_NOTIFY_FOR,
    REMINDER_REPEAT,
} = require('../helper/chatting.keys-constant')
            ...
            try {
                if (!checkObjectIDs([authorID, conversationID])) {
                    return resolve({
                        error: true,
                        message:
                            'Tham số authorID hoặc conversationID không hợp lệ',
                ...
                if (
                    ...
                    checkExistConversation.config.configCreateNote === 2
                ) {
                    ...
                    if (!listAuthorsID.includes(authorID)) {
                        return resolve({
                            ...
                            keyEror:
                                'you_do_not_have_permission_to_create_reminder',
                            status: 403,
                        })
                    }
                }
                ...
                await CHATTING__CONVERSATION_COLL.findByIdAndUpdate(
                    conversationID,
                    {
                        lastMessage: infoMessageReminder._id,
                        modifyAt: Date.now(),
                    }
                )
                ...
                await ctx.call(
                    `${CF_DOMAIN_SERVICES.REMINDER}.${CF_ACTIONS_REMINDER.ADD_JOB_REMINDER_CONVERSATION}`,
                    {
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoMessageReminder = await CHATTING__MESSAGE_COLL.findById(
                    messageID
                )
                    .populate('reminder')
                    .lean()
                ...
                let infoReminderAfterUpdate =
                    await CHATTING__MESSAGE_REMINDER_COLL.findByIdAndUpdate(
                        reminder._id,
                        {
                            $set: {
                                content,
                ...
                let infoMessageNotify =
                    await CHATTING__MESSAGE_MODEL.insertData({
                        usersSeen: [authorID],
                        sender: authorID,
                        conversation: conversationID,
                        receivers: checkExistConversation.members,
                ...
                if (infoMessageNotify) {
                    await CHATTING__CONVERSATION_COLL.findByIdAndUpdate(
                        conversationID,
                        {
                            lastMessage: infoMessageNotify._id,
                            modifyAt: Date.now(),
                        }
                    )
                ...
                if (remindTime) {
                    ...
                    await ctx.call(
                        `${CF_DOMAIN_SERVICES.REMINDER}.${CF_ACTIONS_REMINDER.DELETE_JOB_REMINDER_CONVERSATION}`,
                        {
                            messageID,
                            conversationID,
                        }
