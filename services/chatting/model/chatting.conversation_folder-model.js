    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (removeFoldersID.length) {
          infoAfterUpdate = await CHATTING__CONVERSATION_COLL.findByIdAndUpdate(
            conversationID,
            {
              $pullAll: {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (lastestID && checkObjectIDs(lastestID)) {
          let infoData =
            await CHATTING__CONVERSATION_FOLDER_COLL.findById(lastestID)
          if (!infoData)
            return resolve({
              error: true,
        ...
        // GET TOTAL RECORD
        let totalRecord =
          await CHATTING__CONVERSATION_FOLDER_COLL.count(conditionObjOrg)
        let nextCursor = null
  ...
  delete({ folderID, authorID }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAfterDelete =
          await CHATTING__CONVERSATION_FOLDER_COLL.findByIdAndDelete(folderID)
