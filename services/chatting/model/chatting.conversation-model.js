    ...
    return new Promise(async (resolve) => {
      try {
        ...
        await PromisePool.for(membersID)
          .withConcurrency(2)
          .process(async (memberID) => {
            const infoMember = await CHATTING__CONVERSATION_MEMBER_COLL.findOne(
              {
                member: memberID,
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (isSetAdmin) {
          ...
          if (
            checkExistConversation.author.toString() === memberID.toString()
          ) {
            return resolve({
              error: true,
  ...
  updatePinConversation({ conversationID, userID, isPin, ctx }) {
    return new Promise(async (resolve) => {
      try {
        ...
        if (infoUserAfterUpdated.error) return resolve(infoUserAfterUpdated)
          ...
          listGroupConversationsOfMember.map((conversationOfMember) => {
            ...
            if (
              ...
              listGeneralConversation[listGeneralConversation.length] = {
                ...
                infoOwner:
                  conversationOfMember.authors && conversationOfMember.author,
                avatar:
  ...
  searchMessageConversationById({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        const totalRecordFront = await CHATTING__MESSAGE_COLL.countDocuments(
          conditionObjFrontList
        )
        const totalRecordBack =
          await CHATTING__MESSAGE_COLL.countDocuments(conditionObjBackList)
      ...
      try {
        ...
        let listMemberSendMediaInConversation =
          checkExistConversation.members.map(async (member) => {
            ...
            let totalImagePromise = CHATTING__MESSAGE_COLL.countDocuments({
              ...condition,
              type: 1,
            })
            let totalFilePromise = CHATTING__MESSAGE_COLL.countDocuments({
              ...condition,
              type: 2,
            })
            ...
            let [totalImage, totalFile, totalLink] = await Promise.all([
              totalImagePromise,
              totalFilePromise,
              totalLinkPromise,
            ])
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (checkExistConversation.author.toString() !== userID.toString())
          return resolve({
            error: true,
            message: "You are't root author of conversation (access denined)",
            status: 403,
          })

        let infoAfterDelete =
          await CHATTING__CONVERSATION_COLL.findByIdAndDelete(conversationID)
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (listConversationPin.data && listConversationPin.data.length) {
          ...
          if (!lastestID) {
            listConversationPin = await CHATTING__CONVERSATION_COLL.find(
              {
          ...
          for (const conversation of listConversation) {
            ...
            if (conversation.isPrivate && !conversation.isRename) {
              ...
              if (
                ...
                listConversationFiltered[listConversationFiltered.length] =
                  conversation
              } else if (
        ...
        // GET TOTAL RECORD
        let totalRecord =
          await CHATTING__CONVERSATION_COLL.countDocuments(conditionObj)
        let nextCursor = null
