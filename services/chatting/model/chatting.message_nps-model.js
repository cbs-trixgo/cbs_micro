...
const {
	checkObjectIDs, IsJsonString, validateParamsObjectID
...
class Model extends BaseModel {
        ...
        return new Promise(async (resolve) => {
            try {
				...
				if(!checkObjectIDs([conversationID, serviceID])) {
					return resolve({
						error: true,
						message: 'Tham số conversationID hoặc serviceID không hợp lệ',
				...
				// INSERT MESSAGE NPS
				let infoMessageNps = await CHATTING__MESSAGE_MODEL.insertData(dataInsertMessage);
                ...
                await CHATTING__CONVERSATION_COLL.findByIdAndUpdate(conversationID, {
                    lastMessage: infoMessageNps._id, modifyAt: Date.now()
                });
	 ...
	 * Date: 20/02/2022
	 */
	update({ conversationID, messageID, score, reasons, reasonsCancel, userID }) {
		return new Promise(async resolve => {
			try {
				...
				let isVoted = nps.usersVote.find(({ user }) => user.toString() === userID.toString());
				...
				switch (nps.type) {
					case this.NPS_SCORE: {
						 ...
						 */
						if(isVoted) {
							const removeUserVote = nps.usersVote.filter(item => item.user.toString() !== userID.toString());

							await CHATTING__MESSAGE_NPS_COLL.findByIdAndUpdate(nps._id, {
								$set: {
					...
					case this.NPS_SATISFIED: {
						...
						if(reasons && checkObjectIDs(reasons)){
							...
							reasons.map(reason => {
								const isVote = nps.usersVote.find(({ user, reason: reasonUser }) =>
									user?.toString() === userID && reasonUser.toString() === reason.toString())

								let isVoteCanceled = false;

								if(reasonsCancel && reasonsCancel.length && reasonsCancel.includes(reason.toString())) {
									isVoteCanceled = true;
								}
						...
						if(reasonsCancel && checkObjectIDs(reasonsCancel)){
							const voteCancel = nps.usersVote.filter(({ user, reason }) =>
								user?.toString() === userID && reasonsCancel.includes(reason.toString()));

							const voteIDCancel = voteCancel.map(vote => vote._id);
				...
				if(lastMessage){
					await CHATTING__CONVERSATION_COLL.findByIdAndUpdate(conversationID, {
						lastMessage: lastMessage._id, modifyAt: Date.now()
					});
	...
	closeNps({ conversationID, messageID, userID }) {
		return new Promise(async resolve => {
			try {
				...
				const infoNps = await CHATTING__MESSAGE_NPS_COLL.findByIdAndUpdate(nps._id, {
					$set: {
						status: 2
					}
				}, { new: true });
	 ...
	 * Date: 20/02/2022
	 */
	getList({ conversationID, lastestID, limit = 10, select, populates, userID }){
		return new Promise(async resolve => {
			try {
				...
				let listNps = await CHATTING__MESSAGE_COLL
					.find(conditionObj)
					.limit(limit + 1)
					.sort(sortBy)
					.select(select)
	...
	scoreStatistics({ conversationID, messageID, userID }) {
		return new Promise(async resolve => {
			try {
				...
				const dataStatistics = await CHATTING__MESSAGE_NPS_COLL.aggregate([
                    {
                        $match: {
