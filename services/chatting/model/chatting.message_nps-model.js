  ...
  update({ conversationID, messageID, score, reasons, reasonsCancel, userID }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let infoMessage = await CHATTING__MESSAGE_COLL.findById(messageID)
          .select('_id nps')
          .populate('nps')
          .lean()
              ...
              reasons.map((reason) => {
                const isVote = nps.usersVote.find(
                  ({ user, reason: reasonUser }) =>
                    ...
                    reasonUser.toString() === reason.toString()
                )
  ...
  closeNps({ conversationID, messageID, userID }) {
    return new Promise(async (resolve) => {
      try {
        ...
        const infoNps = await CHATTING__MESSAGE_NPS_COLL.findByIdAndUpdate(
          nps._id,
          {
            $set: {
              status: 2,
