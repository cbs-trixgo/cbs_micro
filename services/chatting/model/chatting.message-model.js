      ...
      try {
          ...
          let { results } = await PromisePool.for(files)
            ...
            .process(async (file) => {
              ...
              if (!infoFileCore.error) {
                ...
                return {
                  file: infoAfterInserted.data && infoAfterInserted.data._id,
                }
              }
            })
        ...
        switch (+type) {
          ...
          case 7: // MESSAGE SHARE CONTACT
            ...
            const { results } = await PromisePool.for(listMessageShareContact)
              .withConcurrency(2)
  ...
  deleteMessage({ conversationID, userID, messagesID, filesID }) {
    return new Promise(async (resolve) => {
      try {
        ...
        const { results: infoMessageAfterUpdated } = await PromisePool.for(
          listMessages
        )
          .withConcurrency(2)
          ...
          .process(async (message) => {
            ...
            if (isLastMessage) {
              ...
              await CHATTING__CONVERSATION_COLL.findByIdAndUpdate(
                ...
                {
                  $set: {
                    lastMessage: lastMessageOfConversation._id,
                  },
                }
              )
            }
            ...
            const values = await PromisePool.for(messagesID)
              ...
              .process(async (messageID) => {
                ...
                let infoMessageShare =
                  await CHATTING__MESSAGE_COLL.findById(messageID)
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (
          ...
          const listAuthorsID = infoConversation.authors.map((author) =>
            author.toString()
          )
          // User is not author of conversation
          ...
          await CHATTING__CONVERSATION_COLL.findById(conversationID)
            .populate({
              ...
              populate: {
                ...
                select:
                  '_id fullname bizfullname email image name options status content',
                populate: {
                  path: 'options.usersVote file',
                  select:
                    '_id bizfullname fullname image name nameOrg path size',
                },
              },
            })
            .lean()
