...
const CHATTING__CONVERSATION_FILE_MODEL =
    require('../model/chatting.conversation_file-model').MODEL
const CHATTING__CONVERSATION_LINK_MODEL =
    require('../model/chatring.conversation_link-model').MODEL
...
class Model extends BaseModel {
    ...
    updateUserMissMessage({ conversationID, userID, numberMissMessage }) {
        return new Promise(async (resolve) => {
            try {
                ...
                if (
                    !infoConversation.usersMissMessage ||
                    !infoConversation.usersMissMessage.length
                ) {
            ...
            try {
                if (!checkObjectIDs([senderID, conversationID])) {
                    return resolve({
                        error: true,
                        message:
                            'Tham số senderID hoặc conversationID không hợp lệ',
                ...
                if (
                    ...
                    checkExistConversation.config.configSendMessage === 2
                ) {
                    ...
                    if (!listAuthorsID.includes(senderID)) {
                        return resolve({
                            error: true,
                            message:
                                'Bạn không có quyền gửi tin nhắn trong cuộc hội thoại này, vui lòng liên hệ người tạo nhóm chat',
                            keyError:
                                'you_do_not_have_permission_to_send_message',
                            status: 403,
                        })
                    }
                }
                ...
                usersAssigned &&
                    (dataInsertMessage.usersAssigned = usersAssigned)

                // MESSAGE IMAGES, FILES
                if (
                    files &&
                    files.length &&
                    checkNumberIsValidWithRange({ arrValid: [1, 2], val: type })
                ) {
                    let { results } = await PromisePool.for(files)
                        .withConcurrency(2)
                        .process(async (file) => {
                            ...
                            if (!infoFileCore.error) {
                                ...
                                const infoAfterInserted =
                                    await CHATTING__CONVERSATION_FILE_MODEL.insert(
                                        {
                                            conversationID,
                                            fileID: file._id,
                                            description,
                ...
                switch (+type) {
                    ...
                    case 7: // MESSAGE SHARE CONTACT
                        ...
                        let listMessageShareContact = usersAssigned.map(
                            (memberID) => ({
                                usersSeen: [senderID],
                                sender: senderID,
                                receivers: receiversID,
                    ...
                    default: // MESSAGE NORMAL OR NOTI
                        infoMessageAfterInserted =
                            await this.insertData(dataInsertMessage)
                        break
                }
                ...
                if (content) {
                    let urlRegex =
                        /((((https|http|HTTP|HTTPS|ftp)?:\/\/)|(www\.))[^\s]+)/g
                    let allLinks = content.match(urlRegex)

                    if (allLinks && allLinks.length) {
                        await PromisePool.for(allLinks)
                            .withConcurrency(2)
                            .process(async (link) =>
                                CHATTING__CONVERSATION_LINK_MODEL.insert({
                                    conversationID,
                ...
                // UPDATE AMOUNT MESSAGE USER MISS IN CONVERSATION
                await this.updateUserMissMessage({
                    conversationID,
                    userID: senderID,
                    numberMissMessage,
                })
            ...
            try {
                ...
                if (!checkObjectIDs([conversationID, ...messagesID])) {
                    return resolve({
                        error: true,
                        message:
                            'Tham số conversationID hoặc messagesID không hợp lệ',
                ...
                const { results } = await PromisePool.for(messagesID)
                    .withConcurrency(2)
                    .process(async (messageID) => {
                        let infoMessageAfterUpdate =
                            await CHATTING__MESSAGE_COLL.findByIdAndUpdate(
                                messageID,
                                {
                                    $addToSet: {
                                        usersSeen: userID,
                        ...
                        if (
                            infoConversation.members.length ===
                            infoMessageAfterUpdate.usersSeen.length
                        ) {
                            infoMessageAfterUpdate =
                                await CHATTING__MESSAGE_COLL.findByIdAndUpdate(
                                    messageID,
                                    {
                                        $set: { status: 2 },
            ...
            try {
                if (!checkObjectIDs([...messagesID, userID, conversationID])) {
                    return resolve({
                        error: true,
                        message:
                            'Tham số messagesID hoặc conversationID không hợp lệ',
                ...
                const { results: infoMessageAfterUpdated } =
                    await PromisePool.for(listMessages)
                        .withConcurrency(2)
                        .process(async (message) => {
                            ...
                            switch (message.type) {
                                ...
                                case 7: // Delete message contact
                                    infoMessageAfterUpdated =
                                        await CHATTING__MESSAGE_COLL.findOneAndUpdate(
                                            {
                                                _id: message._id,
                                                conversation: conversationID,
                                            },
            ...
            try {
                ...
                if (!listMessages || !listMessages.length) {
                    return resolve({
                        error: true,
                        message:
                            'Tin nhắn không thuộc cuộc hội thoại hoặc tin nhắn không tồn tại',
                ...
                listMessages.map((message) => {
                    ...
                    if (expireTime > now) {
                        listMessagesNotExpire[listMessagesNotExpire.length] =
                            message
                    }
                })
                ...
                const { results } = await PromisePool.for(listMessagesNotExpire)
                    .withConcurrency(2)
                    .process(async (message) => {
                        let infoMessageAfterUpdated = null
                        ...
                        switch (message.type) {
                            ...
                            case 7: // Recall message share contact
                                infoMessageAfterUpdated =
                                    await CHATTING__MESSAGE_COLL.findByIdAndDelete(
                                        message._id
                                    )
                                break
                        ...
                        let isLastMessage =
                            checkExistConversation.lastMessage?.toString() ===
                            message._id.toString()

                        if (isLastMessage) {
                            let lastMessageOfConversation =
                                await CHATTING__MESSAGE_COLL.findOne({
                                    conversation: conversationID,
                                })
                                    .select('_id')
                                    .sort({ _id: -1 })
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (isDelete) {
                    ...
                    const lastMessage = await CHATTING__MESSAGE_COLL.find({
                        conversation: conversationID,
                    })
                        .sort({ _id: -1 })
                        .limit(1)
                    ...
                    await ctx.call(
                        `${CF_DOMAIN_SERVICES.REMINDER}.${CF_ACTIONS_REMINDER.DELETE_JOB_AUTO_DELETE_MESSAGE_CONVERSATION}`,
                        {
                            conversationID,
                        }
                    )
                    ...
                    if (+time !== 0) {
                        await ctx.call(
                            `${CF_DOMAIN_SERVICES.REMINDER}.${CF_ACTIONS_REMINDER.ADD_JOB_AUTO_DELETE_MESSAGE_CONVERSATION}`,
                            {
                                conversationID,
                                messageID: lastMessage.length
                                    ? lastMessage[0]._id?.toString()
                                    : '',
                                userID,
                                time,
                            }
    ...
    shareMessageConversation({ conversationsID, messagesID, userID }) {
        return new Promise(async (resolve) => {
            try {
                if (
                    !checkObjectIDs([...conversationsID, ...messagesID, userID])
                )
                    return resolve({
                        error: true,
                        message: 'Request Params invalid',
                ...
                let listConversations = await CHATTING__CONVERSATION_COLL.find({
                ...
                if (!listConversations)
                    return resolve({
                        error: true,
                        message: "You aren't member of conversations",
                        status: 403,
                    })

                const { results } = await PromisePool.for(listConversations)
                    .withConcurrency(2)
                    .process(async (conversation) => {
                        const values = await PromisePool.for(messagesID)
                            .withConcurrency(2)
                            .process(async (messageID) => {
                                // Get info message share
                                let infoMessageShare =
                                    await CHATTING__MESSAGE_COLL.findById(
                                        messageID
                                    )
                                ...
                                await CHATTING__CONVERSATION_COLL.findByIdAndUpdate(
                                    conversation._id,
                                    {
                                        lastMessage: infoAfterInserted._id,
                                        modifyAt: Date.now(),
                                    },
                                    { new: true }
                                )
    ...
    updatePinMessageConversation({ conversationID, messageID, userID, isPin }) {
        return new Promise(async (resolve) => {
            try {
                ...
                // Check config admin conversation
                if (
                    infoConversation.config &&
                    infoConversation.config.configPinMessage === 2
                ) {
                    const listAuthorsID = infoConversation.authors.map(
                        (author) => author.toString()
                    )
                    // User is not author of conversation
                    if (!listAuthorsID.includes(userID)) {
                        return resolve({
                            ...
                            keyEror:
                                'you_do_not_have_permission_to_pin_message',
                            status: 403,
                        })
                    }
                }
    ...
    getListMessagePinConversation({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let listMessagesPined = await CHATTING__MESSAGE_COLL.find(
                    {
                ...
                if (!listMessagesPined)
                    return resolve({
                        error: true,
                        message: "Can't get list message pin",
                        status: 403,
                    })
    ...
    getListMessageReminder({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (isJoined) {
                    conditionJoined['reminder.accepts'] = {
                        $in: [ObjectID(userID)],
                    }
                }

                let listMessagesReminder =
                    await CHATTING__MESSAGE_COLL.aggregate([
                        {
