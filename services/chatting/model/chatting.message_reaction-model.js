  ...
  reactionMessage({ userID, messageID, typeReaction }) {
    return new Promise(async (resolve) => {
      try {
        ...
        const infoReaction = await CHATTING__MESSAGE_REACTION_COLL.findOne({
          message: messageID,
          author: userID,
        }).lean()
        ...
        if (infoReaction) {
          ...
          await CHATTING__MESSAGE_REACTION_COLL.findByIdAndDelete(
            infoReaction._id
          )
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (lastestID && checkObjectIDs(lastestID)) {
          let infoData =
            await CHATTING__MESSAGE_REACTION_COLL.findById(lastestID)
          if (!infoData)
            return resolve({
              error: true,
        ...
        let listReactions = await CHATTING__MESSAGE_REACTION_COLL.find(
          conditionObj
        )
          .limit(limit + 1)
          .sort(sortBy)
        ...
        // GET TOTAL RECORD
        let totalRecord =
          await CHATTING__MESSAGE_REACTION_COLL.count(conditionObjOrg)
        let nextCursor = null
