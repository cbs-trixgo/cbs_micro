        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await CHATTING__CONVERSATION_LINK_COLL.findById(
                            lastestID
                        )
                    if (!infoData)
                        return resolve({
                ...
                let listLinksConversation =
                    await CHATTING__CONVERSATION_LINK_COLL.find(conditionObj)
                        .limit(limit + 1)
                        .sort(sortBy)
                        .select(select)
                        .populate(populates)
                ...
                // GET TOTAL RECORD
                let totalRecord =
                    await CHATTING__CONVERSATION_LINK_COLL.count(
                        conditionObjOrg
                    )
                let nextCursor = null
