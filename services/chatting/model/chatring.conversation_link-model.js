...
class Model extends BaseModel {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(!infoAfterInserted)
                    return resolve({ error: true, message: "Can't create link", status: 403 });

                return resolve({ error: false, data: infoAfterInserted, status: 200 });
            } catch (error) {
	 ...
	 * Date: 10/03/2022
	 */
	getList({ userID, authorID, conversationID, messageID, lastestID, fromDate, toDate, keyword, limit = 100, select, populates }){
        return new Promise(async resolve => {
            try {
				...
				if(lastestID && checkObjectIDs(lastestID)){
					let infoData = await CHATTING__CONVERSATION_LINK_COLL.findById(lastestID);
					if(!infoData)
                ...
                let listLinksConversation = await CHATTING__CONVERSATION_LINK_COLL
					.find(conditionObj)
					.limit(limit + 1)
					.sort(sortBy)
					.select(select)
				...
				// GET TOTAL RECORD
                let totalRecord = await CHATTING__CONVERSATION_LINK_COLL.count(conditionObjOrg);
				let nextCursor	= null;
