  ...
  update: {
    ...
    async handler(ctx) {
      try {
        await this.validateEntity(ctx.params)
        const { _id: authorID } = ctx.meta.infoUser
        const { conversationID, messageID, newVotes, votesCancel, newOptions } =
          ctx.params

        const resultAfterCallHandler =
          await CHATTING__MESSAGE_POLL_MODEL.update({
            conversationID,
            messageID,
            newVotes,
            votesCancel,
  ...
  closePoll: {
    ...
    async handler(ctx) {
      try {
        ...
        const resultAfterCallHandler =
          await CHATTING__MESSAGE_POLL_MODEL.closePoll({
            conversationID,
            messageID,
            authorID,
          })
  ...
  getList: {
    ...
    async handler(ctx) {
      try {
        ...
        const resultAfterCallHandler =
          await CHATTING__MESSAGE_POLL_MODEL.getList({
            conversationID,
            lastestID,
            authorID,
            limit,
