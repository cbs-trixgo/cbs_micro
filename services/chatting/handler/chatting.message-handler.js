...
const CHATTING__MESSAGE_REACTION_MODEL =
    ...
    updateSeenMessage: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await CHATTING__MESSAGE_MODEL.updateSeenMessage({
                        conversationID,
                        messagesID,
                        userID,
                    })
    ...
    deleteMessage: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await CHATTING__MESSAGE_MODEL.deleteMessage({
                        conversationID,
                        userID,
                        messagesID,
                        filesID,
    ...
    revokeMessage: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await CHATTING__MESSAGE_MODEL.revokeMessage({
                        conversationID,
                        userID,
                        messagesID,
                        filesID,
    ...
    autoDeleteMessagesConversation: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await CHATTING__MESSAGE_MODEL.autoDeleteMessagesConversation(
                        {
                            conversationID,
                            messageID,
                            userID,
    ...
    shareMessageConversation: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await CHATTING__MESSAGE_MODEL.shareMessageConversation({
                        conversationsID,
                        messagesID,
                        userID,
                    })
    ...
    updatePinMessageConversation: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await CHATTING__MESSAGE_MODEL.updatePinMessageConversation({
                        conversationID,
                        messageID,
                        userID,
                        isPin,
    ...
    getListMessagePinConversation: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params)
                const { _id: userID } = ctx.meta.infoUser
                const { conversationID, page, limit, filter, select } =
                    ctx.params

                const resultAfterCallHandler =
                    await CHATTING__MESSAGE_MODEL.getListMessagePinConversation(
                        {
                            conversationID,
                            page,
                            limit,
    ...
    reactionMessage: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await CHATTING__MESSAGE_REACTION_MODEL.reactionMessage({
                        messageID,
                        userID,
                        typeReaction,
                    })
    ...
    getListReactionByMessage: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params)
                const { _id: userID } = ctx.meta.infoUser
                const { messageID, type, lastestID, limit, select, populates } =
                    ctx.params

                const resultAfterCallHandler =
                    await CHATTING__MESSAGE_REACTION_MODEL.getListReactionMessage(
                        {
                            messageID,
                            type,
                            lastestID,
