        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await OPERATION__UTILITY_MODEL.insert({
					name, description, latitude, longtitude, userID
				});
    ...
    update: {
        ...
        async handler(ctx) {
            try {
				await this.validateEntity(ctx.params);
				const { _id: userID } = ctx.meta.infoUser;
                const { utilityID, name, description, latitude, longtitude } = ctx.params;
                const resultAfterCallHandler = await OPERATION__UTILITY_MODEL.update({
                    utilityID, name, description, latitude, longtitude, userID
				});
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
				await this.validateEntity(ctx.params);
				const { _id: userID } = ctx.meta.infoUser;
                const { utilityID, latitude, longtitude, keyword, limit, lastestID, filter  } = ctx.params;
                ...
                if(utilityID){
                    ...
                    resultAfterCallHandler = await OPERATION__UTILITY_MODEL.getList({
                        latitude, longtitude, keyword, limit, lastestID, filter
                    });
                }
