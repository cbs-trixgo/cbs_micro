        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await OPERATION__UTILITY_MODEL.insert({
                        name,
                        description,
                        latitude,
                        longtitude,
        ...
        async handler(ctx) {
            try {
                ...
                const { utilityID, name, description, latitude, longtitude } =
                    ctx.params
                const resultAfterCallHandler =
                    await OPERATION__UTILITY_MODEL.update({
                        utilityID,
                        name,
                        description,
                        latitude,
    ...
    remove: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await OPERATION__UTILITY_MODEL.remove({
                        utilityID,
                    })
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                ...
                if (utilityID) {
                    resultAfterCallHandler =
                        await OPERATION__UTILITY_MODEL.getInfo({
                            utilityID,
                        })
                } else {
                    resultAfterCallHandler =
                        await OPERATION__UTILITY_MODEL.getList({
                            latitude,
                            longtitude,
                            keyword,
                            limit,
