 ...
 */
  ...
  * MODELS
  */
 const OPERATION__APAERTMENT_MODEL                 = require('../model/operation.apartment').MODEL;
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await OPERATION__APAERTMENT_MODEL.insert({
                    projectID, name, sign, parentID, description, userID
				});
    ...
    update: {
        ...
        async handler(ctx) {
            try {
				await this.validateEntity(ctx.params);
				const { _id: userID } = ctx.meta.infoUser;
                const { apartmentID, projectID, parentID, name, sign, description, area, startTime, expenses, ownerID, adminsID, adminRemove, membersID, memberRemove } = ctx.params;

                const resultAfterCallHandler = await OPERATION__APAERTMENT_MODEL.update({
                    apartmentID, projectID, parentID, name, sign, description, area, startTime, expenses, ownerID, adminsID, adminRemove, membersID, memberRemove, userID
				});
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
				await this.validateEntity(ctx.params);
				const { _id: userID } = ctx.meta.infoUser;
                const { apartmentID, projectID, parentID, ownerID, keyword, limit, lastestID, filter, select  } = ctx.params;
                ...
                if(apartmentID){
                    ...
                    resultAfterCallHandler = await OPERATION__APAERTMENT_MODEL.getList({
                        apartmentID, projectID, parentID, ownerID, keyword, limit, lastestID, filter 
                    });
                }
