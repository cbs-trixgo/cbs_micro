    ...
    insert: {
        ...
        async handler(ctx) {
            try {
                ...
                const { projectID, name, sign, parentID, description } =
                    ctx.params

                const resultAfterCallHandler =
                    await OPERATION__APAERTMENT_MODEL.insert({
                        projectID,
                        name,
                        sign,
                        parentID,
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await OPERATION__APAERTMENT_MODEL.update({
                        apartmentID,
                        projectID,
                        parentID,
                        name,
    ...
    remove: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await OPERATION__APAERTMENT_MODEL.remove({
                        apartmentID,
                    })
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                ...
                if (apartmentID) {
                    resultAfterCallHandler =
                        await OPERATION__APAERTMENT_MODEL.getInfo({
                            apartmentID,
                            select,
                        })
                } else {
                    resultAfterCallHandler =
                        await OPERATION__APAERTMENT_MODEL.getList({
                            apartmentID,
                            projectID,
                            parentID,
                            ownerID,
