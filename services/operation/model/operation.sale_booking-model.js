    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (!checkObjectIDs(saleBookingID))
          return resolve({
            error: true,
            message: 'Request params saleBookingID invalid',
            status: 400,
          })
        ...
        let infoAfterUpdate =
          await OPERATION__SALE_BOOKING_COLL.findByIdAndUpdate(
            saleBookingID,
            dataUpdate,
            { new: true }
          )
  ...
  remove({ saleBookingID }) {
    return new Promise(async (resolve) => {
      try {
         ...
         */
        let infoAterRemove =
          await OPERATION__SALE_BOOKING_COLL.findByIdAndDelete(saleBookingID)
        if (!infoAterRemove)
  ...
  getInfo({ saleBookingID }) {
    return new Promise(async (resolve) => {
      try {
         ...
         */
        let infoAterRemove =
          await OPERATION__SALE_BOOKING_COLL.findById(saleBookingID)
        if (!infoAterRemove)
          return resolve({ error: true, message: "can't_remove", status: 403 })
  ...
  getList({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoDataAfterGet = await OPERATION__SALE_BOOKING_COLL.find(
        ...
        let totalCurrentPage =
          await OPERATION__SALE_BOOKING_COLL.count(conditionFind)
        if (!infoDataAfterGet)
          return resolve({
            error: true,
        ...
        let totalRecord =
          await OPERATION__SALE_BOOKING_COLL.countDocuments(conditionObj)
