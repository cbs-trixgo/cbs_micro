        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(!checkObjectIDs(utilityID))
                    return resolve({ error: true, message: 'Request params utilityID invalid', status: 400 });
                ...
                let infoAfterUpdate = await OPERATION__UTILITY_COLL.findByIdAndUpdate(utilityID, dataUpdate, { new: true });
                if(!infoAfterUpdate)
                    return resolve({ error: true, message: "can't_insert", status: 403 });
    ...
    remove({ utilityID }) { 
        return new Promise(async (resolve) => {
            try {
                 ...
                 * 3.2: Operation database
                 */
                let infoAterRemove = await OPERATION__UTILITY_COLL.findByIdAndDelete(utilityID);
                if(!infoAterRemove)
    ...
    getInfo({ utilityID }) { 
        return new Promise(async (resolve) => {
            try {
                 ...
                 * 3.2: Operation database
                 */
                let infoAterRemove = await OPERATION__UTILITY_COLL.findById(utilityID);
                if(!infoAterRemove)
                    return resolve({ error: true, message: "can't_remove", status: 403 });
        ...
        return new Promise(async (resolve) => {
            try {
				...
				if(lastestID && checkObjectIDs(lastestID)){
					let infoData = await OPERATION__UTILITY_COLL.findById(lastestID);
					if(!infoData)
                ...
                let infoDataAfterGet = await OPERATION__UTILITY_COLL.find({...conditionObj, ...conditionFind }, { ...filter, ...objSort })
                    .limit(+limit+1)
                    .sort(sortBy)
                    .lean(); 

                  // GET TOTAL RECORD
                let totalCurrentPage = await OPERATION__UTILITY_COLL.count(conditionFind);
                if(!infoDataAfterGet)
                ...
                let totalRecord = await OPERATION__UTILITY_COLL.countDocuments(conditionObj);
