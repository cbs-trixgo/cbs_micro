    ...
    insert({ projectID, parentID, name, sign, description, userID }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                let checkExist = await OPERATION__APARTMENT_COLL.findOne({
                    project: projectID,
                    sign,
                })
                if (checkExist)
                ...
                if (!infoProject || infoProject.error)
                    return resolve({
                        error: true,
                        message: 'Request params projectID invalid',
                        status: 400,
                    })
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!checkObjectIDs(apartmentID))
                    return resolve({
                        error: true,
                        message: 'Request params apartmentID invalid',
                        status: 400,
                    })
                ...
                let infoAfterUpdate =
                    await OPERATION__APARTMENT_COLL.findByIdAndUpdate(
                        apartmentID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    remove({ apartmentID }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                let infoAterRemove =
                    await OPERATION__APARTMENT_COLL.findByIdAndDelete(
                        apartmentID
                    )
                if (!infoAterRemove)
                    return resolve({
    ...
    getInfo({ apartmentID }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                let infoAterRemove =
                    await OPERATION__APARTMENT_COLL.findById(apartmentID)
                if (!infoAterRemove)
                    return resolve({
                        error: true,
                        message: "can't_remove",
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await OPERATION__APARTMENT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await OPERATION__APARTMENT_COLL.find(
                ...
                let totalCurrentPage =
                    await OPERATION__APARTMENT_COLL.count(conditionFind)
                if (!infoDataAfterGet)
                    return resolve({
                        error: true,
                ...
                let totalRecord =
                    await OPERATION__APARTMENT_COLL.countDocuments(conditionObj)
