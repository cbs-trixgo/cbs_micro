        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!accessToken) {
                    return resolve({
                        error: true,
                        message: 'Not found page access token',
                    })
                }
                ...
                if (!recipientId && !phone) {
                    return resolve({
                        error: true,
                        message: 'RecipientID or phone is required',
                    })
                }

                if (phone) {
                    const fbMessage = await FACEBOOK_MESSAGE_COLL.findOne({
                        phones: { $in: [phone] },
                    }).lean()
                ...
                entry.map((item) => {
                    ...
                    item.messaging?.map((mess) => {
                        ...
                        if (phoneNumbers && phoneNumbers.length) {
                            ...
                            console.log(
                                'No phone numbers found in the message content.'
                            )
                        }
                    })
                })
