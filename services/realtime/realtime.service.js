                ...
                '/': {
                    ...
                    middlewares: [
                        async function (socket, next) {
                            ...
                            // Xét user Online
                            const _infoUser =
                                await USER_MODEL.getDataById(userID)
                    ...
                    events: {
                        ...
                        disconnect: async function () {
                            ...
                            this.$service.removeUserDisconnectIntoListConnected(
                                socketDisconnectID
                            )
    ...
    methods: {
        async socketAuthorize(socket) {
            try {
                ...
                let checkAuthorization = await this.broker.call(
                    `${CF_DOMAIN_SERVICES.AUTH}.${CF_ACTIONS_AUTH.RESOLVE_TOKEN}`,
                    { token }
                )
                ...
                let infoUser =
                    checkAuthorization.data && checkAuthorization.data.infoUser

                this.logger.info(
                    'Authenticated via JWT: ',
                    infoUser.bizfullname
                )
        ...
        addNewUserIntoListConnected(socketID) {
            ...
            if (listUserConnected[_userID]) {
                listUserConnected[_userID].sockets = listUserConnected[
                    _userID
                ].sockets.filter((_socketID) => _socketID != socketID)
                listUserConnected[_userID].sockets = [
                    ...listUserConnected[_userID].sockets,
                    socketID,
                ]
        ...
        removeUserDisconnectIntoListConnected(socketDisconnectID) {
            ...
            let listUserConnected = this.listUserConnected
            socketDisconnectID =
                socketDisconnectID && socketDisconnectID.toString()

            if (userID) {
                listUserConnected[userID].sockets = listUserConnected[
                    userID
                ].sockets.filter((_socketID) => _socketID != socketDisconnectID)
            }
