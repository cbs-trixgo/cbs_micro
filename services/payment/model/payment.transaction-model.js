    ...
    updateStatus({ orderID, status }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let isExistsTransaction =
                    await PAYMENT__TRANSACTION_COLL.findOne({
                        orderID,
                    })
                ...
                if (
                    isExistsTransaction.status !=
                    TRANSACTION_WITH_STATUS_WAITING
                )
                    return resolve({
                        error: true,
    ...
    getListByUserWithStatus({ userID, status = 'all' }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let listTransactionsOfUser =
                    await PAYMENT__TRANSACTION_COLL.find(conditionObj).sort({
                        createAt: -1,
                    })
