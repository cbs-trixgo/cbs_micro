...
let PAYMENT__TRANSACTION_MODEL =
  require('../model/payment.transaction-model').MODEL
let {
  vnp_TmnCode,
  vnp_HashSecret,
  vnp_Url,
...
module.exports = {
    ...
    async handler(ctx) {
      try {
        ...
        let listTransactions =
          await PAYMENT__TRANSACTION_MODEL.getListTransactions({
            status,
          })
  ...
  getListTransactionsByUserWithStatus: {
    ...
    async handler(ctx) {
      try {
        ...
        let listTransactions =
          await PAYMENT__TRANSACTION_MODEL.getListByUserWithStatus({
            userID,
            status,
          })
        return listTransactions
  ...
  updateStatusTransaction: {
    ...
    async handler(ctx) {
      try {
        ...
        let infoTransactionAfterUpdateStatus =
          await PAYMENT__TRANSACTION_MODEL.updateStatus({
            transactionID,
            status,
          })
  ...
  createURLPayment: {
    ...
    async handler(ctx) {
      try {
        await this.validateEntity(ctx.params)
        let { orderType, amount, orderDescription, bankCode, language } =
          ctx.params
        ...
        let infoTransactionAfterInsert =
          await PAYMENT__TRANSACTION_MODEL.insert({
            orderID: orderId,
            orderInfo,
            amount,
            bankCode,
  ...
  URL_RETURN: {
    ...
    async handler(ctx) {
      try {
        ...
        if (
          secureHash === checkSum &&
          transactionStatus == VNPAY_TRANSACTION_WITH_STATUS_SUCCESS
        ) {
          ...
          let infoTransactionAfterUpdateStatus =
            await PAYMENT__TRANSACTION_MODEL.updateStatus({
              orderID: orderId,
              status: TRANSACTION_WITH_STATUS_SUCCESS,
            })

          if (infoTransactionAfterUpdateStatus.error)
            return {
              error: true,
              message: infoTransactionAfterUpdateStatus.message,
            }
