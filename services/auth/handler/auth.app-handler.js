    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                ...
                let { appID, lock, keyword, limit, lastestID, select, populates } = ctx.params;
                let { _id: userID } = ctx.meta.infoUser;
                
                let resultAfterCallHandler;
                if(appID){
                    resultAfterCallHandler = await AUTH__APP_MODEL.getInfo({ appID, select, populates });
                }else{
                    resultAfterCallHandler = await AUTH__APP_MODEL.getList({ lock, keyword, limit, lastestID, select, populates });
                }
    ...
    checkVersionApp: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler = { error: false, data: { latestVersion: false }}
                //Teaser
                if(env == 2){
                    if(deviceType == "ios"){
                        if(version == infoAppVersion.Teaser.ios.version){
                           resultAfterCallHandler = { error: false, data: { latestVersion: true }}
                           return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx });
                        }
                    }
