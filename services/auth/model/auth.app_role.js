        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!checkObjectIDs(app))
                    return resolve({
                        error: true,
                        message: 'Mã ứng dụng không hợp lệ',
                        keyError: 'app_invalid',
                        status: 400,
                    })
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await AUTH__APP_ROLE_COLL.findByIdAndUpdate(
                        appRoleID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    remove({ appRoleID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAterRemove =
                    await AUTH__APP_ROLE_COLL.findByIdAndDelete(appRoleID)
                if (!infoAterRemove)
                    return resolve({
                        error: true,
            ...
            try {
                if (!checkObjectIDs(appRoleIDs))
                    return resolve({
                        error: true,
                        message: 'Mã nhóm chức năng không hợp lệ',
                        keyError: 'appRoleIDs_invalid',
                        status: 400,
                    })

                let infoAterRemove = await AUTH__APP_ROLE_COLL.deleteMany({
                    _id: { $in: appRoleIDs },
                })
                if (!infoAterRemove)
                    return resolve({
    ...
    getInfo({ appRoleID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet = await AUTH__APP_ROLE_COLL.findById(
                    appRoleID
                )
                    .select(select)
                    .populate(populates)
                if (!infoDataAfterGet)
                    return resolve({
                        error: true,
                        message: 'Lấy thông tin thấy bại',
    ...
    getList({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet = await AUTH__APP_ROLE_COLL.find(
                    conditionObj
                )
                    .select(select)
                    .limit(+limit + 1)
                ...
                let totalRecord =
                    await AUTH__APP_ROLE_COLL.count(conditionObjOrg)
