...
class Model extends BaseModel {               
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!checkExist) {
                    ...
                    let infoRoleMenuInserted = await this.insertData({ company, app, 
                        role, menu, 
                        read, create, update, 
                        delete: deleteIn,
                        userCreate: userID,
                    ...
                    if (!infoRoleMenuInserted) return resolve({ error: true, message: 'cannot_insert_role_menu' });
                    return resolve({ error: false, data: infoRoleMenuInserted });
                } else {
                    let infoRoleMenuUpdated = await AUTH__APP_ROLE_MENU_COLL.findOneAndUpdate({
                        role, menu
                    }, {
                        read, create, update, delete: deleteIn,
                        userUpdate: userID
                    }, { new: true });
                    if (!infoRoleMenuUpdated) return resolve({ error: true, message: 'cannot_update_role_menu' });
     ...
     * Code  : 
     */
    getList({ company, app, role, limit = 20, lastestID, select, populates = {} }) { 
