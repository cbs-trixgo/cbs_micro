        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await AUTH__APP_ROLE_MENU_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await AUTH__APP_ROLE_MENU_COLL.find(
                    conditionObj
                )
                    .select(select)
                    .limit(+limit + 1)
                ...
                let totalRecord =
                    await AUTH__APP_ROLE_MENU_COLL.count(conditionObjOrg)
