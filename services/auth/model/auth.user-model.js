    ...
    recoverPassword({ email }) {
        return new Promise(async (resolve) => {
            try {
                ...
                const mailContent = templateRecoverPass(
                    checkExist.fullname,
                    codeReset
                )
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (devices.length) {
                    ...
                    if (
                        !newDevices.find(
                            (item) => item.deviceID === itemNewDevice.deviceID
                        )
                    )
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (currentPass && newPass) {
                    let checkCurrentPass = await compare(
                        currentPass,
                        infoUser.password
                    )
                    if (!checkCurrentPass)
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    ...
                    if (!infoUser)
                        return resolve({
                            error: true,
                            message:
                                "Can't get info last user receiveFromFriends",
                            status: 403,
                        })
