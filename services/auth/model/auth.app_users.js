...
class Model extends BaseModel {
  ...
  update({ appID, companyID, memberID, level, userID }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAfterUpdate = await AUTH__APP_USER_COLL.findOneAndUpdate(
  ...
  getList({ appID, companyID, limit = 10, lastestID, select, populates = {} }) {
    return new Promise(async (resolve) => {
      try {
        if (limit > 20) {
          limit = 20
   ...
   * Code  :
   */
  getListAppOfUser({ userID, limit = 10, lastestID, select, populates = {} }) {
    return new Promise(async (resolve) => {
      try {
        let today = new Date()
        let conditionObj = {
