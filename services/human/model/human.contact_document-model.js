...
class Model extends BaseModel {
    ...
    insert({
        ...
        return new Promise(async (resolve) => {
            try {
                let infoContact =
                    await ITEM__CONTACT_COLL.findById(contactID).select(
                        'company'
                    )
                if (!infoContact)
                    return resolve({
                ...
                let dataInsert = {
                    ...
                    note,
                    store,
                    timeMobilize: !isNaN(timeMobilize)
                        ? Number(timeMobilize)
                        : 0,
                ...
                if (
                    educationalBackground2 &&
                    _isValid(educationalBackground2)
                ) {
                ...
                if (
                    infoContact?.name != '' &&
                    infoContact?.name != 'undefined'
                ) {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(infoContact?.name)
                }
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDoc = await HUMAN__CONTACT_DOCUMENT_COLL.findById(
                    documentID
                )
                    .populate({
                        path: 'contact',
                ...
                if (
                    ...
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(infoDoc?.contact?.name)
                }
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (isCertificate == 1) {
                    ...
                    let listContact =
                        await ITEM__CONTACT_COLL.find(conditionContact).select(
                            '_id'
                        )
                    let arrayContactID = listContact.map(
                        (contact) => contact._id
                    )
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await HUMAN__CONTACT_DOCUMENT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
    ...
    exportExcel({ userID, companyID, type, filterParams, ctx }) {
        return new Promise(async (resolve) => {
            try {
                ...
                const listData = await HUMAN__CONTACT_DOCUMENT_COLL.find(
                    conditionObj
                )
                    .select(
