...
class Model extends BaseModel {
    ...
    requestToken({ code_challenge, authCode, state, cacher, ctx }) {
        return new Promise(async resolve => {
            try {
                ...
                if(!verifier) {
                    return resolve({ error: true, message: 'Request is expired' });
                }
                ...
                if(code_challenge !== codeChallenge) {
                    return resolve({ error: true, message: 'Code challenge is not match!'});
                }

                const response = await httpRequest.post(`${ZALO_OA_OAUTH_URL}/v4/oa/access_token`, {
                ...
                if(response.status !== 200 || response.statusText !== 'OK') {
                    return resolve({ error: true, message: "Can't request to zalo" });
                }
                ...
                await cacher.set(`ZALO:oa:refresh_token`, refresh_token, 24 * 60 * 60 * 60);

                await ctx.call(`${CF_DOMAIN_SERVICES.REMINDER}.${CF_ACTIONS_REMINDER.ADD_JOB_REFRESH_TOKEN_ZALO_OA}`);
    ...
    renewAccessToken({ cacher, isCache = false }) {
        return new Promise(async resolve => {
            try {
                ...
                if(!refreshToken) {
                    return resolve({ error: true, message: 'Refresh token is required' });
                }
    ...
    sendMessage({ text, userId, cacher }) {
        return new Promise(async resolve => {
            try {
                ...
                if(!token) {
                    return resolve({ error: true, message: 'Token is required' });
                }

                const response = await httpRequest.post(`${ZALO_OPEN_API_URL}/v2.0/oa/message`, {
                    recipient: {
						user_id: userId,
					},
					message: {
        ...
        return new Promise(async resolve => {
            try {
                ...
                if(!phone || !templateData) {
                    return resolve({ error: true, message: "Phone and template data is required" });
                }
                 ...
                 */
                const response = await httpRequest.post(`${ZALO_ZNS_OPEN_API_URL}/message/template`, {
                    phone,
                    tracking_id: uuidv4(),
                ...
                if(response.status !== 200 || response.statusText !== 'OK') {
                    return resolve({ error: true, message: "Can't request to zalo zns" });
                }
    ...
    handleWebhookFollower({ oaId, source, followerId, eventName, timestamp, cacher }) {
        return new Promise(async resolve => {
            try {
                let gender = '', name = '';
                const status = eventName === 'follow' ? 1 : 2;

                const follower = await ZALOOA_FOLLOWER_COLL.findOne({ followerId, oaId }).lean();
                const profile = await this.getProfileFollower({ userId: followerId, cacher });

                if(!profile.error) {
                    const { display_name, user_gender } = profile.data;

                    name = display_name;
                    gender = user_gender === 1 ? 'Nam' : (user_gender === 2 ? 'Nữ' : 'Không xác định');
                }
                ...
                if(follower) {
                    await ZALOOA_FOLLOWER_COLL.findOneAndUpdate({
                        ...
                        status, eventName, source, timestamp, name, gender
                    });
                } else{
