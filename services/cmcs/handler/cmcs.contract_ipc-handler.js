...
module.exports = {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler =
                    await CMCS__CONTRACT_IPC_MODEL.insert({
                        type,
                        projectID,
                        contractID,
                        name,
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                ...
                const { _id: userID } = ctx.meta.infoUser
                let resultAfterCallHandler =
                    await CMCS__CONTRACT_IPC_MODEL.update({
                        contractIpcID,
                        plan,
                        type,
                        name,
    ...
    remove: {
        ...
        async handler(ctx) {
            try {
                ...
                const { _id: userID } = ctx.meta.infoUser
                let resultAfterCallHandler =
                    await CMCS__CONTRACT_IPC_MODEL.remove({
                        contractIpcID,
                    })
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                ...
                if (contractIpcID) {
                    resultAfterCallHandler =
                        await CMCS__CONTRACT_IPC_MODEL.getInfo({
                            contractIpcID,
                            userID,
                            select,
                            populates,
                    ...
                    resultAfterCallHandler =
                        await CMCS__CONTRACT_IPC_MODEL.getList({
                            isMember,
                            option,
                            companyID,
                            projectID,
    ...
    downloadExcelIpc: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params)
                const { option, ipcID, contractID, conditionDownload } =
                    ctx.params
                const { _id: userID } = ctx.meta.infoUser
