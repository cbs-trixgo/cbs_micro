 ...
 * MODELS
 */
const CMCS__CONTRACT_IPC_MODEL                 = require('../model/cmcs.contract_ipc-model').MODEL;
...
module.exports = {
    ...
    insert: {
        ...
        async handler(ctx) {
            try {
                ...
                let { type, projectID, contractID, name, sign, description, date, note, acceptance, vatAcceptance, plusAcceptance, vatPlusAcceptance, retainedValue, advancePaymentDeduction, otherDeduction, recommendedPayment, timeForPayment, amountPaid, remainingPayment } = ctx.params;
                const { _id: userID } = ctx.meta.infoUser;
                let resultAfterCallHandler = await CMCS__CONTRACT_IPC_MODEL.insert({
                    type, projectID, contractID, name, sign, description, date, note, acceptance, vatAcceptance, plusAcceptance, vatPlusAcceptance, retainedValue, advancePaymentDeduction, otherDeduction, recommendedPayment, timeForPayment, amountPaid, remainingPayment, userID, ctx
                });
    ...
    update: {
        ...
        async handler(ctx) {  
            try {
                await this.validateEntity(ctx.params);
                let { contractIpcID, plan, type, name, sign, description, date, note, acceptance, vatAcceptance, plusAcceptance, vatPlusAcceptance, retainedValue, advancePaymentDeduction, otherDeduction, recommendedPayment, timeForPayment, amountPaid, remainingPayment } = ctx.params;
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { contractIpcID, isMember, option, companyID, projectID, contractID, type, outin, real, field, dependentUnit, personInCharge, chair, buyerInfo, sellerInfo, keyword, limit, lastestID, populates, select } = ctx.params;
                ...
                if(contractIpcID){
                    resultAfterCallHandler = await CMCS__CONTRACT_IPC_MODEL.getInfo({
                        contractIpcID, userID, select, populates
                    });
                }else{
                    ...
                    resultAfterCallHandler = await CMCS__CONTRACT_IPC_MODEL.getList({
                        isMember, option, companyID, projectID, contractID, type: Number(type), outin: Number(outin), real: Number(real), field, dependentUnit, personInCharge, chair, buyerInfo, sellerInfo, userID, keyword, limit, lastestID, select, populates
                    });
                }
    ...
    getAmountByMonth: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { companyID, projectID, contractID, year, outin, option, plan } = ctx.params;
                const { _id: userID, company } = ctx.meta.infoUser;
                ...
                let resultAfterCallHandler = await CMCS__CONTRACT_IPC_MODEL.getAmountByMonth({
                    userID, companyID, projectID, contractID, year, outin, option, plan
                });
    ...
    getListByMonth: {
        ...
        async handler(ctx) {
            try {
                ...
                let { companyID, projectID, contractID, month, year, type, outin, real, field, dependentUnit, personInCharge, chair, buyerInfo, sellerInfo, plan, keyword, limit, lastestID, populates, select } = ctx.params;
				const { _id: userID, company } = ctx.meta.infoUser;
                ...
                let resultAfterCallHandler=await CMCS__CONTRACT_IPC_MODEL.getListByMonth({
                    userID, companyID, projectID, contractID, month, year, type, outin, real, field, dependentUnit, personInCharge, chair, buyerInfo, sellerInfo, plan, keyword, limit, lastestID, select, populates
                });
     ...
     getAmountByObject: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler=await CMCS__CONTRACT_IPC_MODEL.getAmountByObject({
                    userID, outin: Number(outin), companyID, projectID, contractID, ctx
                });
     ...
     getAmountByProperty: {
        ...
        async handler(ctx) {
            try {
                ...
                let { companyID, projectID, contractID, outin, optionTime, option, year, debt } = ctx.params;
				const { _id: userID, company } = ctx.meta.infoUser;
                ...
                let resultAfterCallHandler=await CMCS__CONTRACT_IPC_MODEL.getAmountByProperty({
                    userID, outin: Number(outin), year: Number(year), debt: Number(debt), companyID, projectID, contractID, optionTime: Number(optionTime), option: Number(option), ctx
                });
    ...
    downloadExcelIpc: {
        ...
        async handler(ctx) {
            try {
				...
				const { _id: userID } = ctx.meta.infoUser;
                const resultAfterCallHandler = await CMCS__CONTRACT_IPC_MODEL.downloadExcelIpcDetail({
                    option, ipcID, contractID, conditionDownload, userID 
                })
