 ...
 * MODELS
 */
const CMCS__CONTRACT_EXPENSE_MODEL              = require('../model/cmcs.contract_expense-model').MODEL;
...
module.exports = {
    ...
    insert: {
        ...
        async handler(ctx) {
            try {
                ...
                const { contractID, subtype, date, name, note, value } = ctx.params;

                const { _id: userID } = ctx.meta.infoUser;

                const resultAfterCallHandler = await CMCS__CONTRACT_EXPENSE_MODEL.insert({
                    contractID, subtype: Number(subtype), date, name, note, value: Number(value), userID, ctx
                });
    ...
    update: {
        ...
        async handler(ctx) {  
            try {
                await this.validateEntity(ctx.params);
                const { expenseID, subtype, date, name, note, value } = ctx.params;
                const { _id: userID } = ctx.meta.infoUser;

                const resultAfterCallHandler = await CMCS__CONTRACT_EXPENSE_MODEL.update({
                    expenseID, subtype: Number(subtype), date, name, note, value: Number(value), userID, ctx
                });
                return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx });
            } catch (error) {
                return { error:  true, message: error.message };
    ...
    remove: {
        ...
        async handler(ctx) {
            try {
				...
				const { _id: userID } = ctx.meta.infoUser;
                let resultAfterCallHandler = await CMCS__CONTRACT_EXPENSE_MODEL.remove({
                    expenseID, userID
                });
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { expenseID, contractID, keyword, limit, lastestID, populates, select } = ctx.params;
				const { _id: userID, company } = ctx.meta.infoUser;

                let resultAfterCallHandler;
                if(expenseID){
                    resultAfterCallHandler = await CMCS__CONTRACT_EXPENSE_MODEL.getInfo({
                        expenseID, userID, select, populates
                    });
                }else{
                    resultAfterCallHandler = await CMCS__CONTRACT_EXPENSE_MODEL.getList({
    ...
    getListByMonth: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler=await CMCS__CONTRACT_EXPENSE_MODEL.getListByMonth({
    ...
    getAmountByMonth: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler = await CMCS__CONTRACT_EXPENSE_MODEL.getAmountByMonth({
    ...
    getAmountByObject: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler=await CMCS__CONTRACT_EXPENSE_MODEL.getAmountByObject({
     ...
     getAmountByProperty: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler=await CMCS__CONTRACT_EXPENSE_MODEL.getAmountByProperty({
