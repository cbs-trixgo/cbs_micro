...
const CMCS__CONTRACT_BILL_JOB_MODEL =
    require('../model/cmcs.contract_bill_job-model').MODEL
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler =
                    await CMCS__CONTRACT_BILL_JOB_MODEL.insert({
                        groupID,
                        plus,
                        name,
                        sign,
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                ...
                const { _id: userID } = ctx.meta.infoUser
                let resultAfterCallHandler =
                    await CMCS__CONTRACT_BILL_JOB_MODEL.update({
                        jobID,
                        plus,
                        name,
                        sign,
    ...
    remove: {
        ...
        async handler(ctx) {
            try {
                ...
                const { _id: userID } = ctx.meta.infoUser
                let resultAfterCallHandler =
                    await CMCS__CONTRACT_BILL_JOB_MODEL.remove({
                        jobID,
                    })
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                ...
                if (jobID) {
                    resultAfterCallHandler =
                        await CMCS__CONTRACT_BILL_JOB_MODEL.getInfo({
                            jobID,
                            userID,
                            select,
                            populates,
                    ...
                    resultAfterCallHandler =
                        await CMCS__CONTRACT_BILL_JOB_MODEL.getList({
                            jobID,
                            contractID,
                            itemID,
                            groupID,
