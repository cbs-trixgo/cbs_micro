 ...
 * MODELS
 */
const CMCS__CONTRACT_BILL_ITEM_MODEL                 = require('../model/cmcs.contract_bill_item-model').MODEL;
...
module.exports = {
    ...
    insert: {
        ...
        async handler(ctx) {
            try {
                ...
                let { contractID, name, sign, description, unit, quantity, note } = ctx.params;
                const { _id: userID } = ctx.meta.infoUser;

                let resultAfterCallHandler = await CMCS__CONTRACT_BILL_ITEM_MODEL.insert({
                    contractID, name, sign, description, unit, quantity, note, userID, ctx
                });
    ...
    update: {
        ...
        async handler(ctx) {  
            try {
                ...
                let resultAfterCallHandler = await CMCS__CONTRACT_BILL_ITEM_MODEL.update({
    ...
    updateValue: {
        ...
        async handler(ctx) {  
            try {
                ...
                let resultAfterCallHandler = await CMCS__CONTRACT_BILL_ITEM_MODEL.updateValue({
    ...
    remove: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler = await CMCS__CONTRACT_BILL_ITEM_MODEL.remove({
                    itemID
                });
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { itemID, contractID, keyword, limit, lastestID, populates, select } = ctx.params;
