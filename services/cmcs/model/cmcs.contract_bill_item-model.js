...
class Model extends BaseModel {               
     ...
     * Code: 12/9/2022
     */
    insert({ contractID, name, sign, description, unit, quantity, note, userID, ctx  }) { 
        const that = this
        return new Promise(async (resolve) => {
            try {
                ...
                let dataInsert = { userCreate: userID, company, project, contract: contractID, name };
    ...
    update({ 
        ...
        console.log({itemID, name, sign, description, unit, quantity, note, userID})
        const that = this
        return new Promise(async (resolve) => {
            try {
                /**
                ...
                let infoAfterUpdate = await CMCS__CONTRACT_BILL_ITEM_COLL.findByIdAndUpdate(itemID, dataUpdate, { new: true });
                if(!infoAfterUpdate)
    ...
    getInfo({ itemID, 
        ...
        return new Promise(async resolve => {
            try {
                ...
                let infoContractIPC = await CMCS__CONTRACT_BILL_ITEM_COLL.findById(itemID)
                                .select(select)
                                .populate(populates)
    ...
    getList({ contractID, userID,
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet = await CMCS__CONTRACT_BILL_ITEM_COLL.find(conditionObj)
                    .limit(limit+1)
                    .sort(sortBy)
                    .select(select)
                    .populate(populates)
                ...
                let totalRecord = await CMCS__CONTRACT_BILL_ITEM_COLL.count(conditionObjOrg);
