        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!checkObjectIDs(expenseID))
                    return resolve({
                        error: true,
                        message: 'expenseID không hợp lệ',
                        keyError: KEY_ERROR.PARAMS_INVALID,
                    })

                let infoExpense =
                    await CMCS__CONTRACT_EXPENSE_COLL.findById(expenseID)
                if (!infoExpense)
                    return resolve({
                        error: true,
                ...
                let infoAfterUpdate =
                    await CMCS__CONTRACT_EXPENSE_COLL.findByIdAndUpdate(
                        expenseID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    getInfo({ expenseID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoExpense = await CMCS__CONTRACT_EXPENSE_COLL.findById(
                    expenseID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await CMCS__CONTRACT_EXPENSE_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await CMCS__CONTRACT_EXPENSE_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await CMCS__CONTRACT_EXPENSE_COLL.count(conditionObjOrg)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!isNaN(optionProperty) && Number(optionProperty) > 0) {
                    await CMCS__CONTRACT_EXPENSE_COLL.populate(
                        listData,
                        conditionPopulate
                    )
                }
