...
const CMCS__CONTRACT_BILL_JOB_MODEL             = require('./cmcs.contract_bill_job-model').MODEL;
...
class Model extends BaseModel {               
     ...
     * Code: 
     */
    insert({ ipcID, jobID, plus, quantity, unitPrice, amount, note, userID, ctx  }) { 
        const that = this
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                let info = await CMCS__CONTRACT_IPC_DETAIL_COLL.findOne({ ipc: ipcID, job: jobID })
                // console.log(info)
                if(!info){
                    ...
                    const { company, project, contract, item, group, plus } = infoJob;    

                    let dataInsert = { userCreate: userID, company, project, contract, item, group, job: jobID, ipc: ipcID, plus };
                     ...
                     */    
                    let infoItemNew = await CMCS__CONTRACT_BILL_ITEM_MODEL.updateValue({option: 2, itemID: item, userID})
                    // console.log(infoItemNew)
                    let infoGroupNew = await CMCS__CONTRACT_BILL_GROUP_MODEL.updateValue({option: 2, groupID: group, userID})
                    // console.log(infoGroupNew)
                    let infoJobNew = await CMCS__CONTRACT_BILL_JOB_MODEL.updateValue({option: 2, jobID, userID})
                    // console.log(infoJobNew)
                    
                    return resolve({ error: false, data: infoAfterInsert, status: 200 });
                }else{
                    return resolve({ error: true, message: "Data exists" ,keyError: "can't_insert" });
                }
            } catch (error) {
     ...
     */
    update({ 
        detailID, quantity, unitPrice, amount, note, userID, ctx
    }){
        ...
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                // Thông tin công việc của hợp đồng
                const infoDetail = await CMCS__CONTRACT_IPC_DETAIL_COLL.findById(detailID)
                if(!infoDetail)
                    return resolve({ error: true, message: "Mã không hợp lệ", keyError: KEY_ERROR.PARAMS_INVALID });
                ...
                let infoAfterUpdate = await CMCS__CONTRACT_IPC_DETAIL_COLL.findByIdAndUpdate(detailID, dataUpdate, { new: true });
                if(!infoAfterUpdate)
                ...
                let infoGroupNew = await CMCS__CONTRACT_BILL_GROUP_MODEL.updateValue({option: 2, groupID: group, userID})
                // console.log(infoGroupNew)
                let infoJobNew = await CMCS__CONTRACT_BILL_JOB_MODEL.updateValue({option: 2, jobID: job, userID})
                // console.log(infoJobNew)
