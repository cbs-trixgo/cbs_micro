    ...
    update({ 
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let inforSubmit = await CMCS__CONTRACT_SUBMITTAL_COLL.findById(submitID)
                ...
                let infoAfterUpdate = await CMCS__CONTRACT_SUBMITTAL_COLL.findByIdAndUpdate(submitID, dataUpdate, { new: true })
                if(!infoAfterUpdate)
    ...
    getInfo({ submitID, 
        ...
        return new Promise(async resolve => {
            try {
                ...
                let infoContractIPC = await CMCS__CONTRACT_SUBMITTAL_COLL.findById(submitID)
                                .select(select)
                                .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(lastestID && checkObjectIDs(lastestID)){
					let infoData = await CMCS__CONTRACT_SUBMITTAL_COLL.findById(lastestID);
					if(!infoData)
                ...
                let infoDataAfterGet = await CMCS__CONTRACT_SUBMITTAL_COLL.find(conditionObj)
                    .limit(limit+1)
                    .sort(sortBy)
                    .select(select)
                    .populate(populates)
                ...
                let totalRecord = await CMCS__CONTRACT_SUBMITTAL_COLL.count(conditionObjOrg);
