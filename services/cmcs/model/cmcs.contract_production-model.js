...
class Model extends BaseModel {
      ...
      try {
        ...
        if (!infoAfterInsert)
          return resolve({
            ...
            keyError: 'can_not_insert_contract_production',
            status: 403,
          })
    ...
    return new Promise(async (resolve) => {
      try {
        if (!checkObjectIDs(contractProductionID))
          return resolve({
            error: true,
            message: 'Mã sản lượng hợp đồng không đúng',
            keyError: 'contractProductionID_invalid',
          })
        ...
        let infoContractProduction =
          await CMCS__CONTRACT_PRODUCTION_COLL.findById(contractProductionID)
        ...
        let infoAfterUpdate =
          await CMCS__CONTRACT_PRODUCTION_COLL.findByIdAndUpdate(
            contractProductionID,
            dataUpdate,
            { new: true }
          )
   ...
   * Code:
   */
  getInfo({ contractProductionID, userID, select, populates }) {
    return new Promise(async (resolve) => {
      try {
        if (!checkObjectIDs(contractProductionID))
          return resolve({
            error: true,
            message: 'contractProductionID không hợp lệ',
            keyError: 'contractProductionID_invalid',
          })
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (lastestID && checkObjectIDs(lastestID)) {
          let infoData =
            await CMCS__CONTRACT_PRODUCTION_COLL.findById(lastestID)
          if (!infoData)
            return resolve({
              error: true,
        ...
        let infoDataAfterGet = await CMCS__CONTRACT_PRODUCTION_COLL.find(
          conditionObj
        )
          .limit(limit + 1)
          .sort(sortBy)
        ...
        let totalRecord =
          await CMCS__CONTRACT_PRODUCTION_COLL.count(conditionObjOrg)
  ...
  getAmountByObject({ userID, outin, companyID, projectID, contractID }) {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        return resolve({
          error: false,
          data: { produce, vatProduce, plusProduce, vatPlusProduce },
        })
      } catch (error) {
        return resolve({ error: true, message: error.message })
      }
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (!isNaN(optionProperty) && Number(optionProperty) > 0) {
          await CMCS__CONTRACT_PRODUCTION_COLL.populate(
            listData,
            conditionPopulate
          )
        }
