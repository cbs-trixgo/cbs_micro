    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (!isNaN(orgQuantity) && !isNaN(orgUnitPrice)) {
          ...
          dataInsert.currentAmount = Number(orgQuantity * orgUnitPrice).toFixed(
            0
          )
         ...
         */
        let infoItemNew = await CMCS__CONTRACT_BILL_ITEM_MODEL.updateValue({
          option: 1,
          itemID: item,
          userID,
        })
        // console.log(infoItemNew)
        let infoGroupNew = await CMCS__CONTRACT_BILL_GROUP_MODEL.updateValue({
          option: 1,
          groupID,
          userID,
        })
