...
class Model extends BaseModel {
    ...
    insert({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (
                    !isNaN(vatPlusAcceptance) &&
                    Number(vatPlusAcceptance) >= 0
                ) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!checkObjectIDs(contractIpcID))
                    return resolve({
                        error: true,
                        message: 'Mã IPC không hợp kệ',
                        keyError: 'contract_ipc_invalid',
                    })

                let infoContractIPC =
                    await CMCS__CONTRACT_IPC_COLL.findById(contractIpcID)
                ...
                let infoAfterUpdate =
                    await CMCS__CONTRACT_IPC_COLL.findByIdAndUpdate(
                        contractIpcID,
                        dataUpdate,
                        { new: true }
                    )
     ...
     * Date  : 12/4/2022
     */
    updateValue({ option, ipcID, userID }) {
        return new Promise(async (resolve) => {
            try {
                /**
                 * BA
                ...
                if (option && Number(option) === 3) {
                    ...
                    if (listData && listData.length) {
                        ...
                        dataUpdate.remainingPayment =
                            Number(recommendedPayment) -
                            Number(listData[0].amount)
                    } else {
     ...
     * Code:
     */
    getInfo({ contractIpcID, select, populates }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoContractIPC = await CMCS__CONTRACT_IPC_COLL.findById(
                    contractIpcID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await CMCS__CONTRACT_IPC_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await CMCS__CONTRACT_IPC_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await CMCS__CONTRACT_IPC_COLL.count(conditionObjOrg)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (listData1 && listData1.length) {
                    acceptance =
                        Number(acceptance) + Number(listData1[0].acceptance)
                    vatAcceptance =
                        Number(vatAcceptance) +
                        Number(listData1[0].vatAcceptance)
                    plusAcceptance =
                        Number(plusAcceptance) +
                        Number(listData1[0].plusAcceptance)
                    vatPlusAcceptance =
                        Number(vatPlusAcceptance) +
                        Number(listData1[0].vatPlusAcceptance)
                    retainedValue =
                        Number(retainedValue) +
                        Number(listData1[0].retainedValue)
                    advancePaymentDeduction =
                        Number(advancePaymentDeduction) +
                        Number(listData1[0].advancePaymentDeduction)
                    otherDeduction =
                        Number(otherDeduction) +
                        Number(listData1[0].otherDeduction)
                    recommendedPayment =
                        Number(recommendedPayment) +
                        Number(listData1[0].recommendedPayment)
                    advancePaymentPaid =
                        Number(advancePaymentPaid) +
                        Number(listData1[0].amountPaid) // Tạm ứng
                    remainingPayment =
                        Number(remainingPayment) +
                        Number(listData1[0].remainingPayment)
                }

                if (listData2 && listData2.length) {
                    acceptance =
                        Number(acceptance) + Number(listData2[0].acceptance)
                    vatAcceptance =
                        Number(vatAcceptance) +
                        Number(listData2[0].vatAcceptance)
                    plusAcceptance =
                        Number(plusAcceptance) +
                        Number(listData2[0].plusAcceptance)
                    vatPlusAcceptance =
                        Number(vatPlusAcceptance) +
                        Number(listData2[0].vatPlusAcceptance)
                    retainedValue =
                        Number(retainedValue) +
                        Number(listData2[0].retainedValue)
                    advancePaymentDeduction =
                        Number(advancePaymentDeduction) +
                        Number(listData2[0].advancePaymentDeduction)
                    otherDeduction =
                        Number(otherDeduction) +
                        Number(listData2[0].otherDeduction)
                    recommendedPayment =
                        Number(recommendedPayment) +
                        Number(listData2[0].recommendedPayment)
                    amountPaid =
                        Number(amountPaid) + Number(listData2[0].amountPaid) // Thanh toán
                    remainingPayment =
                        Number(remainingPayment) +
                        Number(listData2[0].remainingPayment)
                }
    ...
    getAmountByProperty({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!isNaN(option) && Number(option) > 0) {
                    await CMCS__CONTRACT_IPC_COLL.populate(
                        listData,
                        conditionPopulate
                    )
                }
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (ipcID) {
                    ...
                    infoContract = await ITEM__CONTRACT_COLL.findById(
                        infoIpc.contract
                    )
                ...
                if (option && option == 1) {
                    XlsxPopulate.fromFileAsync(
                        path.resolve(
                            __dirname,
                            '../../../files/templates/excels/cmcs_ipc_report.xlsx'
                        )
                    ).then(async (workbook) => {
                        ...
                        workbook
                            ...
                            .row(5)
                            .cell(3)
                            .value(infoContract.name)
                        workbook
                            .sheet('Report1')
                            .row(6)
                            .cell(3)
                            .value(infoContract.sign)
                        workbook
                            .sheet('Report1')
                            .row(7)
                            .cell(3)
                            .value(infoContract.date)
                        workbook
                            .sheet('Report1')
                            .row(8)
                            .cell(3)
                            .value(
                                Number(
                                    infoContract.value + infoContract.vatValue
                                )
                            )
                        workbook
                            .sheet('Report1')
                            ...
                            .cell(3)
                            .value(
                                Number(infoContract.plus + infoContract.vatPlus)
                            )
                        workbook
                            .sheet('Report1')
                            .row(10)
                            .cell(3)
                            .value(Number(infoContract.advancePaymentPaid))
                        ...
                        if (listData.length) {
                            ...
                            workbook
                                ...
                                .row(13)
                                .cell(3)
                                .value(Number(listData[0].retainedValue))
                            workbook
                                .sheet('Report1')
                                .row(14)
                                .cell(3)
                                .value(
                                    Number(listData[0].advancePaymentDeduction)
                                )
                            workbook
                                .sheet('Report1')
                                .row(15)
                                .cell(3)
                                .value(
                                    Number(infoContract.advancePaymentOverage)
                                ) //** */
                            workbook
                                .sheet('Report1')
                                .row(16)
                                .cell(3)
                                .value(Number(listData[0].otherDeduction))
                            workbook
                                .sheet('Report1')
                                .row(17)
                                .cell(3)
                                .value(Number(listData[0].recommendedPayment))
                            workbook
                                .sheet('Report1')
                                .row(18)
                                .cell(3)
                                .value(Number(listData[0].amountPaid))
                            workbook
                                .sheet('Report1')
                                .row(19)
                                .cell(3)
                                .value(Number(listData[0].remainingPayment))
                            ...
                            if (ipcID) {
                                workbook
                                    ...
                                    .value(
                                        Number(
                                            infoIpc.acceptance +
                                                infoIpc.vatAcceptance +
                                                infoIpc.plusAcceptance +
                                                infoIpc.vatPlusAcceptance
                                        )
                                    )
                                workbook
                                    .sheet('Report1')
                                    .row(22)
                                    .cell(3)
                                    .value(Number(infoIpc.retainedValue))
                                workbook
                                    .sheet('Report1')
                                    .row(23)
                                    .cell(3)
                                    .value(
                                        Number(infoIpc.advancePaymentDeduction)
                                    )
                                workbook
                                    .sheet('Report1')
                                    .row(25)
                                    .cell(3)
                                    .value(Number(infoIpc.otherDeduction))
                                workbook
                                    .sheet('Report1')
                                    .row(26)
                                    .cell(3)
                                    .value(Number(infoIpc.recommendedPayment))
                            }
                        }
