  ...
  update({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAfterUpdate = await CMCS__CONTRACT_IPC_COLL.findByIdAndUpdate(
          contractIpcID,
          dataUpdate,
          { new: true }
        )
        ...
        if (option && Number(option) === 3) {
          ...
          if (listData && listData.length) {
            ...
            dataUpdate.remainingPayment =
              Number(recommendedPayment) - Number(listData[0].amount)
          } else {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (listData1 && listData1.length) {
          ...
          vatAcceptance =
            Number(vatAcceptance) + Number(listData1[0].vatAcceptance)
          plusAcceptance =
            Number(plusAcceptance) + Number(listData1[0].plusAcceptance)
          vatPlusAcceptance =
            Number(vatPlusAcceptance) + Number(listData1[0].vatPlusAcceptance)
          retainedValue =
            Number(retainedValue) + Number(listData1[0].retainedValue)
          advancePaymentDeduction =
            Number(advancePaymentDeduction) +
            Number(listData1[0].advancePaymentDeduction)
          otherDeduction =
            Number(otherDeduction) + Number(listData1[0].otherDeduction)
          recommendedPayment =
            Number(recommendedPayment) + Number(listData1[0].recommendedPayment)
          advancePaymentPaid =
            Number(advancePaymentPaid) + Number(listData1[0].amountPaid) // Tạm ứng
          remainingPayment =
            Number(remainingPayment) + Number(listData1[0].remainingPayment)
        }

        if (listData2 && listData2.length) {
          acceptance = Number(acceptance) + Number(listData2[0].acceptance)
          vatAcceptance =
            Number(vatAcceptance) + Number(listData2[0].vatAcceptance)
          plusAcceptance =
            Number(plusAcceptance) + Number(listData2[0].plusAcceptance)
          vatPlusAcceptance =
            Number(vatPlusAcceptance) + Number(listData2[0].vatPlusAcceptance)
          retainedValue =
            Number(retainedValue) + Number(listData2[0].retainedValue)
          advancePaymentDeduction =
            Number(advancePaymentDeduction) +
            Number(listData2[0].advancePaymentDeduction)
          otherDeduction =
            Number(otherDeduction) + Number(listData2[0].otherDeduction)
          recommendedPayment =
            Number(recommendedPayment) + Number(listData2[0].recommendedPayment)
          amountPaid = Number(amountPaid) + Number(listData2[0].amountPaid) // Thanh toán
          remainingPayment =
            Number(remainingPayment) + Number(listData2[0].remainingPayment)
        }
        ...
        if (option && option == 1) {
          XlsxPopulate.fromFileAsync(
            ...
            workbook
              ...
              .row(8)
              .cell(3)
              .value(Number(infoContract.value + infoContract.vatValue))
            workbook
              .sheet('Report1')
              .row(9)
              .cell(3)
              .value(Number(infoContract.plus + infoContract.vatPlus))
            workbook
              .sheet('Report1')
              .row(10)
              .cell(3)
            ...
            if (listData.length) {
              ...
              workbook
                ...
                .row(14)
                .cell(3)
                .value(Number(listData[0].advancePaymentDeduction))
              workbook
                .sheet('Report1')
                .row(15)
                .cell(3)
                .value(Number(infoContract.advancePaymentOverage)) //** */
              workbook
                .sheet('Report1')
                .row(16)
                .cell(3)
              ...
              if (ipcID) {
                ...
                workbook
                  ...
                  .row(23)
                  .cell(3)
                  .value(Number(infoIpc.advancePaymentDeduction))
                workbook
                  .sheet('Report1')
                  .row(25)
                  .cell(3)
