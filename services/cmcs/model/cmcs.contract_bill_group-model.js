...
class Model extends BaseModel {
    ...
    insert({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                const infoItem =
                    await CMCS__CONTRACT_BILL_ITEM_COLL.findById(itemID)
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await CMCS__CONTRACT_BILL_GROUP_COLL.findByIdAndUpdate(
                        groupID,
                        dataUpdate,
                        { new: true }
                    )
     ...
     * Code: 12/9/2022
     */
    updateValue({ option, groupID, userID }) {
        return new Promise(async (resolve) => {
            try {
                /**
                 * BA
                ...
                if (option && Number(option) === 1) {
                    ...
                    if (listData && listData.length) {
                        dataUpdate.currentAmount = Number(
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await CMCS__CONTRACT_BILL_GROUP_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet =
                    await CMCS__CONTRACT_BILL_GROUP_COLL.find(conditionObj)
                        .limit(limit + 1)
                        .sort(sortBy)
                        .select(select)
                        .populate(populates)
                ...
                let totalRecord =
                    await CMCS__CONTRACT_BILL_GROUP_COLL.count(conditionObjOrg)
