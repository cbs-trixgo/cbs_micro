 ...
 */
const FINANCIAL_GENERAL_JOURNAL_MODEL =
    require('../.../../../accounting/model/accounting.financial_general_journal-model').MODEL
const ACCOUNT_MODEL =
...
const TIMESHEET__EXPERT_SALARY_MODEL =
    require('../.../../../timesheet/model/timesheet.expert_salary-model').MODEL
    ...
    insert({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (checkObjectIDs(parentID)) {
                    const infoParentCashbook =
                        await FIN__CASH_BOOK_COLL.findById(parentID)
                            .select('company')
                            .lean()
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                const infoCashBook = await FIN__CASH_BOOK_COLL.findById(
                    cashBookID
                )
                    .populate('parent')
                    .lean()
                ...
                const adminsID = [
                    ...infoCashBook.admins?.map((item) => item.toString()),
                    infoCashBook.userCreate?.toString(),
                ]
                ...
                let infoAfterUpdate =
                    await FIN__CASH_BOOK_COLL.findByIdAndUpdate(
                        cashBookID,
                        dataUpdate,
                        { new: true }
                    )
                ...
                if (checkObjectIDs(infoAfterUpdate.parent)) {
                    await that.updateValue({
                        cashBookID: infoAfterUpdate.parent,
                        userID,
                    })
                }
            ...
            try {
                if (!checkObjectIDs(cashBookID))
                    return resolve({
                        ...
                        keyError: 'cashBookID_invalid',
                    })
                let infoCashBook =
                    await FIN__CASH_BOOK_COLL.findById(cashBookID)
                ...
                if (listData && listData.length) {
                    let arising =
                        Number(listData[0].income) - Number(listData[0].expense)
                    let closingBalance =
                        Number(infoCashBook.openingBalance) + Number(arising)
    ...
    getInfo({ cashBookID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoContractIPC = await FIN__CASH_BOOK_COLL.findById(
                    cashBookID
                )
                    .select(select)
                    .populate(populates)
    ...
    getList({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet = await FIN__CASH_BOOK_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await FIN__CASH_BOOK_COLL.count(conditionObjOrg)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!isNaN(option) && Number(option) > 0) {
                    await FIN__CASH_BOOK_COLL.populate(
                        listData,
                        conditionPopulate
                    )
                }
    ...
    downloadTemplateExcel({ option, companyID, parentID, year, userID }) {
        ...
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                if (!option) {
                    let listItems1 = await ACCOUNT_COLL.find({
                        company: companyID,
                    })
                        .limit(200)
                        .select('name')
                    ...
                    let listItems2 = await CONTACT_COLL.find({
                        company: companyID,
                    })
                        .limit(200)
                        .select('name phone taxid')

                    let listItems3 = await CONTRACT_COLL.find({
                        company: companyID,
                    })
                        .limit(200)
                        .select('name sign')
                     ...
                     * DỮ LIỆU SỔ QUỸ
                     */
                    let listData = await FIN__CASH_BOOK_COLL.find({
                        parent: parentID,
                    })
                        .limit(2000)
                        .select(
                            'date name account customer contract income expense note'
                        )
                        .populate({
                    ...
                    XlsxPopulate.fromFileAsync(
                        path.resolve(
                            __dirname,
                            '../../../files/templates/excels/fin_cashbook_import.xlsm'
                        )
                    ).then(async (workbook) => {
                        var i = 3
                        ...
                        listItems2?.forEach((item, index) => {
                            ...
                            workbook
                                ...
                                .cell(4)
                                .value(
                                    `${item.name}_${item?.phone}_${item?.taxid}`
                                )
                            i++
                        })
                        ...
                        listData?.forEach((item, index) => {
                            ...
                            workbook
                                ...
                                .cell(5)
                                .value(
                                    `${item?.customer && item?.customer != null ? item.customer.name : ''}`
                                )
                            workbook
                                .sheet('DataImport')
                                .row(i)
                                .cell(6)
                                .value(
                                    `${item?.contract && item?.contract != null ? item.contract.name : ''}`
                                )
                            workbook
                                .sheet('DataImport')
                                .row(i)
                    ...
                    if (option == 1) {
                        ...
                        let infoParent =
                            await TIMESHEET__EXPERT_TIMESHEET_COLL.findById(
                                ...
                                'name hours unitprice amount status date note'
                            )

                        let listData =
                            await TIMESHEET__EXPERT_TIMESHEET_COLL.find({
                                parent: parentID,
                            })
                                .populate({
                                    path: 'work assignee',
                        ...
                        XlsxPopulate.fromFileAsync(
                            path.resolve(
                                __dirname,
                                '../../../files/templates/excels/timesheet_report.xlsx'
                            )
                        ).then(async (workbook) => {
                            workbook
                                .sheet('Data')
                            ...
                            listData.forEach((item, index) => {
                                ...
                                if (item?.work?.unitPrice) {
                                    amountBudget = Number(
                                        item?.work?.unitPrice * item.hours
                                    )
                                }
                                workbook
                                    .sheet('Data')
                                ...
                                workbook
                                    ...
                                    .row(i)
                                    .cell(10)
                                    .value(Number(item?.work?.unitPrice) | 0)
                                workbook
                                    .sheet('Data')
                                    .row(i)
                                    .cell(11)
                                    .value(Number(amountBudget))
                                i++
                            })
                            ...
                            listData1.forEach((item, index) => {
                                ...
                                workbook
                                    ...
                                    .row(i)
                                    .cell(2)
                                    .value(item._id.work?.name)
                                workbook
                                    .sheet('Report1')
                                    .row(i)
                                    .cell(3)
                                    .value(item._id.work?.unit)
                                workbook
                                    .sheet('Report1')
                                    .row(i)
                                    .cell(4)
                                ...
                                workbook
                                    ...
                                    .row(i)
                                    .cell(5)
                                    .value(Number(item._id.work?.unitPrice))
                                workbook
                                    .sheet('Report1')
                                    .row(i)
                                    .cell(6)
                            ...
                            listData2.forEach((item, index) => {
                                ...
                                workbook
                                    ...
                                    .row(i)
                                    .cell(2)
                                    .value(item._id.assignee?.fullname)
                                workbook
                                    .sheet('Report2')
                                    .row(i)
                                    .cell(3)
                    ...
                    if (option == 2) {
                        ...
                        let infoParent =
                            await TIMESHEET__EXPERT_SALARY_COLL.findById(
                                parentID
                        ...
                        if (
                            ...
                            let listDepartment =
                                await ITEM__DEPARTMENT_COLL.find({
                                    company: companyID,
                                    members: { $in: [userID] },
                                }).select('_id name')
                        ...
                        let listData = await TIMESHEET__EXPERT_SALARY_COLL.find(
                            conditionObj
                        )
                            .populate({
                        ...
                        XlsxPopulate.fromFileAsync(
                            path.resolve(
                                __dirname,
                                '../../../files/templates/excels/salary_report.xlsx'
                            )
                        ).then(async (workbook) => {
                            var i = 3
                            ...
                            listData.forEach((item, index) => {
                                ...
                                workbook
                                    ...
                                    .row(i)
                                    .cell(6)
                                    .value(Number(item.convertFactor))
                                workbook
                                    .sheet('BangLuong')
                                    .row(i)
                                    .cell(7)
                                    .value(Number(item.onLeaveSalary))
                                workbook
                                    .sheet('BangLuong')
                                    .row(i)
                                    .cell(8)
                                    .value(Number(item.revenueBonus))
                                workbook
                                    .sheet('BangLuong')
                                    .row(i)
                                    .cell(9)
                                    .value(Number(item.reward))
                                workbook
                                    .sheet('BangLuong')
                                    .row(i)
                                    .cell(10)
                                    .value(Number(item.mealAllowance))

                                workbook
                                    .sheet('BangLuong')
                                    .row(i)
                                    .cell(12)
                                    .value(Number(item.punishment))
                                workbook
                                    .sheet('BangLuong')
                                    .row(i)
                                    .cell(13)
                                    .value(Number(item.insurance))
                                ...
                                workbook
                                    ...
                                    .row(i)
                                    .cell(16)
                                    .value(Number(item.share))
                                workbook
                                    .sheet('BangLuong')
                                    .row(i)
                                    .cell(17)
                                    .value(Number(item.other))

                                workbook
                                    .sheet('BangLuong')
                                    .row(i)
                                    .cell(20)
                                    .value(Number(item.advance))
                                workbook
                                    .sheet('BangLuong')
                                    .row(i)
                                    .cell(21)
                                ...
                                if (item?.human.contacts.length) {
                                    ...
                                    workbook
                                        ...
                                        .cell(30)
                                        .value(
                                            item?.human.contacts[0].bankAccount
                                        )
                                }
                                ...
                                workbook
                                    ...
                                    .row(i)
                                    .cell(26)
                                    .value(Number(item?.subAllowance1))
                                workbook
                                    .sheet('BangLuong')
                                    .row(i)
                                    .cell(27)
                                    .value(Number(item?.pependent))
                                workbook
                                    .sheet('BangLuong')
                                    .row(i)
                                    .cell(29)
                                    .value(Number(item?.subAllowance2))
                                ...
                                workbook
                                    ...
                                    .row(i)
                                    .cell(13)
                                    .value(Number(item.pitax))
                                workbook
                                    .sheet('BangLuong2')
                                    .row(i)
                                    .cell(14)
                                    .value(Number(item.union))
                                workbook
                                    .sheet('BangLuong2')
                                    .row(i)
                                    .cell(15)
                                ...
                                workbook
                                    ...
                                    .row(i)
                                    .cell(17)
                                    .value(Number(tongThu))
                                workbook
                                    .sheet('BangLuong2')
                                    .row(i)
                                    .cell(18)
                                    .value(Number(tongGiamTru))
                    ...
                    if (option == 3) {
                         ...
                         */
                        let listData =
                            await FINANCIAL_GENERAL_JOURNAL_COLL.find({
                                company: companyID,
                            })
                                // .select('')
                                .populate({
                         ...
                         */
                        let listAccounts = await ACCOUNT_MODEL.getListRecursive(
                            { companyID }
                        )
                        ...
                        let option = 4,
                            ...
                            toDate = '2023-12-31'
                        let listDataCombine =
                            await FINANCIAL_GENERAL_JOURNAL_MODEL.getListByProperty(
                                {
                                    option,
                                    optionTerm,
                                    companyID,
                        ...
                        XlsxPopulate.fromFileAsync(
                            path.resolve(
                                __dirname,
                                '../../../files/templates/excels/fin_financial_report.xlsm'
                            )
                        ).then(async (workbook) => {
                            // NHẬT KÝ CHUNG
                            var i = 4
                                ...
                                arr.forEach((item) => {
                                    ...
                                    if (arrDebitFT[item._id]) {
                                        workbook
                                            ...
                                            .value(
                                                Number(
                                                    arrDebitFT[item._id][0]
                                                        .amount
                                                )
                                            )
                                        debitFT = Number(
                                            arrDebitFT[item._id][0].amount
                                        )
                                    }
                                    ...
                                    workbook
                                        ...
                                        .row(i)
                                        .cell(8)
                                        .value(Number(totalDebitFT))
                                    workbook
                                        .sheet('BangCDTaiKhoan')
                                        .row(i + 2)
                                        .cell(8)
                                        .value(Number(allSubDebitFT))
                        ...
                        if (!infoAppUser.error && infoAppUser.data.level == 0) {
                            for (const data of dataImportJSON) {
                                if (index > 1 && index <= 101) {
                                    // console.log(data)
                                    let infoAfterUpdate =
                                        await TIMESHEET__EXPERT_SALARY_MODEL.update(
                                            {
                                                expertSalaryID:
                                                    data?.__EMPTY_22,
                                                admins: [userID],
                                                ...
                                                subAllowance1: Number(
                                                    data?.__EMPTY_24
                                                ),
                                                pependent: Number(
                                                    data?.__EMPTY_25
                                                ),
                                                subAllowance2: Number(
                                                    data?.__EMPTY_27
                                                ),
                                            }
                                        )
            ...
            try {
                ...
                if (option == 1) {
                    let infoParent =
                        await TIMESHEET__EXPERT_TIMESHEET_COLL.findById(
                            ...
                            'name hours unitprice amount status date note subtype'
                        )
                    ...
                    let listUser = await USER_COLL.find({
                        _id: {
                            $in: listData2.map((item) => item._id.assignee),
                        },
                    })
                ...
                // Tải tổng hợp ngân sách
                    ...
                    XlsxPopulate.fromFileAsync(
                        path.resolve(
                            __dirname,
                            '../../../files/templates/excels/budget_report.xlsx'
                        )
                    ).then(async (workbook) => {
                        workbook
                            .sheet(`Report1`)
                            .row(2)
                            .cell(1)
                            .value(`Báo cáo tổng hợp năm ${yearFilter}`)
                        workbook
                            .sheet(`Report2`)
                            .row(2)
                            .cell(1)
                        ...
                        listData1.forEach((item, index) => {
                            ...
                            workbook
                                ...
                                .cell(2)
                                .value(
                                    `${item._id.work?.sign}-${item._id.work?.name}`
                                )
                            workbook
                                .sheet('Report1')
                                .row(i)
                            ...
                            workbook
                                ...
                                .row(i)
                                .cell(5)
                                .value(Number(item._id.work?.quantity))
                            workbook
                                .sheet('Report1')
                                .row(i)
                                .cell(6)
