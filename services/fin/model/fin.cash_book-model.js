  ...
  insert({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (checkObjectIDs(parentID)) {
          const infoParentCashbook = await FIN__CASH_BOOK_COLL.findById(
            parentID
          )
            .select('company')
            .lean()
  ...
  update({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAfterUpdate = await FIN__CASH_BOOK_COLL.findByIdAndUpdate(
          cashBookID,
          dataUpdate,
          { new: true }
        )
      ...
      try {
        ...
        if (Number(option) === 1) {
          ...
          let listActiveParent = await FIN__CASH_BOOK_COLL.find({
            ...
            parent: { $exists: false },
            company: {
              $in: listCompany.data.map((item) => item._id.company._id),
            },
          })
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (!option) {
          ...
          XlsxPopulate.fromFileAsync(
            ...
            listItems2?.forEach((item, index) => {
              ...
              workbook
                ...
                .row(i)
                .cell(4)
                .value(`${item.name}_${item?.phone}_${item?.taxid}`)
              i++
            })
            ...
            listData?.forEach((item, index) => {
              ...
              workbook
                ...
                .row(i)
                .cell(9)
                .value(`${item?.note && item?.note != '' ? item?.note : ''}`)
              i++
            })
          ...
          if (option == 1) {
            // Danh sách ngân sách
            let infoParent = await TIMESHEET__EXPERT_TIMESHEET_COLL.findById(
              parentID
            ...
            XlsxPopulate.fromFileAsync(
              ...
              listData1.forEach((item, index) => {
                ...
                workbook
                  ...
                  .row(i)
                  .cell(6)
                  .value(Number(item.quantity * item._id.work?.unitPrice))
          ...
          if (option == 2) {
            ...
            let infoParent = await TIMESHEET__EXPERT_SALARY_COLL.findById(
              parentID
              ...
              listData.forEach((item, index) => {
                ...
                if (item?.human.contacts.length) {
                  ...
                  workbook
                    ...
                    .row(i)
                    .cell(30)
                    .value(item?.human.contacts[0].bankAccount)
                }
          ...
          if (option == 3) {
             ...
             * NHẬT KÝ CHUNG
             */
            let listData = await FINANCIAL_GENERAL_JOURNAL_COLL.find({
              company: companyID,
            })
              // .select('')
              .populate({
                ...
                arr.forEach((item) => {
                  ...
                  for (const element of item.nestedChilds) {
                    if (arrDebitFT[`${element}`]) {
                      ...
                      allSubDebitFT =
                        allSubDebitFT + Number(arrDebitFT[element][0].amount)
                    }
                  }
                  ...
                  if (arrDebitO[item._id]) {
                    workbook
                      ...
                      .row(i + 1)
                      .cell(4)
                      .value(Number(arrDebitO[item._id][0].amount))
                  }
                  if (arrCreditO[item._id]) {
                    workbook
                      ...
                      .row(i + 1)
                      .cell(5)
                      .value(Number(arrCreditO[item._id][0].amount))
                  }
                  if (arrDebitOF[item._id]) {
                    workbook
                      ...
                      .row(i + 1)
                      .cell(6)
                      .value(Number(arrDebitOF[item._id][0].amount))
                  }
                  if (arrCreditOF[item._id]) {
                    workbook
                      ...
                      .row(i + 1)
                      .cell(7)
                      .value(Number(arrCreditOF[item._id][0].amount))
                  }
                  ...
                  if (arrDebitFT[item._id]) {
                    workbook
                      ...
                      .row(i + 1)
                      .cell(8)
                      .value(Number(arrDebitFT[item._id][0].amount))
                  ...
                  if (arrCreditFT[item._id]) {
                    workbook
                      ...
                      .row(i + 1)
                      .cell(9)
                      .value(Number(arrCreditFT[item._id][0].amount))
                  }
                  ...
                  if (item.childs.length) {
                    workbook.sheet('BangCDTaiKhoan').row(i).cell(1).style({
                      bold: true,
                      fill: '0000ff',
                    })
                  }
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (option == 1) {
          let infoParent = await TIMESHEET__EXPERT_TIMESHEET_COLL.findById(
            parentID
          ).select('name hours unitprice amount status date note subtype')
            ...
            listData2?.forEach((item, index) => {
              for (var i = 4; i <= listUser.length + 3; i++) {
                ...
                for (var j = 1; j <= 31; j++) {
                  if (
                    item._id.day == j &&
                    userID.toString() === item._id.assignee.toString()
                  ) {
                    workbook
                      .sheet('Data2')
                      .row(i)
          ...
          XlsxPopulate.fromFileAsync(
            ...
            listData1.forEach((item, index) => {
              ...
              workbook
                ...
                .row(i)
                .cell(2)
                .value(`${item._id.work?.sign}-${item._id.work?.name}`)
              workbook
                .sheet('Report1')
                .row(i)
                .cell(3)
              ...
              workbook
                ...
                .cell(7)
                .value(
                  Number(item._id.work?.quantity * item._id.work?.unitPrice)
                )
