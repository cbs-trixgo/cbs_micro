...
module.exports = {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await FIN__CASH_PAYMENT_MODEL.insert({
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params)
                const { cashPaymentID, name, note, amount, payment, active } =
                    ctx.params
                ...
                const resultAfterCallHandler =
                    await FIN__CASH_PAYMENT_MODEL.update({
                        cashPaymentID,
                        userID,
                        email,
                        name,
    ...
    remove: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await FIN__CASH_PAYMENT_MODEL.remove({
                        cashPaymentID,
                        userID,
                    })
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                ...
                if (cashPaymentID) {
                    resultAfterCallHandler =
                        await FIN__CASH_PAYMENT_MODEL.getInfo({
                            cashPaymentID,
                            userID,
                            select,
                            populates,
                    ...
                    resultAfterCallHandler =
                        await FIN__CASH_PAYMENT_MODEL.getList({
                            companyID: company._id,
                            active,
                            customerID,
                            userCreateID,
    ...
    getAmountByProperty: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler =
                    await FIN__CASH_PAYMENT_MODEL.getAmountByProperty({
                        companyID: company._id,
                        customerID,
                        option,
                        year,
    ...
    downloadTemplateExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await FIN__CASH_PAYMENT_MODEL.downloadTemplateExcel({
                        companyID: company._id,
                        parentID,
                        userID,
                        option,
    ...
    importFromExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await FIN__CASH_PAYMENT_MODEL.importFromExcel({
                        companyID: company._id,
                        parentID,
                        dataImport,
                        userID,
    ...
    exportExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await FIN__CASH_PAYMENT_MODEL.exportExcel({
                        customerID,
                        userCreateID,
                    })
