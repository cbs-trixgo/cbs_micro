    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let inforSubmit =
                    await BUDGET__SUBMITTAL_COLL.findById(submitID)
                ...
                let infoAfterUpdate =
                    await BUDGET__SUBMITTAL_COLL.findByIdAndUpdate(
                        submitID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    getInfo({ submitID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoContractIPC = await BUDGET__SUBMITTAL_COLL.findById(
                    submitID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await BUDGET__SUBMITTAL_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await BUDGET__SUBMITTAL_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await BUDGET__SUBMITTAL_COLL.count(conditionObjOrg)
