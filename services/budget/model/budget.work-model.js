...
class Model extends BaseModel {
    ...
    insert({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let dataInsert = {
                    userCreate: userID,
                    company: infoGroup.company,
                    budget: infoGroup.budget,
                    item: infoGroup.item,
                    group: groupID,
                    name,
                }
                ...
                if (!isNaN(vatUnitPrice) && Number(vatUnitPrice) >= 0) {
                    ...
                    dataInsert.vatAmount =
                        Number(quantity) * Number(vatUnitPrice)
                }
                ...
                if (
                    !isNaN(forecastUnitPrice) &&
                    Number(forecastUnitPrice) >= 0
                ) {
                    dataInsert.forecastUnitPrice = forecastUnitPrice
                    dataInsert.forecastAmount =
                        Number(forecastQuantity) * Number(forecastUnitPrice)
                }
                ...
                if (!infoAfterInsert)
                    return resolve({
                        error: true,
                        message: "can't_insert_budget_work",
                        keyError: "can't_insert_budget_work",
                    })

                /**
                 * CẬP NHẬT GIÁ TRỊ NGÂN SÁCH, DỰ KIẾN NGÂN SÁCH
                 */
                let InfoGroupAfterUpdate =
                    await BUDGET__GROUP_MODEL.updateValue({
                        option: 1,
                        groupID,
                        userID,
                    })
                let InfoItemAfterUpdate = await BUDGET__ITEM_MODEL.updateValue({
                    option: 1,
                    itemID: infoGroup.item,
                    userID,
                })
                let InfoBudgetAfterUpdate = await BUDGET_MODEL.updateValue({
                    option: 1,
                    budgetID: infoGroup.budget,
                    userID,
                })
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (
                    ...
                    dataUpdate.forecastAmount =
                        Number(forecastQuantity) * Number(forecastUnitPrice)
                }
                if (
                    !isNaN(forecastVatUnitPrice) &&
                    Number(forecastVatUnitPrice) >= 0
                ) {
                    dataUpdate.forecastVatUnitPrice = forecastVatUnitPrice
                    dataUpdate.forecastVatAmount =
                        Number(forecastQuantity) * Number(forecastVatUnitPrice)
                }

                let infoAfterUpdate = await BUDGET__WORK_COLL.findByIdAndUpdate(
                    workID,
                    dataUpdate,
                    { new: true }
                )
                if (!infoAfterUpdate)
                    return resolve({
                        ...
                        keyError: "can't_update_budget_work",
                        status: 403,
                    })
                ...
                let InfoGroupAfterUpdate =
                    await BUDGET__GROUP_MODEL.updateValue({
                        option: 1,
                        groupID: infoWork.group,
                        userID,
                    })
                let InfoItemAfterUpdate = await BUDGET__ITEM_MODEL.updateValue({
                    option: 1,
                    itemID: infoWork.item,
                    userID,
                })
                let InfoBudgetAfterUpdate = await BUDGET_MODEL.updateValue({
                    option: 1,
                    budgetID: infoWork.budget,
                    userID,
                })
    ...
    getList({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet = await BUDGET__WORK_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
     ...
     * Date  : 10/4/2022
     */
    updateValue({ option, workID, userID, ctx }) {
        return new Promise(async (resolve) => {
            try {
                /**
                 * BA
                ...
                if (option && Number(option) === 2) {
                    ...
                    if (infoImpleBudget) {
                        dataUpdate.impleQuantity = Number(
                            infoImpleBudget.data.quantity
                        )
                        dataUpdate.impleAmount = Number(
    ...
    importExcel({ option, budgetID, dataImport, userID }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let listItems = await BUDGET__ITEM_COLL.find({
                    _id: { $in: arrItems },
                })
                ...
                for (const data of dataImportJSON) {
                    if (index > 0) {
                        if (data?.__EMPTY_1 && data?.__EMPTY_1 != '') {
                            ...
                            let infoGroup =
                                await BUDGET__GROUP_MODEL.insert(
                                    dataGroupInsert
                                )
    ...
    copy({ option, budgetID, userID }) {
        ...
        return new Promise(async (resolve) => {
            try {
                let infoBudget = await BUDGET_COLL.findById(budgetID).select(
                    'company name sign type'
                )
                if (!infoBudget)
                    return resolve({
                        error: true,
                ...
                if (infoBudgetNew) {
                    let listItem = await BUDGET__ITEM_COLL.find({
                        budget: budgetID,
                    })
                    ...
                    for (const item of listItem) {
                        ...
                        if (infoItemNew) {
                            let listGroup = await BUDGET__GROUP_COLL.find({
                                item: item._id,
                            })
                            ...
                            for (const group of listGroup) {
                                ...
                                if (infoGroupNew) {
                                    let listWork = await BUDGET__WORK_COLL.find(
                                        { group: group._id }
                                    )
                                    ...
                                    for (const work of listWork) {
                                        ...
                                        let infoWorkNew = await that.insert({
                                            ...
                                            forecastQuantity:
                                                work.forecastQuantity,
                                            forecastUnitPrice:
                                                work.forecastUnitPrice,
                                        })
                                    }
                                }
                            }
