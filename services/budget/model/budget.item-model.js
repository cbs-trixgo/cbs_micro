...
class Model extends BaseModel {
     ...
     */
    insert({
        userID, budgetID, plus, reasonID, name, sign, unit, description, note, type, quantity,
        unitPrice, forecastAmount, forecastQuantity
    }) {
        return new Promise(async (resolve) => {
            try {
                let infoBudget = await BUDGET_COLL.findById(budgetID);
                if(!infoBudget)
                    return resolve({ error: true, message: "Budget không hợp lệ", keyError: KEY_ERROR.PARAMS_INVALID });

                let dataInsert = { userCreate: userID, company: infoBudget.company, budget: budgetID };
                ...
                if(!infoAfterInsert)
                    return resolve({ error: true, message: "Không thể thêm contract ipc" ,keyError: "can't_insert_budget_item" });
     ...
     */
    update({
        itemID, plus, reasonID, name, sign, unit, unitPrice, forecastAmount, forecastQuantity, description, note, type, quantity, userID
    }){
        return new Promise(async (resolve) => {
            try {
                if(!checkObjectIDs(itemID))
                    return resolve({ error: true, message: "Mã hiệu không đúng", keyError: "itemID_invalid" });
                ...
                let infoAfterUpdate = await BUDGET__ITEM_COLL.findByIdAndUpdate(itemID, dataUpdate, { new: true });
                if(!infoAfterUpdate)
     ...
     * Date: 9/4/2022
     */
    getList({ budgetID, userID,
        keyword, limit = 50, lastestID, select, populates= {} }) {
        return new Promise(async (resolve) => {
            try {
