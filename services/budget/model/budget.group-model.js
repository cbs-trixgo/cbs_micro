...
class Model extends BaseModel {
            ...
            try {
                ...
                if (!infoAfterInsert)
                    return resolve({
                        ...
                        keyError: "can't_insert_budget_group",
                    })
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await BUDGET__GROUP_COLL.findByIdAndUpdate(
                        groupID,
                        dataUpdate,
                        { new: true }
                    )
                if (!infoAfterUpdate)
                    return resolve({
                        ...
                        keyError: "can't_update_budget_group",
                        status: 403,
                    })
    ...
    getList({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet = await BUDGET__GROUP_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await BUDGET__GROUP_COLL.count(conditionObjOrg)
     ...
     * Date  : 10/4/2022
     */
    updateValue({ option, groupID, userID, ctx }) {
        return new Promise(async (resolve) => {
            try {
                /**
                 * BA
                ...
                if (option && Number(option) === 1) {
                    ...
                    if (listDataWork && listDataWork.length) {
                        ...
                        dataUpdate.forecastAmount = Number(
