 ...
 module.exports = {
    ...
    insert: {
        ...
        async handler(ctx) {
            try {
                ...
                const { itemID, plus, reasonID, name, sign, unit, unitPrice, forecastAmount, forecastQuantity, description, note, type, quantity } = ctx.params;

                const { _id: userID } = ctx.meta.infoUser;

                const resultAfterCallHandler = await BUDGET__GROUP_MODEL.insert({
                    ctx, userID, itemID, plus, reasonID, name, sign, unit, unitPrice, forecastAmount, forecastQuantity, description, note, type, quantity
                });
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { groupID, plus, reasonID, name, sign, unit, unitPrice, forecastAmount, forecastQuantity, description, note, type, quantity } = ctx.params;

                const { _id: userID } = ctx.meta.infoUser;
                let resultAfterCallHandler = await BUDGET__GROUP_MODEL.update({
                    groupID, plus, reasonID, name, sign, unit, unitPrice, forecastAmount, forecastQuantity, description, note, type, quantity, userID, ctx
                });
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { groupID, itemID, budgetID, keyword, limit, lastestID, populates, select } = ctx.params;
                const { _id: userID } = ctx.meta.infoUser;
                ...
                if(groupID){
                    ...
                    resultAfterCallHandler = await BUDGET__GROUP_MODEL.getList({
                        itemID, budgetID, userID, keyword, limit, lastestID, select, populates
                    });
                }
