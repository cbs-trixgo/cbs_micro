 ...
 module.exports = {
    ...
    insert: {
        ...
        async handler(ctx) {
            try {
                ...
                let { groupID, plus, reasonID, name, sign, unit, description, note, type, quantity, unitPrice, vatUnitPrice, amount, forecastQuantity, forecastUnitPrice, forecastAmount } = ctx.params;

                const { _id: userID } = ctx.meta.infoUser;

                let resultAfterCallHandler = await BUDGET__WORK_MODEL.insert({
                    ctx, userID, groupID, plus, reasonID, name, sign, unit, description, note, type, quantity, unitPrice, vatUnitPrice, amount, forecastQuantity, forecastUnitPrice, forecastAmount
                });
    ...
    update: {
        ...
        async handler(ctx) {  
            try {
                await this.validateEntity(ctx.params);
                let { workID, plus, reasonID, name, sign, unit, description, note, type, quantity, unitPrice, vatUnitPrice, amount, vatAmount, forecastQuantity, forecastUnitPrice, forecastVatUnitPrice, forecastAmount, forecastVatAmount } = ctx.params;
            
                const { _id: userID } = ctx.meta.infoUser;
                let resultAfterCallHandler = await BUDGET__WORK_MODEL.update({
                workID, plus, reasonID, name, sign, unit, description, note, type, quantity, unitPrice, vatUnitPrice, amount, vatAmount, forecastQuantity, forecastUnitPrice, forecastVatUnitPrice, forecastAmount, forecastVatAmount, userID, ctx
                });
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { companyID, workID, groupID, itemID, budgetID, keyword, limit, lastestID, populates, select } = ctx.params;
                ...
                if(workID){
                    resultAfterCallHandler = await BUDGET__WORK_MODEL.getInfo({
                        workID, userID, select, populates
                    });
                }else{
                    resultAfterCallHandler = await BUDGET__WORK_MODEL.getList({
                        companyID, budgetID, itemID, groupID, userID, keyword, limit, lastestID, select, populates
                    });
                }
    ...
    downloadTemplateExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await BUDGET__WORK_MODEL.downloadTemplateExcel({
                    companyID: company._id, budgetID, userID 
                })
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await BUDGET__WORK_MODEL.importExcel({
                    budgetID, dataImport, userID, ctx
                })
                return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx });
            } catch (error) {
                return renderStatusCodeAndResponse({ error_message: error.message, ctx });
