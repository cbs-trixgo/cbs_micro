    ...
    insert({ subjectID, userID, ctx }) { 
        return new Promise(async (resolve) => {
            try {   
                ...
                const checkExist = await TRAINING__SUBJECT_SEEN_COLL.findOne({ subject: subjectID, userCreate: userID });
                if(checkExist){
                    await TRAINING__SUBJECT_COLL.findByIdAndUpdate(subjectID, { $inc: { amountViews: 1 }}, { new:true });
                    return resolve({ error: false, message: "Ok", status: 200 });
                }else{

                    let infoUser = await ctx.call(`${CF_DOMAIN_SERVICES.AUTH}.${CF_ACTIONS_AUTH.USER_GET_INFO_AND_GET_LIST}`, {
                        userID, select: "_id company birthDay", populates: JSON.stringify({ path: "company" })
                    })
                    ...
                    if(!infoAfterInsert)
                        return resolve({ error: true, message: "Can't insert",  keyError: KEY_ERROR.INSERT_FAILED });

                    // Cập nhật amountUserViews
                    await TRAINING__SUBJECT_COLL.findByIdAndUpdate(subjectID, { $inc: { amountUserViews: 1, amountViews: 1 }}, { new:true });
    ...
    getList({ subjectID, limit = 10, lastestID, select, populates = {} }) { 
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet = await TRAINING__SUBJECT_SEEN_COLL.find(conditionObj)
                    .select(select)
                    .limit(limit+1)
                    .populate(populates)
                    .sort(sortBy)
                ...
                let totalRecord = await TRAINING__SUBJECT_SEEN_COLL.count(conditionObjOrg);
