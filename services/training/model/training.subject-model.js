    ...
    remove({ subjectID, userID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                if (
                    infoData &&
                    infoData.userCreate.toString() != userID.toString()
                )
                    return resolve({
                        error: true,
                        message:
                            'Bạn không có quyền xóa, người tạo mới được phép xóa',
    ...
    getInfo({ subjectID, userID, select, populates, ctx }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let checkUserRating =
                    await TRAINING__SUBJECT_RATING_COLL.findOne({
                        lession: subjectID,
                        userCreate: userID,
                    }).select('score')
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await TRAINING__SUBJECT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await TRAINING__SUBJECT_COLL.find(
                    conditionObj
                )
                    .select(select)
                    .limit(limit + 1)
                ...
                let totalRecord =
                    await TRAINING__SUBJECT_COLL.count(conditionObjOrg)
