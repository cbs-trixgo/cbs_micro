    ...
    insert: {
        ...
        async handler(ctx) {
            try {
                ...
                return await FILE__CORE_MODEL.insert({ app, name, path, size, userID });
            } catch (error) {
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                ...
                return await FILE__CORE_MODEL.update({ fileID, status, description, userID });
            } catch (error) {
                return { error: true, message: error.message };
            }
        }
    ...
    getInfoFile: {
        ...
        async handler(ctx) {
            try {
                ...
                return await FILE__CORE_MODEL.getInfoFile({ authorID, fileID, select, populates });
            } catch (error) {
                return { error: true, message: error.message, status: 500 };
            }
        }
    ...
    getListFileWithStatus: {
        ...
        async handler(ctx) {
            try {
                ...
                return await FILE__CORE_MODEL.getListFileWithStatus({ status, userID });
            } catch (error) {
                return { error: true, message: error.message };
            }
        }
    ...
    generateLinkS3: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let {
                    appID, mimeType, type, name, size, typeUpload,
                    companyID, projectID, groupID, packageID, conversationID, taskID
                } = ctx.params;
                ...
                let infoGenerateLink = await GENERATE_LINK_S3(newFileName, mimeType, typeUpload, pathS3);
                /**
