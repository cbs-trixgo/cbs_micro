...
const { checkObjectIDs, checkNumberIsValidWithRange, checkNumberValid } = require('../../../tools/utils/utils');
const { FILE_PROCESSING, FILE_UPLOAD_ERROR, FILE_UPLOAD_SUCCESS }  = require('../helper/file.keys-constant');
...
class Model extends BaseModel {
    ...
    insert({ appID, companyID, projectID, groupID, nameOrg, name, path, size, mimeType, type, userID, ctx }) {
        return new Promise(async (resolve) => {
            try {
                ...
                if(type && !checkNumberIsValidWithRange({ arrValid: [1,2,3,4], val: type })) {
                    return resolve({
                        error: true,
    ...
    update({ fileID, status, description, userID }) {
        return new Promise(async resolve => {
            try {
                ...
                if(status) {
                    if (!checkNumberIsValidWithRange({ arrValid: listStatusForUpdateValid, val: status }))
                    ...
                    if (infoFile.status === FILE_PROCESSING)
                        return resolve({ error: true, message: 'file_not_processing' });

                    if (infoFile.userCreate != userID)
                        return resolve({ error: true, message: 'file_not_auth' });
                ...
                let infoAfterUpdate = await FILE__CORE_COLL.findByIdAndUpdate(fileID, dataUpdate, { new: true });
                if(!infoAfterUpdate)
                    return resolve({ error: true, message: 'cannot_insert' });
    ...
    deleteFiles({ fileIDs, authorID }) {
        return new Promise(async resolve => {
            try {
                ...
                const infoBeforeDelete = await FILE__CORE_COLL.find({ _id: { $in: fileIDs }, author: authorID });
                if(!infoBeforeDelete.length)
                    return resolve({ error: true, message: 'cannot_find_files' });

                const infoAfterDelete = await FILE__CORE_COLL.deleteMany({ _id: { $in: fileIDs } });
                if(!infoAfterDelete)
                    return resolve({ error: true, message: 'cannot_delete_files' });
    ...
    getInfoFile({ fileID, authorID, select, populates }) {
        ...
        return new Promise(async resolve => {
            try {
                 ...
                 *  3.2: operation database
                 */
                let infoFile = await FILE__CORE_COLL
                    .findById(fileID)
                    .select(select)
                    .populate(populates)
                    .lean();
    ...
    getListFileWithStatus({ status='all', userID }) {
        return new Promise(async resolve => {
            try {
                 ...
                 * DECALARTION VARIABLE (1)
                 */
                let listStatusValid = [FILE_PROCESSING, FILE_UPLOAD_ERROR, FILE_UPLOAD_SUCCESS];
                 /**
                 * VALIDATION STEP (2)
                 *  - kiểm tra valid từ các input
                 *  - kiểm tra valid từ database (ví dụ: đảm bảo foreign_key hoặc các điều kiện của coll)
                 */
                if (!checkNumberIsValidWithRange({ arrValid: listStatusValid, val: status }) && status != 'all')
                    return resolve({ error: true, message: 'status_invalid' });
                ...
                let listFiles = await FILE__CORE_COLL.find(conditionObj).sort({ createAt: -1 });
                if (!listFiles)
