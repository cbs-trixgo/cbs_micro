    ...
    insert({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                project &&
                    checkObjectIDs(project) &&
                    (dataInsert.project = project)
                contract &&
                    checkObjectIDs(contract) &&
            ...
            try {
                ...
                if (isUpdateSendSuccess) {
                    infoAfterUpdateStatus =
                        await NOTIFICATION__LOG_COLL.updateMany(
                            {
                                status: 1,
                                receiver: userID,
                            },
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await NOTIFICATION__LOG_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let listNotifications = await NOTIFICATION__LOG_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .select(select)
                ...
                let totalRecord =
                    await NOTIFICATION__LOG_COLL.count(conditionObjOrg)
            ...
            try {
                ...
                if (!infoAfterCall.error) {
                    ...
                    if (listRecords && Array.isArray(listRecords)) {
                                ...
                                listDeviceFromMobile = [
                                    ...
                                    ...devices
                                        ...
                                        .map((itemReturn) => {
                                            return {
                                                oneSignalID:
                                                    itemReturn.oneSignalID,
                                                app_id: _findAppIdWithEnv(
                                                    itemReturn.env
                                                ),
                                            }
                                        }),
                        ...
                        if (
                            listDeviceFromMobile &&
                            listDeviceFromMobile.length
                        ) {
                            /**
                             * chia mảng listDeviceFromMobile thành 2 nhóm để push theo app_id của (CBS và TEASER)
                             */
                            let listDeviceFromMobileWith_CBS =
                                listDeviceFromMobile.filter(
                                    (item) => item.app_id == ONESIGNAL_APP_CBS
                                )

                            let listDeviceFromMobileWith_TEASER =
                                listDeviceFromMobile.filter(
                                    (item) =>
                                        item.app_id == ONESIGNAL_APP_TEASER
                                )

                            let listDeviceFromMobileWith_FNB =
                                listDeviceFromMobile.filter(
                            ...
                            let dataForMobilePushTeaser = {
                                ...
                                priority: 'high',
                                include_player_ids:
                                    listDeviceFromMobileWith_TEASER.map(
                                        (item) => item.oneSignalID
                                    ),
                            }
                            if (objDataForSend.amountNoti) {
                            ...
                            let dataForMobilePushFNB = {
                                ...
                                priority: 'high',
                                include_player_ids:
                                    listDeviceFromMobileWith_FNB.map(
                                        (item) => item.oneSignalID
                                    ),
                            }
                            if (objDataForSend.amountNoti) {
                                dataForMobilePushFNB.ios_badgeType = 'SetTo'
                                dataForMobilePushFNB.ios_badgeCount =
                                    objDataForSend.amountNoti
                            }
                            // console.info('MOBILE PUSH (FNB)', dataForMobilePushFNB.include_player_ids);

                            if (
                                dataForMobilePushFNB.include_player_ids.length
                            ) {
                        ...
                        if (listDeviceFromWebBrowser.length) {
                            ...
                            let dataForWebPushCBS = {
                                ...
                                web_url,
                                include_player_ids:
                                    listDeviceFromWebBrowserWith_CBS.map(
                                        (item) => item.oneSignalID
                                    ),
                            }
                            ...
                            let dataForWebPushTeaser = {
                                ...
                                web_url,
                                include_player_ids:
                                    listDeviceFromWebBrowserWith_TEASER.map(
                                        (item) => item.oneSignalID
                                    ),
                            }
