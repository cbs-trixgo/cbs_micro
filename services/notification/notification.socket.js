 ...
 */
const {
  EVENT_SOCKET_CONSTANT_NOTIFICATION,
} = require('./helper/notification.events-socket-constant')
...
exports.NOTIFICATION_SOCKET = {
  [EVENT_SOCKET_CONSTANT_NOTIFICATION.NOTIFICATION_CSS_RECEIVE_NEW_NOTIFICATION]:
    async function (data) {
      let socket = this
      ...
      if (!receivers || !receivers.length) {
        return sendDataToMultilSocketsOfListUser({
          ...
          data: {
            error: true,
            message: "Can't get receivers notification",
          },
          event: [
            EVENT_SOCKET_CONSTANT_NOTIFICATION.NOTIFICATION_SSC_RECEIVE_NEW_NOTIFICATION,
          ],
          io,
        })
      }

      let listReceivers = await broker.call(
