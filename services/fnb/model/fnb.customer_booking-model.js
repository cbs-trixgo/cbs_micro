        ...
        return new Promise(async (resolve) => {
            try {
                ...
                const infoAfterUpdate =
                    await FNB_CUSTOMER_BOOKING_COLL.findByIdAndUpdate(
                        customerCareID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    getInfo({ customerCareID, select, populates = {} }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoPlanGroup = await FNB_CUSTOMER_BOOKING_COLL.findById(
                    customerCareID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await FNB_CUSTOMER_BOOKING_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await FNB_CUSTOMER_BOOKING_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await FNB_CUSTOMER_BOOKING_COLL.count(conditionObjOrg)
                ...
                if (!option) {
                    ...
                    if (option != 5) {
                        ...
                        if (
                            ...
                            await FNB_CUSTOMER_BOOKING_COLL.populate(
                                listData,
                                conditionPopulate
                            )
                        }
    ...
    exportExcel({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let template =
                    '../../../files/templates/excels/crm_export_booking.xlsx'

                let listData = await FNB_CUSTOMER_BOOKING_COLL.find({
                    company: companyID,
                ...
                let listUser = await USER_COLL.find({
                    _id: { $in: listData2.map((item) => item._id.assignee) },
                }).select('fullname')
