...
/**
     ...
     * Code: 12/2/2024
     */
insert({ userID, companyID, customerID, orderID, name, note, date, status, files }) {
 ...
 * Code: 12/2/2024
 */
update({ userID, customerCareID, customerID, orderID, name, note, date, status, imagesID }) {
    return new Promise(async resolve => {
        try {
            ...
            const infoAfterUpdate = await FNB_AFFILIATE_SIGNUP_COLL.findByIdAndUpdate(customerCareID, dataUpdate, { new: true });
            if (!infoAfterUpdate)
    ...
    getInfo({ customerCareID, select, populates={} }) {
        return new Promise(async resolve => {
            try {
                ...
                let infoPlanGroup = await FNB_AFFILIATE_SIGNUP_COLL.findById(customerCareID)
                                    .select(select)
                                    .populate(populates)
     ...
     * Code: 24/11/2022
     */
    getList({ companyID, userID, fundasID, customerID, userCreateID, channelID, businessID, fromDate, toDate, status, keyword, limit = 20, lastestID, select, populates }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(lastestID && checkObjectIDs(lastestID)){
                    let infoData = await FNB_AFFILIATE_SIGNUP_COLL.findById(lastestID);
                    if(!infoData)
                ...
                let infoDataAfterGet = await FNB_AFFILIATE_SIGNUP_COLL.find(conditionObj)
                    .limit(limit+1)
                    .sort(sortBy)
                    .select(select)
                    .populate(populates)
                ...
                let totalRecord = await FNB_AFFILIATE_SIGNUP_COLL.count(conditionObjOrg);
    ...
    exportExcel({ companyID, userID, fundaID, fromDate, toDate, month, year }) {
        ...
        return new Promise(async resolve => {
            try {
                ...
                let listData = await ITEM__CONTACT_COLL.find({company: companyID, parent: {$exists: true, $ne: null}})
                .populate({
                ...
                let listAff = await ITEM__CONTACT_COLL.find({_id: {$in: listData2.map(item => item._id.dataSource)}}).select('name phone email')
                // console.log(listAff)
               
                // Modify the workbook.
                XlsxPopulate.fromFileAsync(path.resolve(__dirname, ('../../../files/templates/excels/crm_export_affiliate.xlsx')))
                .then(async workbook => {
                    ...
                    listData?.forEach((item, index) => {
                        ...
                        workbook.sheet("Data").row(i).cell(7).value(item?.joinedDate);
                        workbook.sheet("Data").row(i).cell(8).value(`${item?.parent?.name}-${item?.parent?.phone}`);
                        workbook.sheet("Data").row(i).cell(9).value(item?.parent?.level);
                    ...
                    listData2?.forEach((item, index) => {
                        for(var i=4; i<=listAff.length + 3; i++){
                            ...
                            for(var j=1; j<=12; j++){
                                if(item._id.month == j && userID.toString() === item._id.dataSource.toString()){
