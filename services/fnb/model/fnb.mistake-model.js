...
class Model extends BaseModel {
    ...
    insert({
        ...
        return new Promise(async (resolve) => {
            try {
                if (
                    !checkObjectIDs(mistakeID) ||
                    !checkObjectIDs(executorID) ||
                    !checkObjectIDs(userID)
                )
                    return resolve({
                        error: true,
                        message: 'orderID|userID invalid',
                ...
                if (checkObjectIDs(fundaID)) {
                    ...
                    infoFunda = await ITEM__FUNDA_COLL.findOne({
                        company: companyID,
                    })
                }

                let countMistake = await FNB_MISTAKE_COLL.count({
                    executor: executorID,
                    mistake: mistakeID,
                })
                ...
                if (orderID && checkObjectIDs(orderID)) {
                    ...
                    let infoShift = await FNB_SHIFT_COLL.findById(
                        infoOrder.shift
                    )
                    if (!infoShift)
                        return resolve({
                ...
                await ITEM__FUNDA_COLL.findByIdAndUpdate(
                    infoFunda._id,
                    {
                        $inc: {
                            numberOfMistakes: 1,
                            amountOfMistakes: Number(amount) | 0,
                            amountOfBonus: Number(bonus) | 0,
                        },
                    },
                    { new: true }
                )
                ...
                if (orderID && checkObjectIDs(orderID)) {
                    await FNB_SHIFT_COLL.findByIdAndUpdate(
                        infoOrder.shift,
                        {
                            $inc: {
                                numberOfMistakes: 1,
                    ...
                    await FNB_ORDER_COLL.findByIdAndUpdate(
                        orderID,
                        {
                            $inc: {
                                numberOfMistakes: 1,
        ...
        return new Promise(async (resolve) => {
            try {
                if (!checkObjectIDs(orderMistakeID))
                    return resolve({
                        error: true,
                        message: 'orderMistakeID invalid',
                        keyError: KEY_ERROR.PARAMS_INVALID,
                    })
                ...
                let infoAfterUpdate = await FNB_MISTAKE_COLL.findByIdAndUpdate(
                    orderMistakeID,
                    dataUpdate,
                    { new: true }
                )
        ...
        return new Promise(async (resolve) => {
            try {
                if (!checkObjectIDs(executorID))
                    return resolve({
                        error: true,
                        message: 'executorID invalid',
                        keyError: KEY_ERROR.PARAMS_INVALID,
                    })

                let dataInsert, dataUpdate
                let info = await FNB_MISTAKE_COLL.findOne({
                    company: companyID,
                    month,
                    year,
                    executor: executorID,
                ...
                // Cập nhật mẩu tin
                    ...
                    let infoAfterUpdate =
                        await FNB_MISTAKE_COLL.findByIdAndUpdate(
                            info._id,
                            dataUpdate,
                            { new: true }
                        )
    ...
    getInfo({ orderMistakeID, select, populates = {} }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoPlanGroup = await FNB_MISTAKE_COLL.findById(
                    orderMistakeID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                XlsxPopulate.fromFileAsync(
                    path.resolve(
                        __dirname,
                        '../../../files/templates/excels/fnb_export_mistake.xlsx'
                    )
                ).then(async (workbook) => {
                    workbook
                        .sheet('DashBoard')
                    ...
                    listDataErrCombine3?.forEach((item, index) => {
                        for (var m = 1; m <= 12; m++) {
                            listDataErrCombine4?.forEach((mistake) => {
                                if (
                                    mistake._id.executor._id.toString() ===
                                        item._id.executor._id.toString() &&
                                    Number(mistake._id.month) === Number(m)
                                ) {
                                    workbook
                                        .sheet('DashBoard')
                                        .row(i)
                                        .cell(m * 2 + 2)
                                        .value(mistake?.number)
                                    workbook
                                        .sheet('DashBoard')
                                        .row(i)
                                        .cell(m * 2 + 3)
                                        .value(mistake?.amount)
                                }
                            })
                        }
                        i++
