  ...
  update({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoFunda = await ITEM__FUNDA_COLL.findById(infoShift.funda).select(
              ...
              let info =
                await TIMESHEET__EXPERT_TIMESHEET_COLL.findOneAndUpdate(
                  ...
                  },
                  {
                    ...
                    unitprice: infoShift.officialStaffSalar,
                    amount: Number(
                      infoShift.workingHours * infoShift.officialStaffSalar
                    ),
                  },
                  { new: true }
                )
              ...
              let info =
                await TIMESHEET__EXPERT_TIMESHEET_COLL.findOneAndUpdate(
                  ...
                  },
                  {
                    ...
                    unitprice: infoShift.trialStaffSalary,
                    amount: Number(
                      infoShift.subWorkingHours * infoShift.trialStaffSalary
                    ),
                  },
                  { new: true }
                )
    ...
    return new Promise(async (resolve) => {
      try {
        if (
           ...
           */
          if (fundaID) {
            let infoFunda = await ITEM__FUNDA_COLL.findById(fundaID).populate({
              path: 'company',
              select: 'name sign',
            })
                  ...
                  if (Number(item._id.month) === Number(m)) {
                    for (let i = 5; i <= 35; i++) {
                      for (let j = 2; j <= Number(listStaffs.length + 1); j++) {
                        ...
                        if (
                          item._id.staff.toString() === staffID.toString() &&
                          Number(i - 4) === Number(item._id.day)
                        ) {
                          workbook
                            .sheet(`T${m}`)
                            .row(i)
                    ...
                    for (let i = 5; i <= 35; i++) {
                      for (let j = 2; j <= Number(listStaffs.length + 1); j++) {
                        ...
                        if (
                          ...
                          let timesheet =
                            workbook.sheet(`T${m}`).row(i).cell(j).value() | 0
                          ...
                          workbook
                            ...
                            .cell(j)
                            .value(
                              Number(timesheet) + Number(item.subWorkingHours)
                            )
                        }
                      }
                    }
                  ...
                  for (var m = 1; m <= 12; m++) {
                    if (Number(item._id.month) === Number(m)) {
                      for (let i = 4; i <= Number(listStaffs.length + 3); i++) {
                        ...
                        if (item._id.staff.toString() === staffID.toString()) {
                          let timesheet =
                            workbook.sheet(`T${m}`).row(i).cell(6).value() | 0
