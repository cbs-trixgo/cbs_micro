    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await FNB_ORDER_PRODUCT_COLL.findByIdAndUpdate(
                        productID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    getInfo({ productID, select, populates = {} }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoPlanGroup = await FNB_ORDER_PRODUCT_COLL.findById(
                    productID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await FNB_ORDER_PRODUCT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await FNB_ORDER_PRODUCT_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await FNB_ORDER_PRODUCT_COLL.count(conditionObjOrg)
            ...
            try {
                ...
                if (Number(duration / 86400) > 90) {
                    ...
                    if (!isNaN(option) && [1].includes(Number(option))) {
                        await FNB_ORDER_PRODUCT_COLL.populate(
                            listData,
                            conditionPopulate
                        )
                    }
