 ...
 * import inter-coll
 */
const FNB_ZALOOA_COLL                         = require('../database/fnb.zalooa-coll');
...
class Model extends BaseModel {
     ...
     * Code: 24/11/2022
     */
    insert({ userID, companyID, parentID, name, sign, unit, note, quantity, unitPrice, unitPrice2, unitPrice3, unitPrice4, type }) {
        ...
        return new Promise(async resolve => {
            try {
                if(!checkObjectIDs(companyID) || !name)
                    return resolve({ error: true, message: 'companyID|name invalid', keyError: KEY_ERROR.PARAMS_INVALID });
                ...
                if(sign && sign != ""){
                    let checkExist = await FNB_ZALOOA_COLL.findOne({ company: companyID, sign });
                    if(checkExist)
     ...
     * Code:
     */
    update({ companyID, productID, userID, parentID, type, name, sign, unit, note, quantity, unitPrice, unitPrice2, unitPrice3, unitPrice4, imagesID, fundasID, option, convertOption, status }) {
        ...
        return new Promise(async resolve => {
            try {
                if(!option){
                    ...
                    let infoAfterUpdate = await FNB_ZALOOA_COLL.findByIdAndUpdate(productID, dataUpdate, { new: true });
                    if (!infoAfterUpdate)
                        return resolve({ error: true, message: 'Cập nhật thất bại', keyError: KEY_ERROR.UPDATE_FAILED });

                    return resolve({ error: false, data: infoAfterUpdate });
                }else{
                    let infoAfterUpdate = await FNB_ZALOOA_COLL.updateMany(
                        // { company: companyID}, 
    ...
    getInfo({ productID, select, populates={} }) {
        return new Promise(async resolve => {
            try {
                ...
                let infoPlanGroup = await FNB_ZALOOA_COLL.findById(productID)
                                    .select(select)
                                    .populate(populates)
     ...
     * Code: 24/11/2022
     */
    getList({ companyID, parentID, fundaID, type, isParent, convertOption, keyword, limit = 10, lastestID, select, populates, sortKey, userID, status }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(lastestID && checkObjectIDs(lastestID)){
                    let infoData = await FNB_ZALOOA_COLL.findById(lastestID);
                    if(!infoData)
                ...
                let infoDataAfterGet = await FNB_ZALOOA_COLL.find(conditionObj)
                    .limit(limit+1)
                    .sort(sortBy)
                    .select(select)
                    .populate(populates)
                ...
                let totalRecord = await FNB_ZALOOA_COLL.count(conditionObjOrg);
