    ...
    insert({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoCustomer =
                    await ITEM__COCNTACT_COLL.findById(customerID)
                ...
                if (
                    infoCustomer?.name != '' &&
                    infoCustomer?.name != 'undefined'
                ) {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(infoCustomer?.name)
                }
                if (
                ...
                if (
                    infoCustomer?.sign != '' &&
                    infoCustomer?.sign != 'undefined'
                ) {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(infoCustomer?.sign)
                }
                if (
                    infoCustomer?.note != '' &&
                    infoCustomer?.note != 'undefined'
                ) {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(infoCustomer?.note)
                }
                if (
                    infoCustomer?.phone != '' &&
                    ...
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(infoCustomer?.phone)
                }
                if (
                    infoCustomer?.taxid != '' &&
                    infoCustomer?.taxid != 'undefined'
                ) {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(infoCustomer?.taxid)
                }
                if (
                    infoCustomer?.email != '' &&
                    infoCustomer?.email != 'undefined'
                ) {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(infoCustomer?.email)
                }
                if (
                    ...
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(infoCustomer?.identity)
                }
                if (
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (customerID && checkObjectIDs(customerID)) {
                    ...
                    if (
                        infoCustomer &&
                        checkObjectIDs(infoCustomer.dataSource)
                    ) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await FNB_CUSTOMER_CARE_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await FNB_CUSTOMER_CARE_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await FNB_CUSTOMER_CARE_COLL.count(conditionObjOrg)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!option) {
                    ...
                    if (
                        !isNaN(option) &&
                        [1, 3, 4, 5].includes(Number(option))
                    ) {
                        await FNB_CUSTOMER_CARE_COLL.populate(
                            listData,
                            conditionPopulate
                        )
    ...
    exportExcel({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (option && option == 1) {
                    template =
                        '../../../files/templates/excels/crm_export_support.xlsx'
                            ...
                            workbook
                                ...
                                .value(
                                    item.status &&
                                        ...
                                        ? CRM_STATUS[Number(item?.status) - 1]
                                              .text
                                        : ''
                                )
                            workbook
                        ...
                        listUser?.forEach((item, index) => {
                            for (var m = 1; m <= listJourney.length; m++) {
                                listData?.forEach((subject) => {
                                    ...
                                    if (
                                        subject._id.userCreate.toString() ===
                                            item._id.toString() &&
                                        subject._id.journey.toString() ===
                                            journeyID.toString()
                                    ) {
                                        workbook
                                            .sheet('Report')
