      ...
      try {
        ...
        if (salesChannel == 1) {
          ...
          if (
            ...
            if (
              ...
              if (
                Number(infoCustomer.purchasedOffValue) <
                Number(FNB_FACTOR.credit.level4.purchasedOffValue)
              ) {
          ...
          if (voucherID && checkObjectIDs(voucherID)) {
            ...
            if (Number(infoVoucher.salesoffAmount) > 0) {
              ...
              offer = Number(
                (Number(infoVoucher.salesoffRate) * Number(total)) / 100
              ).toFixed(0)
            }
           ...
           * ĐIỂM TRUNG THÀNH
           */
          loyaltyPoints = Number(Number(amount) * Number(factor / 100)).toFixed(
            0
          )
        ...
        if (infoShift.staffs && infoShift.staffs.length) {
          ...
          if (infoShift.subStaffs && infoShift.subStaffs.length) {
            ...
            numberStaff =
              Number(numberStaff) + Number(infoShift.subStaffs.length)
          }
        ...
        if (affiliateID && checkObjectIDs(affiliateID)) {
          ...
          for (const item of infoAffiliate.nestedParents) {
            ...
            let infoNetworkCom = await FNB_NETWORK_COM_MODEL.insert({
              ...
              commission:
                (Number(FNB_AFFILIATE_COM[Number(networkLevel)].rate) *
                  amount) /
                100,
            })
          ...
          for (const item of listData) {
            ...
            if (item._id.status && item._id.status == 4) {
              numberOfCancelOrders =
                Number(numberOfCancelOrders) + Number(item.quantity)
              totalOfCancelOrders =
        ...
        let listData1 = await FNB_ORDER_PRODUCT_COLL.aggregate([
          {
            $match: {
              order: {
                $in: listOrders.map((item) => ObjectID(item._id)),
              },
              shift: ObjectID(shiftID),
            },
          },
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (listData3 && listData3.length) {
          ...
          totalLoyaltyPoints =
            Number(totalLoyaltyPoints) + Number(listData3[0].loyaltyPoints)
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (Number(duration / 86400) > 370) {
          return resolve({
            error: true,
            message: 'Chỉ tra cứu được trong khoảng thời gian <= 90 ngày',
            keyError: KEY_ERROR.PARAMS_INVALID,
          })
        } else {
          let yearFilter
      ...
      try {
        ...
        if (1 > 0) {
          ...
          let conditionObjYear = {
            year: Number(yearFilter),
            month: {
              $in: [currentMonth - 2, currentMonth - 1, currentMonth],
            },
          }
          ...
          XlsxPopulate.fromFileAsync(
            ...
            listDataErr?.forEach((item, index) => {
              ...
              workbook
                ...
                .row(i)
                .cell(3)
                .value(`${item?._id.executor?.department?.name}`)
          ...
          if (!option || option == undefined) {
            ...
            XlsxPopulate.fromFileAsync(
              ...
              listData?.forEach((item, index) => {
                ...
                workbook
                  ...
                  .row(i)
                  .cell(5)
                  .value(`${FNB_SHIFT_TYPES[Number(item.shiftType) - 1].text}`)
                ...
                workbook
                  ...
                  .row(i)
                  .cell(11)
                  .value(`${FNB_STATUS[Number(item.status) - 1].text}`)
                workbook
                  .sheet('Order')
                  .row(i)
                  .cell(12)
                ...
                workbook
                  ...
                  .value(
                    item.amount != 0
                      ? Number(item.loyaltyPoints) / Number(item.amount)
                      : 0
                  )
                workbook
                  .sheet('Order')
                  ...
                  .value(
                    item.total != 0
                      ? Number(item.salesoff) / Number(item.total)
                      : ''
                  )
                if (
                  item.customerType &&
                ...
                workbook
                  ...
                  .row(i)
                  .cell(33)
                  .value(item?.referrer ? `${item?.referrer?._id}` : '')
                workbook
                  .sheet('Order')
                  .row(i)
                  .cell(34)
                  .value(item?.referrer ? item?.referrer?.phone : '')
                workbook
                  .sheet('Order')
                  .row(i)
                  .cell(35)
                  .value(item?.referrer ? item?.referrer?.name : '')
                workbook
                  .sheet('Order')
                  .row(i)
                  .cell(36)
                  .value(item?.campaignType ? item?.campaignType?.name : '')
              ...
              XlsxPopulate.fromFileAsync(
                    ...
                    if (Number(item._id.month) === Number(m)) {
                      for (var i = 3; i <= Number(listFuda.length + 2); i++) {
                        ...
                        if (item._id.funda.toString() === fundaID.toString()) {
                          for (var j = 4; j <= 10; j++) {
                            if (
                              Number(item._id.salesChannel) === Number(j - 3)
                            ) {
                              workbook
                                .sheet(`T${m}`)
                                .row(i)
                ...
                for (var m = 1; m <= 12; m++) {
                  for (var i = 3; i <= Number(listFuda.length + 2); i++) {
                    ...
                    listData?.forEach((item, index) => {
                      if (
                        item._id.funda.toString() === fundaID.toString() &&
                ...
                listData2?.forEach((item, index) => {
                  for (var i = 3; i <= 379; i++) {
                    ...
                    for (var j = 5; j <= 11; j++) {
                      if (
                        Number(item._id.salesChannel) === Number(j - 4) &&
                        Number(item._id.day) === Number(xlsDay) &&
                        Number(item._id.month) === Number(xlsMonth)
                      ) {
                        workbook
                          .sheet(`BaoCaoNgay`)
                          .row(i)
            ...
            if (option == 2) {
              if (
                ...
                if (fundasID && fundasID.length) {
                  listFundaIsMember = await ITEM__FUNDA_COLL.find({
                    _id: { $in: fundasID },
                  }).select('_id name')
                  let arrFun = fundasID.map((item) => ObjectID(item))
                ...
                conditionPopulate = {
                  path: '_id.customer',
                  select:
                    '_id name phone note funda customerType type nonResident',
                  populate: {
                    path: 'funda',
                    select: '_id name',
                    model: 'funda',
                ...
                XlsxPopulate.fromFileAsync(
                  ...
                  workbook
                    ...
                    .row(1)
                    .cell(1)
                    .value(`TỔNG SỐ KHÁCH HÀNG ${yearFilter}`)
                  var j = 3
                  ...
                  listDataContact?.forEach((item, index) => {
                    for (var m = 1; m <= 12; m++) {
                      if (Number(item._id.month) === Number(m)) {
                        for (
                          let j = 3;
                          j <= Number(listFundaIsMember.length + 2);
                          j++
                        ) {
                          let fundaID = workbook
                            .sheet(`TongSoKhachHang`)
                  ...
                  listData?.forEach((item, index) => {
                    ...
                    workbook
                      ...
                      .row(i)
                      .cell(5)
                      .value(`${item._id.customer.funda._id}`)
                    workbook
                      .sheet(`KhachHangQuayLai`)
                      .row(i)
                      .cell(6)
