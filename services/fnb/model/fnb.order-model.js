...
class Model extends BaseModel {
     ...
     * Code: 24/11/2022
     */
    insert({ voucherID, fundaID, parentID, customerID, affiliateID, shiftID, date, name, appOrderSign, note, sources, salesChannel, paymentMethod, service, total, discount, salesoff, credit, offer, vatAmount, shippingFee, paid, products, userID, ctx }) {
        const that = this
        return new Promise(async resolve => {
            try {
                if(!checkObjectIDs(fundaID) || !checkObjectIDs(customerID) || !checkObjectIDs(shiftID) || !name)
                    return resolve({ error: true, message: 'fundaID|name invalid', keyError: KEY_ERROR.PARAMS_INVALID });

                // Thông tin cơ sở
                let infoFunda = await ITEM__FUNDA_COLL.findById(fundaID)
                if (!infoFunda)
                    return resolve({ error: true, message: 'cannot_get_infoFunda' });

                // Thông tin ca làm việc
                let infoShift = await FNB_SHIFT_COLL.findById(shiftID)
                if (!infoShift)
                    return resolve({ error: true, message: 'cannot_get_infoShift' });

                // Thông tin khách hàng
                let infoCustomer = await ITEM__CONTACT_COLL.findById(customerID)
                if (!infoCustomer)
                    return resolve({ error: true, message: 'cannot_get_infoCustomer' })

                // Lấy số đơn hàng khách đã mua (thành công)
                let numberOD = await FNB_ORDER_COLL.count({ customer: customerID, status: 5 })

                /**
                 * Tính mã hóa đơn tự động theo hệ thống (từ 1->N)
                 */
                let numberOfOrder = await FNB_ORDER_COLL.count({ company: infoFunda.company })
                ...
                // Chiến dịch
                if(infoFunda.campaignName && checkObjectIDs(infoFunda.campaignName)){
                ...
                if(infoCustomer.birthday){
                    ...
                    dataInsert.age = Number(dateNow.getFullYear()) - Number(infoCustomer.birthday.getFullYear())
                    // console.log(Number(dateNow.getFullYear()) - Number(infoCustomer.birthday.getFullYear()))
                }

                // Người giới thiệu
                if(infoCustomer.referrer && checkObjectIDs(infoCustomer.referrer)){
                    dataInsert.referrer = infoCustomer.referrer;
                }

                // Phân loại chiến dịch
                if(infoCustomer.campaignType && checkObjectIDs(infoCustomer.campaignType)){
                    dataInsert.campaignType = infoCustomer.campaignType;
                }

                // Doanh số khách cũ, mới và khách vãng lai
                if((numberOD && numberOD > 0) || (infoCustomer.new && infoCustomer.new == 2)){
                ...
                if(infoCustomer.nonResident && !isNaN(infoCustomer.nonResident)){
                ...
                if(!isNaN(sources) && [1,2,3,4,5].includes(Number(sources))){
                    dataInsert.sources = Number(sources);
                }

                if(!isNaN(paymentMethod)&& [1,2].includes(Number(paymentMethod))){
                    dataInsert.paymentMethod = Number(paymentMethod);
                }

                if(!isNaN(salesChannel) && [1,2,3,4,5,6,7,8,9].includes(Number(salesChannel))){
                ...
                if(salesChannel == 1){
                     ...
                     */
                    let factor = 0
                    if (Number(infoCustomer.purchasedOffValue) < Number(FNB_FACTOR.credit.level2.purchasedOffValue)){
                        factor = Number(FNB_FACTOR.credit.level1.factor)
                    } else {
                        if (Number(infoCustomer.purchasedOffValue) < Number(FNB_FACTOR.credit.level3.purchasedOffValue)){
                            factor = Number(FNB_FACTOR.credit.level2.factor)
                        } else {
                            if (Number(infoCustomer.purchasedOffValue) < Number(FNB_FACTOR.credit.level4.purchasedOffValue)){
                    ...
                    if(voucherID && checkObjectIDs(voucherID)){
                        ...
                        */
                        ...
                        if (!infoVoucher)
                            return resolve({ error: true, message: 'cannot_get_infoVoucher' })

                        // Kiểm tra xem đúng VoucherID của customerID hay không
                        if(!infoVoucher.receivers.includes(customerID))
                            return resolve({ error: true, message: 'Voucher không áp dụng cho khách hàng này' })

                        // Kiểm tra thời hạn VoucherID
                        let currentTime = new Date();
                        let checkExpired = compareTwoTime(currentTime, infoVoucher.expired)
                        if(checkExpired == 1)
                            return resolve({ error: true, message: 'Voucher đã hết hạn' })

                        /**
                         * Kiểm tra giá trị đơn hàng xem có được áp dụng Voucher hay không
                         */
                        if( Number(total) < Number(infoVoucher.minOrderAmount))
                            return resolve({ error: true, message: `Đơn hàng không đủ điều kiện áp dụng (Giá trị phải >= ${infoVoucher.minOrderAmount})` })

                        /**
                         * Kiểm tra xem khách hàng đã sử dụng voucherID hay chưa
                         */
                        if(infoVoucher.buyers.includes(customerID))
                            return resolve({ error: true, message: 'Khách hàng đã sử dụng Voucher này' })
                        ...
                        if(Number(infoVoucher.salesoffAmount) > 0){
                            ...
                            offer = Number(Number(infoVoucher.salesoffRate) * Number(total)/100).toFixed(0)
                        }
                     ...
                     * ĐIỂM TRUNG THÀNH
                     */
                    loyaltyPoints = Number(Number(amount)*(Number(factor/100))).toFixed(0)
                ...
                if(!isNaN(vatAmount)){
                    ...
                    dataInsert.vatAmount = Number(amount*Number(FNB_FACTOR.vatRate/100)/(Number(1 + Number(FNB_FACTOR.vatRate/100)))).toFixed(0)
                }
                ...
                if(infoShift.staffs && infoShift.staffs.length){
                    ...
                    if(infoShift.subStaffs && infoShift.subStaffs.length){
                        ...
                        numberStaff = Number(numberStaff) + Number(infoShift.subStaffs.length)
                    }
                    ...
                    if(Number(numberStaff) != 0){
                        dataInsert.avgTotalPerStaff = Number(amount/Number(numberStaff)).toFixed(0)
                    }else{
                ...
                if (products && products.length){
                    ...
                    dataInsert.avgQuantityPerStaff = Number(totalQuantity/Number(numberStaff)).toFixed(2)
                }

                /**
                 * THÊM SIZE VÀO ĐƠN HÀNG VÀ CA LÀM VIỆC
                 */
                let totalSizeM = 0, totalSizeL = 0
                ...
                if(products && products.length){
                    for(const prodSize of products){
                        ...
                        // }
                        if(prodSize.size == 1){
                            totalSizeM = Number(totalSizeM) + Number(prodSize.quantity)
                        }
                        if(prodSize.size == 2){
                            totalSizeL = Number(totalSizeL) + Number(prodSize.quantity)
                        }
                ...
                if(products && products.length){
                    ...
                    const productsAsync = products.map(product => {
                        ...
                        if(product.variantsSelected && product.variantsSelected.length){
                            for(const subProd of product.variantsSelected){
                                // console.log(subProd)
                                // M-Off
                                if(Number(product.size) == 1 && Number(salesChannel) == 1){
                                    newUnitPrice = Number(newUnitPrice) + subProd.unitPrice
                                }

                                // M-App
                                if(Number(product.size) == 1 && Number(salesChannel) != 1){
                                    newUnitPrice = Number(newUnitPrice) + subProd.unitPrice2
                                }

                                // L-Off
                                if(Number(product.size) == 2 && Number(salesChannel) == 1){
                                    newUnitPrice = Number(newUnitPrice) + subProd.unitPrice3
                                }

                                // L-App
                                if(Number(product.size) == 2 && Number(salesChannel) != 1){
                                    newUnitPrice = Number(newUnitPrice) + subProd.unitPrice4
                                }
                            }
                        }
                 ...
                 */
                if(salesChannel == 1 && voucherID && checkObjectIDs(voucherID)){
                    //______Cập nhật thông tin Người sử dụng voucher
                    await FNB_ORDER_VOUCHER_MODEL.update({ userID, voucherID, buyers: [customerID] })
                }

                /**
                 * NGƯỜI GIỚI THIỆU
                 */
                if(salesChannel == 1 && numberOD == 0 && infoCustomer.referrer && checkObjectIDs(infoCustomer.referrer)){

                    // Thông tin người giới thiệu
                    let infoReferral = await ITEM__CONTACT_COLL.findById(infoCustomer.referrer)

                    // Kiểm tra xem Người giới thiệu đã được nhận Voucher khi Bạn của mình mua hàng hay chưa (tránh trường hợp đơn hủy)
                    if (infoReferral && infoReferral.achievements && infoReferral.achievements.includes(customerID)){
                        ...
                        */
                        ...
                        if(infoOffer1){
                            ...
                            await ITEM__CONTACT_COLL.findByIdAndUpdate(infoCustomer.referrer, {
                            ...
                            await ctx.call(`${CF_DOMAIN_SERVICES.ZALO}.${CF_ACTIONS_ZALO.ZALO_SEND_MESSAGE_ZNS}`, dataOA)
                            ...
                            await Promise.all([
                                ctx.call(`${CF_DOMAIN_SERVICES.ZALO}.${CF_ACTIONS_ZALO.ZALO_SEND_MESSAGE_ZNS}`, dataOA2),
                                ctx.call(`${CF_DOMAIN_SERVICES.ZALO}.${CF_ACTIONS_ZALO.ZALO_SEND_MESSAGE_ZNS}`, dataOA3)
                            ])
                        }
                 ...
                 */
                if([1].includes(Number(salesChannel)) && Number(total) >= 300000){
                    /**
                    ...
                    let infoConfig = await ITEM__CONFIG_COLL.findOne({ company: infoFunda.company, type: 1, active: 1 })
                    if(infoConfig){
                        ...
                        await ctx.call(`${CF_DOMAIN_SERVICES.ZALO}.${CF_ACTIONS_ZALO.ZALO_SEND_MESSAGE_ZNS}`, dataOA1)
                        ...
                        await ctx.call(`${CF_DOMAIN_SERVICES.ZALO}.${CF_ACTIONS_ZALO.ZALO_SEND_MESSAGE_ZNS}`, dataOA2)
                    }
                }
                 ...
                 */
                 if(affiliateID && checkObjectIDs(affiliateID)){
                    let infoAffiliate = await ITEM__CONTACT_COLL.findById(affiliateID)
                    ...
                    await FNB_NETWORK_COM_MODEL.insert({
                        ...
                        commission: Number(FNB_AFFILIATE_COM[0].rate) * amount/100,
                    })
                    ...
                    for(const item of infoAffiliate.nestedParents){
                        ...
                        let infoNetworkCom = await FNB_NETWORK_COM_MODEL.insert({
                            ...
                            rate: FNB_AFFILIATE_COM[Number(networkLevel)].rate,
                            commission: Number(FNB_AFFILIATE_COM[Number(networkLevel)].rate) * amount/100,
                        })
     ...
     * Code: 2/2/2024
     */
    insert2({ voucherID, fundaID, parentID, customerID, businessID, channelID, affiliateID, shiftID, date, name, appOrderSign, note, sources, salesChannel, paymentMethod, service, total, discount, salesoff, credit, offer, vatAmount, shippingFee, paid, products, userID, ctx }) {
        console.log({ voucherID, fundaID, parentID, customerID, businessID, channelID, affiliateID, shiftID, date, name, appOrderSign, note, sources, salesChannel, paymentMethod, service, total, discount, salesoff, credit, offer, vatAmount, shippingFee, paid, products, userID })
        const that = this
        return new Promise(async resolve => {
            try {
                if(!name || !checkObjectIDs(fundaID) || !checkObjectIDs(customerID) | !checkObjectIDs(businessID) || !checkObjectIDs(channelID) || !checkObjectIDs(shiftID))
                    return resolve({ error: true, message: 'fundaID|customerID|businessID|channelID|shiftID|name invalid', keyError: KEY_ERROR.PARAMS_INVALID });
    ...
    update({ orderID, userID,
        customerID, cancelNote, shippingFeeTotal, shippingFee, paid, filesID, status, paymentMethod, salesChannel, starRating, complaintID, complaint }) {
        ...
        return new Promise(async resolve => {
            try {
                ...
                if(customerID && checkObjectIDs(customerID)){
                    let infoCustomer = await ITEM__CONTACT_COLL.findById(customerID)
                    if(!infoCustomer)
                        return resolve({ error: true, message: 'customerID invalid', keyError: KEY_ERROR.PARAMS_INVALID })
                    ...
                    if(infoCustomer.birthday){
                        let dateNow = new Date
                        dataUpdate.age = Number(dateNow.getFullYear()) - Number(infoCustomer.birthday.getFullYear())
                        // console.log(Number(dateNow.getFullYear()) - Number(infoCustomer.birthday.getFullYear()))
                    }
                    ...
                    let numberOD = await FNB_ORDER_COLL.count({customer: customerID})
                    if((numberOD && numberOD > 0) || (infoCustomer.new && infoCustomer.new == 2)){
                    ...
                    // Khách vãng lai hay không
                    if(infoCustomer.nonResident && !isNaN(infoCustomer.nonResident)){
                        dataUpdate.nonResident = Number(infoCustomer.nonResident);
                ...
                let infoAfterUpdate = await FNB_ORDER_COLL.findByIdAndUpdate(orderID, dataUpdate, { new: true })
                if (!infoAfterUpdate)
                ...
                if(!isNaN(status) || !isNaN(paymentMethod) || !isNaN(salesChannel)){
                ...
                if(customerID && checkObjectIDs(customerID)){
                    ...
                    await that.updateCustomer({ customerID: oldCustomerID, userID })
        ...
        return new Promise(async resolve => {
            try {
                if(!checkObjectIDs(shiftID))
                    return resolve({ error: true, message: 'shiftID invalid', keyError: KEY_ERROR.PARAMS_INVALID })
                ...
                let listData = await FNB_ORDER_COLL.aggregate([
                    ...
                    {
                        $group: {
                            _id: { salesChannel: "$salesChannel", paymentMethod: "$paymentMethod", status: "$status" },
                            quantity: { $sum: 1 },
                ...
                if(listData && listData.length){
                    for(const item of listData){
                        ...
                        if(item._id.status && item._id.status == 5){
                            // console.log('---------------------->>>>>>>>>>>>>>>>>>')
                            numberOfOrders = Number(numberOfOrders) + Number(item.quantity)

                            // Tiền mặt và chuyển khoản
                            if(item._id.paymentMethod && item._id.paymentMethod == 1){
                                // console.log('==========TIỀN MẶT============')
                                cashAmount = Number(cashAmount) + Number(item.amount)

                                console.log({cashAmount})
                            }else{
                                // console.log('==========CHUYỂN KHOẢN============')
                                transferAmount = Number(transferAmount) + Number(item.amount)
                            }
                            ...
                            if(item._id.salesChannel){
                                ...
                                if(item._id.salesChannel && item._id.salesChannel == 1){
                                    numberOfOrders1 = Number(numberOfOrders1) + Number(item.quantity)
                                    total1 = Number(total1) + Number(item.total)
                                }

                                if(item._id.salesChannel && item._id.salesChannel == 2){
                                    numberOfOrders2 = Number(numberOfOrders2) + Number(item.quantity)
                                    total2 = Number(total2) + Number(item.total)
                                }

                                if(item._id.salesChannel && item._id.salesChannel == 3){
                                    numberOfOrders3 = Number(numberOfOrders3) + Number(item.quantity)
                                    total3 = Number(total3) + Number(item.total)
                                }

                                if(item._id.salesChannel && item._id.salesChannel == 4){
                                    numberOfOrders4 = Number(numberOfOrders4) + Number(item.quantity)
                                    total4 = Number(total4) + Number(item.total)
                                }

                                if(item._id.salesChannel && item._id.salesChannel == 5){
                                    numberOfOrders5 = Number(numberOfOrders5) + Number(item.quantity)
                                    total5 = Number(total5) + Number(item.total)
                                }

                                if(item._id.salesChannel && item._id.salesChannel == 6){
                                    numberOfOrders6 = Number(numberOfOrders6) + Number(item.quantity)
                                    total6 = Number(total6) + Number(item.total)
                                }

                                if(item._id.salesChannel && item._id.salesChannel == 7){
                                    numberOfOrders7 = Number(numberOfOrders7) + Number(item.quantity)
                                    total7 = Number(total7) + Number(item.total)
                                }

                                if(item._id.salesChannel && item._id.salesChannel == 8){
                                    numberOfOrders8 = Number(numberOfOrders8) + Number(item.quantity)
                                    total8 = Number(total8) + Number(item.total)
                                }

                                if(item._id.salesChannel && item._id.salesChannel == 9){
                                    numberOfOrders9 = Number(numberOfOrders9) + Number(item.quantity)
                        ...
                        if(item._id.status && item._id.status == 4){
                            numberOfCancelOrders = Number(numberOfCancelOrders) + Number(item.quantity)
                            totalOfCancelOrders = Number(totalOfCancelOrders) + Number(item.total)
                        }
                    }
                }
                ...
                let listOrders = await FNB_ORDER_COLL.find({ shift: shiftID, status: 5 })
                ...
                if(listData1 && listData1.length){
                    for(const item of listData1){
                        if(item._id.size && item._id.size == 1){
                            numberOfSizeM = Number(numberOfSizeM) + Number(item.quantity)
                        }else{
                            numberOfSizeL = Number(numberOfSizeL) + Number(item.quantity)
                        }
                    }
                }
                ...
                let infoAfterUpdate = await FNB_SHIFT_COLL.findByIdAndUpdate(shiftID, dataUpdate, { new: true });
                if (!infoAfterUpdate)
                    return resolve({ error: true, message: 'Cập nhật thất bại', keyError: KEY_ERROR.UPDATE_FAILED });
        ...
        return new Promise(async resolve => {
            try {
                ...
                if(listData3 && listData3.length){
                    ...
                    totalLoyaltyPoints = Number(totalLoyaltyPoints) + Number(listData3[0].loyaltyPoints)
                ...
                if(listData4 && listData4.length){
                    totalLoyaltyPoints = Number(totalLoyaltyPoints) + Number(listData4[0].amount)
                }

                // Giá trị còn lại
                remainLoyaltyPoints = Number(totalLoyaltyPoints) - Number(usedLoyaltyPoints)
                ...
                let infoAfterUpdate = await ITEM__CONTACT_COLL.findByIdAndUpdate(customerID, dataUpdate, { new: true })
                if (!infoAfterUpdate)
                    return resolve({ error: true, message: 'Cập nhật thất bại', keyError: KEY_ERROR.UPDATE_FAILED })
     ...
     * Code: 24/11/2022
     */
    getList({ type, option, companyID, fundasID, shiftsID, assigneesID, shiftTypes, salesChannels, services, statuss, fromDate, toDate, isMistake, isDiscount, isSalesoff, isCredit, isOffer, isNonResident, isCustomerType, isComplaint, isCampaignType, isPaymentMethod, starRating, managerID, keyword, limit = 10, lastestID, select, populates, sortKey, userID, customerID, voucherID, productID, ctx }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(!checkObjectIDs(customerID) && !checkObjectIDs(voucherID) && !checkObjectIDs(productID)){
                    ...
                    if(customerID && checkObjectIDs(customerID)){
                        ...
                        if(productID && checkObjectIDs(productID)){
                            ...
                            conditionObj._id = {$in: listOP.map(item=>item.order)}
                            limit = 5
                        }
                    }
                }
                ...
                shiftsID && shiftsID.length                 && (conditionObj.shift = { $in: shiftsID });
                ...
                services && services.length                 && (conditionObj.service = { $in: services });
     ...
     * Code: 24/11/2022
     */
    getListByProperty({ companyID, fundasID, fundas, customerID, optionFixFunda, option, optionGroup, optionStatus, optionTime, year, month, fromDate, toDate, voucherType, userID, staffID, optionStar, duration, ctx }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(Number(duration/86400) > 370){
                    ...
                    let conditionGroup = {}
                    let conditionObjYear = {}, conditionPopulate = {}, sortBy = {"total": -1}
                    ...
                    if(optionFixFunda && Number(optionFixFunda) === 1){
                        if(fundas && Number(fundas.length) > 2){
                            ...
                            let arrFun2 = arr[1].split(" ").map(item=>ObjectID(item))
                    ...
                    if(option && Number(option) == 1){
                        conditionGroup = {
                            ...
                            _id: { new: "$new", internal: "$internal", salesChannel: "$salesChannel", paymentMethod: "$paymentMethod"},
                            quantity: { $sum: 1 },
                            total: { $sum: "$total" },
                            discount: { $sum: "$discount" },
                            salesoff: { $sum: "$salesoff" },
                    ...
                    if(!isNaN(option) && [3,6,7,8,9,10,11,13,14].includes(Number(option))){
                        await FNB_ORDER_COLL.populate(listData, conditionPopulate)
                    }
        ...
        return new Promise(async resolve => {
            try {
                ...
                if(1>0){
                    ...
                    let conditionGroup = {}
                    let conditionPopulate = {}, sortBy = {"_id.month": -1, "avgQuantityPerStaff": -1}

                    let conditionObjYear = {
                        "year": Number(yearFilter),
                        "month": {$in: [currentMonth - 2, currentMonth - 1, currentMonth]},
                    }

                    conditionGroup = {
                        _id: {  staff: "$staff._id", month: "$month", year: "$year" },
                        avgQuantityPerStaff: { $sum: "$avgQuantityPerStaff" },
                    }
                    ...
                    await FNB_MISTAKE_COLL.populate(listDataErr, conditionPopulateErr)
                    ...
                    // Modify the workbook.
                    XlsxPopulate.fromFileAsync(path.resolve(__dirname, ('../../../files/templates/excels/fnb_export_kpi.xlsx')))
                    .then(async workbook => {
                        var i = 3;
                        listDataR?.forEach((item, index) => {
                            workbook.sheet("DoanhSo").row(i).cell(1).value(Number(index+1));
                            workbook.sheet("DoanhSo").row(i).cell(2).value(item?._id?.staff.fullname);
                            workbook.sheet("DoanhSo").row(i).cell(3).value(`${item?._id.staff?.department?.name}`);
                            // workbook.sheet("DoanhSo").row(i).cell(5).value(item?.number);
                            // workbook.sheet("DoanhSo").row(i).cell(6).value(item?.avgTotalPerStaff);
                            workbook.sheet("DoanhSo").row(i).cell(4).value(Number(item?._id?.month));
                            workbook.sheet("DoanhSo").row(i).cell(5).value(item?.avgQuantityPerStaff != null ? Number(item?.avgQuantityPerStaff) : 0)
                        ...
                        listDataErr?.forEach((item, index) => {
                            workbook.sheet("Err").row(i).cell(1).value(Number(index+1));
                            workbook.sheet("Err").row(i).cell(2).value(item?._id?.executor.fullname);
                            workbook.sheet("Err").row(i).cell(3).value(`${item?._id.executor?.department?.name}`);
                            workbook.sheet("Err").row(i).cell(4).value(item?._id.month);
                            workbook.sheet("Err").row(i).cell(5).value(item?.number);
                            workbook.sheet("Err").row(i).cell(6).value(item?.amount);
     ...
     * Date: 4/2/2023
     */
    exportExcel2({ companyID, option, month, year, fromDate, toDate, filterParams, email, userID }) {
                ...
                if(FNB_ACC.cskh.includes(email.toString()) || FNB_ACC.man.includes(email.toString()) || FNB_ACC.taichinh.includes(email.toString())){
                    ...
                    if(!option || option == undefined){
                        ...
                        if(option == 1){
                            ...
                            let listFuda = await ITEM__FUNDA_COLL.find({members: {$in: [userID]}}).select("_id name sign").sort({_id: 1})
                            let fundasIDIsMember = listFuda.map(item=>ObjectID(item._id));
                            if(fundasIDIsMember.length) {
                                conditionObj.funda = { $in: fundasIDIsMember };
                            }else{
                                conditionObj.funda = { $in: [] }
                            ...
                            let conditionGroup = {}, conditionPopulate = {}

                            conditionGroup = {
                                _id: {  salesChannel: "$salesChannel", funda: "$funda", month: "$month" },
                                number: { $sum: 1 },
                                total: { $sum: "$total" },
                                discount: { $sum: "$discount" },
                                salesoff: { $sum: "$salesoff" },
                            ...
                            let conditionGroupDay = {
                                _id: {  salesChannel: "$salesChannel", day: "$day", month: "$month" },
                                number: { $sum: 1 },
                                total: { $sum: "$total" },
                                discount: { $sum: "$discount" },
                                salesoff: { $sum: "$salesoff" },
                            ...
                            // Modify the workbook.
                            XlsxPopulate.fromFileAsync(path.resolve(__dirname, ('../../../files/templates/excels/fnb_export_revenue_all.xlsx')))
                            .then(async workbook => {
                                for(var m = 1; m<=12; m++){
                                    var i = 3
                                ...
                                listData?.forEach((item, index) => {
                                    for(var m = 1; m<=12; m++){
                                        if(Number(item._id.month) === Number(m)){
                                            for(var i=3; i<=Number(listFuda.length + 2); i++){

                                                let discount = 0, salesoff = 0, credit = 0, amount = 0
                                                let fundaID = workbook.sheet(`T${m}`).row(i).cell(3).value()

                                                if((item._id.funda.toString() === fundaID.toString())){
                                                    for(var j=4; j<=10; j++){
                                                        if((Number(item._id.salesChannel) === Number(j-3))){
                                                            workbook.sheet(`T${m}`).row(i).cell(j).value(Number(item.total))
                                                        }
                                                    }
                                                }
                                            }
                                ...
                                for(var m = 1; m<=12; m++){
                                    for(var i=3; i<=Number(listFuda.length + 2); i++){
                                        ...
                                        listData?.forEach((item, index) => {
                                            if((item._id.funda.toString() === fundaID.toString()) && Number(item._id.month) === Number(m)){

                                                discount = Number(discount) + Number(item.discount)
                                                salesoff = Number(salesoff) + Number(item.salesoff)
                                                credit = Number(credit) + Number(item.credit)
                                                amount = Number(amount) + Number(item.amount)
                                            }
                                        })

                                        workbook.sheet(`T${m}`).row(i).cell(11).value(Number(discount))
                                        workbook.sheet(`T${m}`).row(i).cell(12).value(Number(salesoff))
                                        workbook.sheet(`T${m}`).row(i).cell(13).value(Number(credit))
                                        workbook.sheet(`T${m}`).row(i).cell(14).value(Number(amount))
                                    }
                                }


                                workbook.sheet(`BaoCaoNgay`).row(1).cell(2).value(Number(currentYear))
                                listData2?.forEach((item, index) => {

                                    for(var i = 3; i<=379; i++){

                                        let xlsDay = workbook.sheet(`BaoCaoNgay`).row(i).cell(3).value()
                                        let xlsMonth = workbook.sheet(`BaoCaoNgay`).row(i).cell(4).value()

                                        for(var j=5; j<=11; j++){
                                            if((Number(item._id.salesChannel) === Number(j-4)) && Number(item._id.day) === Number(xlsDay) && Number(item._id.month) === Number(xlsMonth)){
                                                workbook.sheet(`BaoCaoNgay`).row(i).cell(j).value(Number(item.total))
                                            }
                                        }
                                    }
                                })
                                ...
                                for(var i = 3; i<=379; i++){
                                    ...
                                    if(!isNaN(xlsDay)){
                                        ...
                                        listData2?.forEach((item, index) => {
                                            if(Number(item._id.day) === Number(xlsDay) && Number(item._id.month) === Number(xlsMonth)){
                                                discount = Number(discount) + Number(item.discount)
                                                salesoff = Number(salesoff) + Number(item.salesoff)
                                                credit = Number(credit) + Number(item.credit)
                                                amount = Number(amount) + Number(item.amount)
                                        ...
                                        workbook.sheet(`BaoCaoNgay`).row(i).cell(12).value(Number(discount))
                                        workbook.sheet(`BaoCaoNgay`).row(i).cell(13).value(Number(salesoff))
                                        workbook.sheet(`BaoCaoNgay`).row(i).cell(14).value(Number(credit))
                                        workbook.sheet(`BaoCaoNgay`).row(i).cell(15).value(Number(amount))
                                    }
                                }
                        ...
                        if(option == 2){
                            if(FNB_ACC.bod.includes(email.toString()) || FNB_ACC.cskh.includes(email.toString())){
                                 ...
                                 */
                                let conditionObj = { company:  ObjectID(companyID) }
                                let conditionContactObj = { company:  ObjectID(companyID) } //
                                ...
                                if(fundasID && fundasID.length ){
                                    listFundaIsMember = await ITEM__FUNDA_COLL.find({_id: {$in: fundasID}}).select('_id name')
                                    let arrFun = fundasID.map(item=>ObjectID(item))

                                    if(arrFun.length) {
                                        conditionObj.funda = { $in: arrFun }
                                        conditionContactObj.funda = { $in: arrFun }//
                                    }
                                }else{
                                    listFundaIsMember = await ITEM__FUNDA_COLL.find({members: {$in: [userID]}}).select('_id name')
                                    let fundasIDIsMember = listFundaIsMember.map(item=>ObjectID(item._id))

                                    if(fundasIDIsMember.length) {
                                        conditionObj.funda = { $in: fundasIDIsMember }
                                        conditionContactObj.funda = { $in: fundasIDIsMember }//
                                    }else{
                                        conditionObj.funda = { $in: [] }
                                ...
                                let conditionGroupContact = {
                                    _id: { year: "$year", month: "$month", funda: "$funda" },
                                    quantity: { $sum: 1 },
                                }
                                let listDataContact = await ITEM__CONTACT_COLL.aggregate([
                                    {
                                ...
                                // Modify the workbook.
                                XlsxPopulate.fromFileAsync(path.resolve(__dirname, ('../../../files/templates/excels/fnb_export_customer.xlsx')))
                                .then(async workbook => {
                                    // Tổng số khách hàng
                                    workbook.sheet(`TongSoKhachHang`).row(1).cell(1).value(`TỔNG SỐ KHÁCH HÀNG ${yearFilter}`)
                                    var j = 3
                                    listFundaIsMember?.forEach((item, index) => {
                                        workbook.sheet(`TongSoKhachHang`).row(1).cell(j).value(`${item._id}`)
                                        workbook.sheet(`TongSoKhachHang`).row(2).cell(j).value(item.name)
                                    ...
                                    listDataContact?.forEach((item, index) => {
                                        for(var m = 1; m<=12; m++){
                                            if(Number(item._id.month) === Number(m)){
                                                for(let j=3; j<=Number(listFundaIsMember.length +2); j++){
                                                    let fundaID = workbook.sheet(`TongSoKhachHang`).row(1).cell(j).value()

                                                    if((item._id.funda.toString() === fundaID.toString())){
                                                        workbook.sheet(`TongSoKhachHang`).row(Number(m+2)).cell(j).value(item.quantity)
                                                    }
                                                }
                                            }
                                        }
                                    ...
                                    // Khách hàng quay lại
                                    if(fromDate && toDate){
                                        workbook.sheet(`KhachHangQuayLai`).row(1).cell(1).value(`Từ ${moment(fromDate).format("DD/MM/YYYY")}-${moment(toDate).format("DD/MM/YYYY")}`)
                                    }

                                    var i = 3
                                    listData?.forEach((item, index) => {
                                        workbook.sheet(`KhachHangQuayLai`).row(i).cell(1).value(Number(index+1))
                                        workbook.sheet(`KhachHangQuayLai`).row(i).cell(2).value( `${item._id.customer.name}` )
                                        workbook.sheet(`KhachHangQuayLai`).row(i).cell(3).value(item._id.customer.phone)
                                        workbook.sheet(`KhachHangQuayLai`).row(i).cell(4).value(item._id.customer.funda.name)
                                        workbook.sheet(`KhachHangQuayLai`).row(i).cell(5).value(`${item._id.customer.funda._id}`)
                                        workbook.sheet(`KhachHangQuayLai`).row(i).cell(6).value(Number(item.number))
                                        workbook.sheet(`KhachHangQuayLai`).row(i).cell(7).value(Number(item.total))
                                        workbook.sheet(`KhachHangQuayLai`).row(i).cell(8).value(Number(item.amount))
                                        workbook.sheet(`KhachHangQuayLai`).row(i).cell(9).value(`${item._id.customer._id}`)
                                        if(item._id.customer.type && Number(item._id.customer.type) === 1){
                                            workbook.sheet("KhachHangQuayLai").row(i).cell(10).value('Nhân viên');
                                        }
                                        if(Number(item._id.customer.nonResident) === 2){
                                            workbook.sheet("KhachHangQuayLai").row(i).cell(11).value('Khách vãng lai');
                                        }
                        ...
                        if(option == 3){
                            ...
                            let sortBy = { "_id.funda": 1, "_id.salesChannel": 1 }
                            let conditionGroup = {}, conditionPopulate = {}

                            conditionGroup = {
                                _id: {  salesChannel: "$salesChannel", funda: "$funda" },
                                total: { $sum: "$total" },
                                amount: { $sum: "$amount" },
                            }
                            ...
                            // Modify the workbook.
                            XlsxPopulate.fromFileAsync(path.resolve(__dirname, ('../../../files/templates/excels/fnb_export_revenue_type.xlsx')))
                            .then(async workbook => {

                                if(fromDate && toDate){
                                    workbook.sheet(`PhanLoaiDoanhSo`).row(1).cell(1).value(`Từ ${fromDate} tới ${toDate}`)
                                }else{
                                    workbook.sheet(`PhanLoaiDoanhSo`).row(1).cell(1).value(`Từ ${fromDate} tới ${toDate}`)
                                }
                                ...
                                listData?.forEach((item, index) => {
                                    ...
                                    if(Number(item._id.salesChannel) === 1){
                                        str = 'Off'
                                    }else if(Number(item._id.salesChannel) === 2){
                                        str = 'Grab'
                                    }else if(Number(item._id.salesChannel) === 3){
                                        str = 'Shopee'
                                    }else if(Number(item._id.salesChannel) === 4){
                                        str = 'Gojek'
                                    }else if(Number(item._id.salesChannel) === 5){
                                        str = 'Baemin'
                                    }else if(Number(item._id.salesChannel) === 6){
                                        str = 'Loship'
                                    }else{
                                        str = 'Bee'
                                    }

                                    workbook.sheet(`PhanLoaiDoanhSo`).row(i).cell(1).value(Number(index + 1))
                                    workbook.sheet(`PhanLoaiDoanhSo`).row(i).cell(2).value(item._id.funda.name)
                                    workbook.sheet(`PhanLoaiDoanhSo`).row(i).cell(3).value(`${item._id.funda._id}`)
                                    workbook.sheet(`PhanLoaiDoanhSo`).row(i).cell(4).value(item._id.funda.sign)
                                    workbook.sheet(`PhanLoaiDoanhSo`).row(i).cell(5).value(Number(item._id.salesChannel))
                                    workbook.sheet(`PhanLoaiDoanhSo`).row(i).cell(6).value(str)
                                    workbook.sheet(`PhanLoaiDoanhSo`).row(i).cell(7).value(Number(item.total))
