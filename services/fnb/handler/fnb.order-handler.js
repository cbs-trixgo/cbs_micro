    ...
    getListByProperty: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler =
                    await FNB_ORDER_MODEL.getListByProperty({
                        companyID: company._id,
                        fundasID,
                        fundas,
                        customerID,
    ...
    importFromExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await FNB_ORDER_MODEL.importFromExcel({
                        option,
                        companyID: company._id,
                        dataImport,
                        userID,
    ...
    exportExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await FNB_ORDER_MODEL.exportExcel({
                        userID,
                        email,
                        companyID,
                        filterParams,
    ...
    exportExcel2: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await FNB_ORDER_MODEL.exportExcel2({
                        userID,
                        email,
                        companyID,
                        option,
    ...
    exportExcel3: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await FNB_ORDER_MODEL.exportExcel3({
                        userID,
                        email,
                        companyID,
                        filterParams,
    ...
    resetAllData: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params)
                const { _id: userID, email, company } = ctx.meta.infoUser
                let { option, password, fundaID, page, arrCustomers } =
                    ctx.params

                const resultAfterCallHandler =
                    await FNB_ORDER_MODEL.resetAllData({
                        option,
                        email,
                        password,
                        userID,
    ...
    convertAllData: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await FNB_ORDER_MODEL.convertAllData({
                        password,
                        option,
                        companyID,
                        page,
