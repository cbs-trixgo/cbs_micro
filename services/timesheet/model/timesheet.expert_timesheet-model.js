...
class Model extends BaseModel {
     ...
     * Code:
     */
    insert({ userCreate, companyID, parentID, name, date, type, note, hours, unitprice, projectID, assigneeID, shiftID, fundaID }) {
        ...
        return new Promise(async resolve => {
            try {
                ...
                let dataInsert = {
                    userCreate, name, admins: [userCreate], members: [userCreate]
                }
                ...
                if(checkObjectIDs(parentID)){
                    let infoParent = await TIMESHEET__EXPERT_TIMESHEET_COLL.findById(parentID)
                    .populate({
                        path: 'assignee',
                ...
                if(assigneeID){
                    ...
                    let infoAssignee = await USER_COLL.findById(assigneeID).select('department position')
                    if(infoAssignee){
     ...
     * Code:
     */
    update({ expertTimesheetID, projectID, name, type, assigneeID, hours, unitprice, status, approver, note, date, admins, members, userID, workID, subtype }){
        ...
        return new Promise(async (resolve) => {
            const that = this
            try {
                let infoTimeSheetID = await TIMESHEET__EXPERT_TIMESHEET_COLL.findById(expertTimesheetID);
                if(!infoTimeSheetID) {
                    return resolve({
                        error: true,
                ...
                if(infoTimeSheetID.parent){
                    ...
                    if(checkObjectIDs(workID)){
                        ...
                        let infoWork = await BUDGET__WORK_COLL.findById(workID).select('unitPrice')
                        if(infoWork){
                ...
                if(subtype && Number(subtype)  != Number(infoTimeSheetID.subtype)){
                    ...
                    if([2,3,4].includes(Number(subtype))){
                        ...
                        if(infoUser.contacts && infoUser.contacts.length){
                            for(const contact of infoUser.contacts){
                                if(contact.company.toString() === infoTimeSheetID.company.toString() && contact.sallaryBasic){
                                    // Tính toán cho 1 năm 365 ngày - 52 ngày nghỉ (tuần nghỉ 1 ngày)*8h. Tổng lương cơ bản * 12 tháng
                                    unitprice2New = Number(unitprice2New) + Number(contact.sallaryBasic*12/(8*(365-52)))
                                }
                            }
                        }
                ...
                let adminsID = [...infoTimeSheetID.admins?.map(item => item?.toString())]
                ...
                let infoAfterUpdate = await TIMESHEET__EXPERT_TIMESHEET_COLL.findByIdAndUpdate(expertTimesheetID, dataUpdate, { new: true });
                if(!infoAfterUpdate) {
                    return resolve({
                        error: true,
                ...
                if(infoAfterUpdate.parent){
                    await that.updateValue({ parentID: infoAfterUpdate.parent, userID })
                }
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let dataUpdate = {
                    ...
                    unitprice: (listData[0].hours != 0 ? Number(listData[0].amount / listData[0].hours) : 0)
                }
                // console.log(dataUpdate)

                let infoAfterUpdate = await TIMESHEET__EXPERT_TIMESHEET_COLL.findByIdAndUpdate(parentID, dataUpdate, { new: true })
                if(!infoAfterUpdate) {
                    return resolve({
                        error: true,
                        message: "Cannot update",
        ...
        return new Promise(async (resolve) => {
            try {
                if(!checkObjectIDs(expertTimesheetID))
                    return resolve({ error: true, message: 'Request params expertTimesheetID invalid', status: 400 });
                ...
                let infoAterGet = await TIMESHEET__EXPERT_TIMESHEET_COLL.findById(expertTimesheetID)
                .select(select)
                .populate(populates)
     ...
     */
    getList({
        sortOption, parentID, workID, userID,
        keyword, limit = 10, lastestID, select, populates= {}, option, month, year, assigneeID, humanID, companyID, type, subtype, ctx }) {
        ...
        return new Promise(async (resolve) => {
            try {
				...
				if(lastestID && checkObjectIDs(lastestID)){
					let infoData = await TIMESHEET__EXPERT_TIMESHEET_COLL.findById(lastestID);
					if(!infoData)
                ...
                let infoDataAfterGet = await TIMESHEET__EXPERT_TIMESHEET_COLL.find(conditionObj)
                    .select(select)
                    .limit(+limit + 1)
                    .sort(sortBy)
                    .populate(populates)
                ...
                let totalRecord = await TIMESHEET__EXPERT_TIMESHEET_COLL.count(conditionObjOrg);
     ...
     * Date: 21/9/2022
     */
    getListByProperty({ userID, option, year, month, companyID, assigneeID, projectID, ctx }){
        ...
        return new Promise(async resolve => {
            try {
                ...
                // Báo cáo chấm công theo ngày
                    ...
                    let numberUser = await USER_COLL.count({company: companyID})
                    let numberToday = 0
                    let listData = await TIMESHEET__EXPERT_TIMESHEET_COLL.aggregate([
                        {
                            $match: conditionObj
                ...
                // Danh sách user chấm công/chưa chấm ngày nay
                    ...
                    let listData2 = await USER_COLL.find({company: companyID, _id: {$nin: listData1.map(item=>item._id.assignee)}})
                    ...
                    if(listData1.length){
                        await TIMESHEET__EXPERT_TIMESHEET_COLL.populate(listData1, conditionPopulateUser)
                    }
                ...
                // Góm nhóm theo ngân sách
                    ...
                    if(listData.length){
                        await TIMESHEET__EXPERT_TIMESHEET_COLL.populate(listData, conditionPopulate)
                    }
