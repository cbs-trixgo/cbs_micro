...
class Model extends BaseModel {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (assigneeID) {
          ...
          let infoAssignee = await USER_COLL.findById(assigneeID).select(
            'department position'
          )
          if (infoAssignee) {
  ...
  update({
    ...
    return new Promise(async (resolve) => {
      ...
      try {
        let infoTimeSheetID =
          await TIMESHEET__EXPERT_TIMESHEET_COLL.findById(expertTimesheetID)
        if (!infoTimeSheetID) {
          return resolve({
            error: true,
        ...
        if (infoTimeSheetID.parent) {
          ...
          if (checkObjectIDs(workID)) {
            ...
            let infoWork =
              await BUDGET__WORK_COLL.findById(workID).select('unitPrice')
            if (infoWork) {
              ...
              for (const contact of infoUser.contacts) {
                if (
                  ...
                  unitprice2New =
                    ...
                    Number((contact.sallaryBasic * 12) / (8 * (365 - 52)))
                }
              }
            }
  ...
  getInfo({ expertTimesheetID, userID, select, populates, ctx }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAterGet = await TIMESHEET__EXPERT_TIMESHEET_COLL.findById(
          expertTimesheetID
        )
          .select(select)
          .populate(populates)
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (lastestID && checkObjectIDs(lastestID)) {
          let infoData =
            await TIMESHEET__EXPERT_TIMESHEET_COLL.findById(lastestID)
          if (!infoData)
            return resolve({
              error: true,
        ...
        let infoDataAfterGet = await TIMESHEET__EXPERT_TIMESHEET_COLL.find(
          conditionObj
        )
          .select(select)
          .limit(+limit + 1)
        ...
        let totalRecord =
          await TIMESHEET__EXPERT_TIMESHEET_COLL.count(conditionObjOrg)
