 ...
 * MODELS
 */
const BIDDING__BILL_PRODUCT_MODEL              = require('../model/bidding.bill_product-model').MODEL;
...
module.exports = {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await BIDDING__BILL_PRODUCT_MODEL.insert({
                    userID, ctx, docID, productID, type, unitprice
                });
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { docProductID, projectID, docID, keyword, limit, lastestID, populates, select, sortKey } = ctx.params;
                ...
                if(docProductID){
                    resultAfterCallHandler = await BIDDING__BILL_PRODUCT_MODEL.getInfo({
                        docProductID, userID, select, populates
                    });
                }else{
                    resultAfterCallHandler = await BIDDING__BILL_PRODUCT_MODEL.getList({
                        projectID, docID, userID, keyword, limit, lastestID, select, populates, sortKey
                    });
                }
                return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx });
            } catch (error) {
