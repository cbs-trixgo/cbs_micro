 ...
 * MODELS
 */
const BIDDING__BILL_WORK_MODEL              = require('../model/bidding.bill_work-model').MODEL;
...
module.exports = {
    ...
    insert: {
        ...
        async handler(ctx) {
            try {
                ...
                let { groupID, datahubJobID, name, sign, unit, description, note, plus, quantity, quantity2, unitprice1, unitprice2, unitprice3, unitprice, amount } = ctx.params;

                let { _id: userID, company } = ctx.meta.infoUser;

                const resultAfterCallHandler = await BIDDING__BILL_WORK_MODEL.insert({
                    userID, ctx, groupID, datahubJobID, name, sign, unit, description, note, plus, quantity, quantity2, unitprice1, unitprice2, unitprice3, unitprice, amount
                });
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                const { 
                    workID, datahubJobID, name, sign, unit, description, note, plus, quantity, quantity2, unitprice1, unitprice2, unitprice3, unitprice, amount
                } = ctx.params;
                const { _id: userID } = ctx.meta.infoUser;

                const resultAfterCallHandler = await BIDDING__BILL_WORK_MODEL.update({
                    userID, ctx, workID, datahubJobID, name, sign, unit, description, note, plus, quantity, quantity2, unitprice1, unitprice2, unitprice3, unitprice, amount
                });
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { workID, groupID, keyword, limit, lastestID, populates, select, sortKey } = ctx.params;
                ...
                if(workID){
                    ...
                    resultAfterCallHandler = await BIDDING__BILL_WORK_MODEL.getList({
                        groupID, keyword, limit, lastestID, select, populates, sortKey
                    });
                }
                return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx });
            } catch (error) {
    ...
    assignJobInTemplate: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await BIDDING__BILL_WORK_MODEL.assignJobInTemplate({
                    datahubJobID, workID, userID, ctx
                });
     ...
     updateProductPrice: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                const { 
                    docID, productID, unitprice
                } = ctx.params;
                const { _id: userID } = ctx.meta.infoUser;

                console.log({docID, productID, unitprice})

                const resultAfterCallHandler = await BIDDING__BILL_WORK_MODEL.updateProductPrice({
                    userID, ctx, docID, productID, unitprice
                });
