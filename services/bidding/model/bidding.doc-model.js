...
class Model extends BaseModel {
     ...
     * Date: 30/4/2022
     */
    insert({ userID, planID, parentID, name, sign, note, releaseTime, closingTime, amount, companyOfAssigneeID, assigneeID, agreeStatus, bidPrice, bidPriceDiscount, bidPriceAdjustment, openedStatus, bidderID, tenderPrice, vatTenderPrice, factor1, factor2, factor3, factor4, factor5, vat, vatValue, ctx }) {
        const that = this
        return new Promise(async (resolve) => {
            try {
                if(!checkObjectIDs(userID) || !checkObjectIDs(planID))
                    return resolve({ error: true, message: 'Request params planID|userID invalid', status: 400 })
                
                // Thông tin Kế hoạch
                let infoPlan = await BIDDING__PLAN_COLL.findById(planID)

                if (!infoPlan)
                    return resolve({ error: true, message: 'Can not get infoPlan', status: 400 })
                ...
                // Tạo hồ sơ dự thầu
                if (checkObjectIDs(parentID) && checkObjectIDs(assigneeID) && checkObjectIDs(companyOfAssigneeID)) {
     ...
     * Date: 30/4/2022
     */
    update({ docID, userID, ctx, name, sign, note, releaseTime, closingTime, amount, agreeStatus, bidPrice, bidPriceDiscount, bidPriceAdjustment, openedStatus, bidderID, tenderPrice, vatTenderPrice, timeApproved, factor1, factor2, factor3, factor4, factor5, vat, vatValue }) {
        const that = this
        return new Promise(async (resolve) => {
            try {
                /**
                ...
                if (!infoDoc)
                    return resolve({ error: true, message: 'Can not get infoDoc', status: 400 })
                ...
                let infoAfterUpdate = await BIDDING__DOC_COLL.findByIdAndUpdate(docID, dataUpdate, { new: true })
                if (!infoAfterUpdate)
     ...
     * Date: 30/4/2022
     */
    getList({ clientID, projectID, planID, parentID, bidderID, contractID,
