...
class Model extends BaseModel {
    ...
    insert({ workID, productID, type, quantity, unitprice, note, userID }) {
        ...
        return new Promise(async resolve => {
            try {
                if(!_isValid(workID) || !_isValid(productID) || !_isValid(userID))
                    return resolve({ error: true, message: 'param_not_valid' })
                
                // Kiểm tra sự tồn tại của product trong job
                let info = await BIDDING__BILL_WORKLINE_COLL.findOne({work: workID, product: productID})
                if(info){
                    return resolve({ error: true, message: 'data_product_exists_in_work' })
                }else{
                    ...
                    let dataInsert = {
                        ...
                        quantity: (quantity && !isNaN(quantity)) ? Number(quantity).toFixed(3) : 0,
    ...
    getInfo({ worklineID, select, populates }) {
        return new Promise(async resolve => {
            try {
                ...
                let info = await BIDDING__BILL_WORKLINE_COLL.findById(worklineID)
                    .select(select)
                    .populate(populates)
     ...
     * Date: 02/5/2022
     */
    getList({ productID, workID, groupID, itemID, docID, projectID,
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData = await BIDDING__BILL_WORKLINE_COLL.findById(lastestID);
                    if (!infoData)
                ...
                let infoDataAfterGet = await BIDDING__BILL_WORKLINE_COLL.find(conditionObj)
                    .limit(limit + 1)
                    .sort(sortBy)
                    .select(select)
                    .populate(populates)
                ...
                let totalRecord = await BIDDING__BILL_WORKLINE_COLL.count(conditionObjOrg);
