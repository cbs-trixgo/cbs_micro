    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await BIDDING__REQUEST_COLL.findByIdAndUpdate(
                        requestID,
                        dataUpdate,
                        { new: true }
                    )
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await BIDDING__REQUEST_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await BIDDING__REQUEST_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await BIDDING__REQUEST_COLL.count(conditionObjOrg)
