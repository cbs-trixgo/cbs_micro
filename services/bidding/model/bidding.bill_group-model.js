        ...
        return new Promise(async (resolve) => {
            try {
                if (!name || !checkObjectIDs(userID) || !checkObjectIDs(itemID))
                    return resolve({
                        error: true,
                        message: 'Request params name|userID|itemID invalid',
                        status: 400,
                    })
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await BIDDING__BILL_GROUP_COLL.findByIdAndUpdate(
                        groupID,
                        dataUpdate,
                        { new: true }
                    )
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await BIDDING__BILL_GROUP_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await BIDDING__BILL_GROUP_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await BIDDING__BILL_GROUP_COLL.count(conditionObjOrg)
