    ...
    insert({ docID, productID, type, unitprice, userID }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let info = await BIDDING__BILL_PRODUCT_COLL.findOne({
                    doc: docID,
                    product: productID,
                })
                if (info) {
                    console.log(
                        'Product: Vật tư đã tồn tại in Package=========>>>>>>>>>>>>>'
                    )
                    return resolve({
                        error: true,
                        message: 'data_exists222222222222222',
                    })
                } else {
                    console.log(
                        'Product: Vật tư chưa tồn tại in Package=========>>>>>>>>>>>>>'
                    )
    ...
    update({ userID, docProductID, note, ctx }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await BIDDING__BILL_PRODUCT_COLL.findByIdAndUpdate(
                        docProductID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    getInfo({ docProductID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let info = await BIDDING__BILL_PRODUCT_COLL.findById(
                    docProductID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await BIDDING__BILL_PRODUCT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await BIDDING__BILL_PRODUCT_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await BIDDING__BILL_PRODUCT_COLL.count(conditionObjOrg)
