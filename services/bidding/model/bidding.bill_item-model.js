        ...
        return new Promise(async (resolve) => {
            try {
                if (!name || !checkObjectIDs(userID) || !checkObjectIDs(docID))
                    return resolve({
                        error: true,
                        message: 'Request params name|userID|docID invalid',
                        status: 400,
                    })
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await BIDDING__BILL_ITEM_COLL.findByIdAndUpdate(
                        itemID,
                        dataUpdate,
                        { new: true }
                    )
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await BIDDING__BILL_ITEM_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await BIDDING__BILL_ITEM_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await BIDDING__BILL_ITEM_COLL.count(conditionObjOrg)
