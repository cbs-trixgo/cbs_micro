    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await BIDDING__QUOTATION_COLL.findByIdAndUpdate(
                        cashBookID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    getInfo({ cashBookID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoContractIPC = await BIDDING__QUOTATION_COLL.findById(
                    cashBookID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await BIDDING__QUOTATION_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await BIDDING__QUOTATION_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await BIDDING__QUOTATION_COLL.count(conditionObjOrg)
