  ...
  getListReaction({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoDataAfterGet = await MEDIA__REACTION_COLL.find(conditionObj)
          .limit(limit + 1)
          .sort(sortBy)
          .select(select)
          .populate(populates)
        ...
        if (isGetTotal) {
          const totalRecordLike = await MEDIA__REACTION_COLL.countDocuments({
            ...conditionObjOrg,
            type: 1,
          })
          const totalRecordHeart = await MEDIA__REACTION_COLL.countDocuments({
            ...conditionObjOrg,
            type: 2,
          })
          const totalRecordSmile = await MEDIA__REACTION_COLL.countDocuments({
            ...conditionObjOrg,
            type: 3,
          })
          const totalRecordSurprise = await MEDIA__REACTION_COLL.countDocuments(
            {
              ...conditionObjOrg,
              type: 4,
            }
          )
          const totalRecordSad = await MEDIA__REACTION_COLL.countDocuments({
            ...conditionObjOrg,
            type: 5,
          })
          const totalRecordAngry = await MEDIA__REACTION_COLL.countDocuments({
            ...conditionObjOrg,
            type: 6,
          })
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (isGetTotal) {
          ...
          const totalRecordSad = await MEDIA__FILE_REACTION_COLL.countDocuments(
            {
              ...conditionObjOrg,
              type: 5,
            }
