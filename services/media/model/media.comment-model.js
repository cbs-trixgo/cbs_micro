...
class Model extends BaseModel {
        ...
        return new Promise(async resolve => {
            try {
                ...
                if(arrReceiver && arrReceiver.length) {
                    const content = infoComment?.content?.length > 100 ? infoComment?.content?.substr(0, 100) : infoComment?.content;
        ...
        return new Promise(async resolve => {
            try {
                ...
                for (const comment of comments) {
                    const lastReactionComment = await MEDIA__COMMENT_REACTION_COLL
                        .find({ "comment.item": comment._id })
                        .select('type')
                        .sort({ _id: -1 })
                        .limit(3)
                        .lean();
                    ...
                    if(comment.lastestReplyID && typeof comment.lastestReplyID !== 'string') {
                        const lastReactionCommentReply = await MEDIA__COMMENT_REACTION_COLL
                            .find({ "comment.item": comment.lastestReplyID?._id })
                            .select('type')
                            .sort({ _id: -1 })
                            .limit(3)
                            .lean();

                        const infoReactionReply = await MEDIA__COMMENT_REACTION_COLL.findOne({
                ...
                return resolve({ error: false, data: dataResponse, status: 200 });
            } catch (error) {
                return resolve({ error: true, message: error.message, status: 500 });
            }
        })
     ...
     */
    insert({ mediaID, parentID, content, files, images, authorID, select, populates, bizfullname, ctx }) { 
        return new Promise(async (resolve) => {
            try {
                let dataInsert = {  
                    media: mediaID,
                ...
                infoAfterInsert = await MEDIA__COMMENT_COLL
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                */    
                ...
                infoAfterUpdate = await MEDIA__COMMENT_COLL
                    .findById(commentID)
                    .select(select)
                    .populate(populates)
                    .lean();
	...
	delete({ authorID, commentID }) {
        return new Promise(async resolve => {
            try {
                ...
                let infoComment = await MEDIA__COMMENT_COLL
                    .findOne({ _id: commentID, author: authorID })
                    .lean();

                if(!infoComment) {
                    return resolve({
                        error: true,
                        message: "Comment không tồn tại hoặc bạn không phải người tạo comment",
                        keyError: "comment_not_exists_or_you_are_not_author_of_comment",
                        status: 400
                    })
                }

				let infoCommentAfterDelete = await MEDIA__COMMENT_COLL.findByIdAndDelete(commentID);
	 ...
	 */
    getList({ mediaID, parentID, limit = 5, lastestID, select, populates, authorID, isLoadAll }) { 
        return new Promise(async (resolve) => {
            try {
                /**
                 * DECALARTION VARIABLE (1)
                ...
                if(isLoadAll) {
                    let listComments = await MEDIA__COMMENT_COLL
                        .find({})
                        .select('author')
                        .lean(); 

                    return resolve({ error: false, data: listComments, status: 200 });
                }
                ...
                let infoDataAfterGet = await MEDIA__COMMENT_COLL
                    .find(conditionObj)
                    .limit(limit + 1)
                    .sort(sortBy)
                    .select(select)
                ...
                let dataResponse = await this.responseDataComment({ authorID, comments: infoDataAfterGet });
     ...
     */
    insertCommentFile({ mediaID, fileID, parentID, content, files, images, select, populates, authorID, bizfullname, ctx }) { 
        return new Promise(async (resolve) => {
            try {
                /** 
                 * VALIDATION STEP (2)
                ...
                infoAfterInsert = await MEDIA__COMMENT_FILE_COLL
                    .findById(infoAfterInsert._id)
                    .select(select)
                    .populate(populates)
                    .lean();
     ...
     * Date: 22/02/2022
     */ 
    updateCommentFile({ commentID, authorID, content, files, images, select, populates }) {
        return new Promise(async (resolve) => {
            try {
               /**
                * DECALARTION VARIABLE (1)
                ...
                */    
                ...
                infoAfterUpdate = await MEDIA__COMMENT_FILE_COLL
                    .findById(commentID)
                    .select(select)
                    .populate(populates)
                    .lean();
	...
	deleteCommentFile({ authorID, commentID }) {
        return new Promise(async resolve => {
            try {
                ...
                let infoComment = await MEDIA__COMMENT_FILE_COLL
                    .findOne({ _id: commentID, author: authorID })
                    .lean();
				...
				let infoCommentAfterDelete = await MEDIA__COMMENT_FILE_COLL.findByIdAndDelete(commentID);
	 ...
	 */
    getListCommentFile({ mediaID, fileID, parentID, limit = 5, lastestID, select, populates, authorID }) { 
        return new Promise(async (resolve) => {
            try {
                /**
                 * DECALARTION VARIABLE (1)
                ...
                let infoDataAfterGet = await MEDIA__COMMENT_FILE_COLL
                    .find(conditionObj)
                    .limit(limit + 1)
                    .sort(sortBy)
                    .select(select)
