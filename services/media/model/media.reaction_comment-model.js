    ...
    reactionCommentMedia({ authorID, commentID, typeReaction }) {
        return new Promise(async (resolve) => {
            try {
                ...
                const infoComment =
                    await MEDIA__COMMENT_COLL.findById(commentID).lean()
                ...
                const infoReaction = await MEDIA__REACTION_COMMENT_COLL.findOne(
                    {
                ...
                if (infoReaction) {
                    const infoAfterDeleteReaction =
                        await MEDIA__REACTION_COMMENT_COLL.findByIdAndDelete(
                            infoReaction._id
                        )
    ...
    reactionCommentFile({ authorID, commentID, typeReaction }) {
        return new Promise(async (resolve) => {
            try {
                ...
                const infoComment =
                    await MEDIA__FILE_COMMENT_COLL.findById(commentID).lean()
                ...
                if (infoReaction) {
                    ...
                    if (infoReaction.type === typeReaction) {
                        await MEDIA__FILE_COMMENT_COLL.findByIdAndUpdate(
                            commentID,
                            {
                                $inc: {
                                    amountReaction: -1,
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await MEDIA__REACTION_COMMENT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await MEDIA__REACTION_COMMENT_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                const totalRecord =
                    await MEDIA__REACTION_COMMENT_COLL.count(conditionObjOrg)

                if (isGetTotal) {
                    const totalRecordLike =
                        await MEDIA__REACTION_COMMENT_COLL.countDocuments({
                            ...conditionObjOrg,
                            type: 1,
                        })
