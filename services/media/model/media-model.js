            ...
            try {
                ...
                if (!infoMedia) {
                    return resolve({
                        ...
                        keyError:
                            'media_not_exists_or_you_are_not_author_of_post',
                        status: 400,
                    })
                }
                ...
                if (background) {
                    ...
                    if (!infoFileCore.error) {
                        ...
                        if (infoAfterInserted && infoMedia.background) {
                            await MEDIA__FILE_COLL.findByIdAndDelete(
                                infoMedia.background
                            )
                        }
                ...
                switch (infoMedia.type) {
                    case 1:
                        if (
                            ...
                            infoMedia.company?.toString() !==
                                companyID?.toString()
                        ) {
                            hasPermission = false
                        } else if (
                            infoMedia.company?._id?.toString() !==
                            companyID?.toString()
                        ) {
                            hasPermission = false
                        }
                    ...
                    case 2:
                        ...
                        if (!infoProject.error && infoProject.status === 200) {
                            const membersID = infoProject.data?.members.map(
                                (item) => item.toString()
                            )
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (isSave) {
                    ...
                    infoMediaAfterInserted =
                        await MEDIA__SAVE_COLL.findOneAndUpdate(
                            conditionObj,
                            {
                                $set: {
                                    modifyAt: new Date(),
