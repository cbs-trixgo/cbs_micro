...
class Model extends BaseModel {
    ...
    getList({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!isFileOfProject && !isSearchFile) {
                    if (!companyID && !projectID && !groupID && !taskID) {
                        return resolve({
                            error: true,
                            message:
                                'Tham số companyID || projectID || groupID || taskID là cần thiết',
                            keyError: 'params_invalid',
                            status: 400,
                        })
                    }
                 ...
                 */
                if (isFileOfProject) {
                    const { error, data } = await ctx.call(
                ...
                if (isSearchFile) {
                    ...
                    const listGroupUserIsMember =
                        await PCM_PLAN_GROUP__COLL.find(conditionObjGroup)
                            .select('_id')
                            .lean()
                    ...
                    const groupsID = listGroupUserIsMember.map(
                        (item) => item._id
                    )
                    ...
                    const listTaskUserIsMember = await PCM_PLAN_TASK__COLL.find(
                        conditionObjTask
                    )
                        .select('_id project')
                        .lean()
                ...
                let infoDataAfterGet = await PCM_FILE__COLL.find(conditionObj)
                    .select(select)
                    .limit(limit + 1)
                    .sort(sortBy)
                    .populate(populates)
    ...
    getListByFilter({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let listGroups = await PCM_PLAN_GROUP__COLL.find({
                    members: { $in: [userID] },
                })
                ...
                const listTask = await PCM_PLAN_TASK__COLL.find(
                    conditionObjTask
                )
                    .select('_id project')
                    .lean()
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (projectID) {
                    ...
                    conditionObj.createAt = {
                        $gte: new Date(
                            moment(infoProject.data?.startTime || Date.now())
                                .startOf('day')
                                .format()
                        ),
                    }
                ...
                const dataFilesGroupByDate =
                    await PCM_FILE__COLL.aggregate(pipeline)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let conditionObj = {
                    ...
                    contract: {
                        $in: listContract.map((item) => ObjectID(item._id)),
                    },
                }
                ...
                const dataFilesGroupByContract =
                    await PCM_FILE__COLL.aggregate(pipeline)
    ...
    downloadFiles({ taskID }) {
        return new Promise(async (resolve) => {
            try {
                const filesTask = await PCM_FILE__COLL.find({ task: taskID })
                    .select('nameOrg path')
                    .lean()
                ...
                const pathDownload = path.join(
                    __dirname,
                    `../../../files/temporary_uploads/${taskID}`
                )
                const pathZip = path.join(
                    __dirname,
                    `../../../files/temporary_uploads/${taskID}.zip`
                )
                ...
                await compressing.zip
                    .compressDir(pathDownload, pathZip)
    ...
    downloadFilesV2({ taskID }) {
        return new Promise(async (resolve) => {
            try {
                const files = await PCM_FILE__COLL.find({ task: taskID })
                    .select('nameOrg path')
                    .lean()
                ...
                this.s3Object.listObjectsV2(
                    ...
                    async (err, data) => {
                        ...
                        for (const file of data.Contents) {
                            ...
                            const response = await this.s3Object
    ...
    downloadFilesInGroup({ groupID, userID }) {
        return new Promise(async (resolve) => {
            try {
                const group = await PCM_PLAN_GROUP__COLL.findOne({
                    _id: groupID,
                    $or: [
                        { author: userID },
                        { admins: { $in: [userID] } },
    ...
    downloadFilesRecursiveInGroup({ groupID, userID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                const groupsChild = await PCM_PLAN_GROUP__COLL.find({
                    parent: groupID,
                })
                    .select('name sign')
                    .lean()
                ...
                if (foldersChild.length) {
                    await PromisePool.for(foldersChild)
                        .withConcurrency(10)
                        .handleError((err) =>
                            console.log({
                                error: true,
                        ...
                        .process(async (childGroup) => {
                            ...
                            const { error, data } =
                                await this.createFolderTasksByGroup({
                            ...
                            return (
                                !error &&
                                (await PromisePool.for(data.tasks)
                                    .withConcurrency(10)
                                    .handleError((err) =>
                                        console.log({
                                            error: true,
                                    ...
                                    .process(async (task) => {
                                        const taskName = replaceSpaceWithChar(
                                            nonAccentVietnamese(
                                                task.namecv || task.name
                                            ),
                                            '_'
                                        )
                    ...
                    await PromisePool.for(data.tasks)
                        ...
                        .process((task) => {
                            const taskName = replaceSpaceWithChar(
                                nonAccentVietnamese(task.namecv || task.name),
                                '_'
                            )
    ...
    createFolderTasksByGroup({ groupID, key, bucket }) {
        return new Promise(async (resolve) => {
            const tasks = await PCM_PLAN_TASK__COLL.find({ group: groupID })
