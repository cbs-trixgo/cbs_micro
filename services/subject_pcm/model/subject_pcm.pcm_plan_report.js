        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!infoAfterInsert)
                    return resolve({
                        error: true,
                        message: KEY_ERROR.INSERT_FAILED,
            ...
            try {
                if (!option) {
                    ...
                    if (!checkObjectIDs(reportID))
                        return resolve({
                            error: true,
                            message: 'Request params reportID invalid',
                            status: 400,
                        })
                     ...
                     *  3.2: Operation database
                     */
                    const infoReport = await PCM_PLAN_REPORT_COLL.findById(
                        reportID
                    )
                        .select('author')
                        .lean()
                    ...
                    let infoAfterUpdate =
                        await PCM_PLAN_REPORT_COLL.findByIdAndUpdate(
                            reportID,
                            dataUpdate,
                            { new: true }
                        ).select(select)
                    ...
                    if (!infoAfterUpdate)
                        return resolve({
                            error: true,
                            message: "Can't update pcm plan report",
                    ...
                    for (const item of reportsID) {
                        // console.log(item)
                        let info = await PCM_PLAN_REPORT_COLL.findByIdAndUpdate(
                            item,
                            {
                                $addToSet: {
                                    subjects: taskID,
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!reportsID.length || !checkObjectIDs(reportsID))
                    return resolve({
                        error: true,
                        message: 'Request params reportsID invalid',
                        status: 400,
                    })
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await PCM_PLAN_REPORT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await PCM_PLAN_REPORT_COLL.find(
                    conditionObj
                )
                    .limit(+limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await PCM_PLAN_REPORT_COLL.count(conditionObjOrg)
                let nextCursor = null
    ...
    exportTaskReport({ reportID, userID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                const infoReport =
                    await PCM_PLAN_REPORT_COLL.findById(reportID).lean()

                if (!infoReport) {
                    return resolve({
                        error: true,
                        message:
                            'Report không tồn tại hoặc bạn không có quyền xem report',
                        keyError: 'report_not_exists',
                        status: 400,
                    })
                }

                const dataTaskByCondition =
                ...
                XlsxPopulate.fromFileAsync(
                    ...
                    workbook
                        ...
                        .row(1)
                        .cell(1)
                        .value(infoReport.name.toUpperCase())
                    workbook
                        .sheet('Data')
                        .row(2)
                        .cell(1)
