    ...
    return new Promise(async (resolve) => {
      try {
        if (!option) {
           ...
           *  3.2: Operation database
           */
          const infoReport = await PCM_PLAN_REPORT_COLL.findById(reportID)
            .select('author')
            .lean()
          ...
          let infoAfterUpdate = await PCM_PLAN_REPORT_COLL.findByIdAndUpdate(
            reportID,
            dataUpdate,
            { new: true }
          ).select(select)
  ...
  getList({
    ...
    return new Promise(async (resolve) => {
      try {
         ...
         */
        let conditionObj = {}
        conditionObj.$or = [{ author: userID }, { members: { $in: [userID] } }]
  ...
  exportTaskReport({ reportID, userID }) {
    return new Promise(async (resolve) => {
      try {
        ...
        const infoReport = await PCM_PLAN_REPORT_COLL.findById(reportID).lean()
