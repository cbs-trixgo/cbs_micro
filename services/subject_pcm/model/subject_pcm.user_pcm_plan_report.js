    ...
    insert({ userID, subjectID, role, author }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoPlanReport =
                    await PCM_PLAN_REPORT_COLL.findById(subjectID)
    ...
    update({ userSubjectID, role }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await USER_PCM_PLAN_REPORT_COLL.findByIdAndUpdate(
                        userSubjectID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    remove({ userSubjectID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAterRemove =
                    await USER_PCM_PLAN_REPORT_COLL.findByIdAndDelete(
                        userSubjectID
                    )
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await USER_PCM_PLAN_REPORT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
