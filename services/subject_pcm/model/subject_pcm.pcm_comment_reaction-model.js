...
class Model extends BaseModel {
	...
	reaction({ authorID, commentID, typeReaction }) {
		return new Promise(async resolve => {
			try {
				...
				const infoReaction = await PCM_COMMENT_REACTION_COLL
					.findOne({
						comment: commentID,
						author: authorID
					})
				...
				if(infoReaction){
					const infoAfterDeleteReaction = await PCM_COMMENT_REACTION_COLL.findByIdAndDelete(infoReaction._id);
     ...
     * Updated: 17/03/2022
	 */
	getListReaction({ authorID, commentID, lastestID, isGetTotal, type, limit = 15, select, populates }) {
		return new Promise(async (resolve) => {
            try {
                /**
                ...
                let infoDataAfterGet = await PCM_COMMENT_REACTION_COLL
                    .find(conditionObj)
                    .limit(limit + 1)
                    .sort(sortBy)
                    .select(select)
                ...
                const totalRecord = await PCM_COMMENT_REACTION_COLL.count(conditionObjOrg);

                if(isGetTotal) {
                    const totalRecordLike       = await PCM_COMMENT_REACTION_COLL.countDocuments({ ...conditionObjOrg, type: 1 });
                    const totalRecordHeart      = await PCM_COMMENT_REACTION_COLL.countDocuments({ ...conditionObjOrg, type: 2 });
                    const totalRecordSmile      = await PCM_COMMENT_REACTION_COLL.countDocuments({ ...conditionObjOrg, type: 3 });
                    const totalRecordSurprise   = await PCM_COMMENT_REACTION_COLL.countDocuments({ ...conditionObjOrg, type: 4 });
                    const totalRecordSad        = await PCM_COMMENT_REACTION_COLL.countDocuments({ ...conditionObjOrg, type: 5 });
                    const totalRecordAngry      = await PCM_COMMENT_REACTION_COLL.countDocuments({ ...conditionObjOrg, type: 6 });
