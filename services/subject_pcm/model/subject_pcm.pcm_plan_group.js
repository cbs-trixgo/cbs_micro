    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await PCM_PLAN_GROUP_COLL.findByIdAndUpdate(
                        groupID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    getList({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet = await PCM_PLAN_GROUP_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await PCM_PLAN_GROUP_COLL.count(conditionObjOrg)
            ...
            try {
                ...
                XlsxPopulate.fromFileAsync(
                    ...
                    listData?.forEach((item, index) => {
                        ...
                        for (
                            var i = 4;
                            i < infoProject.members?.length + 4;
                            i++
                        ) {
                            let userID = workbook
                                .sheet('NhomDuLieuCongViec')
