...
class Model extends BaseModel {
        ...
        return new Promise(async resolve => {
            try {
                if(!checkObjectIDs(projectID))
                    return resolve({ error: true, message: 'projectID__invalid', keyError: KEY_ERROR.PARAMS_INVALID });

                let checkExist = await PCM_PLAN_GROUP_COLL.findOne({ project: projectID, name, sign });
                if(checkExist)
                ...
                if(!infoProject)
                    return resolve({ error: true, message: "Thông tin dự án không tồn tại", keyError: KEY_ERROR.ITEM_EXISTED })
     ...
     * Code:
     */
    update({ groupID, name, sign, userID, parentID, contractID, image, members, membersRemove, admins, adminsRemove }) {
        ...
        return new Promise(async resolve => {
            try {
                ...
                if(!infoGroup)
                    return resolve({ error: true, message: 'groupID__invalid', keyError: KEY_ERROR.PARAMS_INVALID });

                let checkExist = await PCM_PLAN_GROUP_COLL.findOne({ project: infoGroup.project, _id: { $ne: groupID }, sign });
                if(checkExist)
                    return resolve({ error: true, message: "Tên và mã hiệu đã tồn tại", keyError: KEY_ERROR.ITEM_EXISTED });
                ...
                let infoAfterUpdate = await PCM_PLAN_GROUP_COLL.findByIdAndUpdate(groupID, dataUpdate, { new: true })
                if (!infoAfterUpdate)
     ...
     * Code  :
     */
    getList({ userID, parentID, projectID, lastestID, keyword, limit = 10, select, populates = {}, isShowAll, isShowParentAndChild }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(!checkObjectIDs(projectID))
                    return resolve({ error: true, message: "projectID_invalid", keyError: KEY_ERROR.PARAMS_INVALID });
                ...
                let infoDataAfterGet = await PCM_PLAN_GROUP_COLL
                    .find(conditionObj)
                    .limit(limit+1)
                    .sort(sortBy)
                    .select(select)
    ...
    downloadTemplateExcel({ option, projectID, userID }) {
        ...
        return new Promise(async resolve => {
            try {
                ...
                // Modify the workbook.
                XlsxPopulate.fromFileAsync(path.resolve(__dirname, ('../../../files/templates/excels/pcm_setup_template.xlsx')))
                .then(async workbook => {
                    var i = 4;
                    ...
                    var i = 4;
                    infoProject.members?.forEach((item, index) => {
                        workbook.sheet("NhomDuLieuCongViec").row(i).cell(1).value(Number(index+1))
                        workbook.sheet("NhomDuLieuCongViec").row(i).cell(2).value(item.fullname)
                        workbook.sheet("NhomDuLieuCongViec").row(i).cell(3).value(`${item._id}`)
                        i++
                    });

                    var j = 4;
                    listData?.forEach((item, index) => {
                        workbook.sheet("NhomDuLieuCongViec").row(2).cell(j).value(`${item._id}`)
                        workbook.sheet("NhomDuLieuCongViec").row(3).cell(j).value(item.name)

                        for (var i = 4; i < infoProject.members?.length + 4; i++) {
                           let userID = workbook.sheet("NhomDuLieuCongViec").row(i).cell(3).value()
