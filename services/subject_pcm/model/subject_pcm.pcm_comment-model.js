...
class Model extends BaseModel {
     ...
     * Updated: 18/05/2022
     */
    handleAddFiles({ infoTask, infoComment, infoProject, files, fileType, authorID, switchType = this.SWITCH_TYPE.TASK, companyOfAuthor, ctx }) {
            ...
            try {
                let { results, errors } = await PromisePool
                    ...
                    .process(async fileID => {
                        // File Cũ
                        const infoPcmFile = await PCM_FILE_COLL
                            .findOne({
                                file: fileID,
                ...
                if(fileType === 'image') {
                    infoTask.contract && await ctx.call(`${CF_DOMAIN_SERVICES.ITEM}.${CF_ACTIONS_ITEM.CONTRACT_UPDATE}`, {
                        contractID: infoTask.contract.toString(),
                        photos: files
                    })

                    infoTask.project && await ctx.call(`${CF_DOMAIN_SERVICES.ITEM}.${CF_ACTIONS_ITEM.DEPARTMENT_UPDATE}`, {
	 ...
	 */
    insertCommentTask({
        taskID, parentID, authorID, type, content, rawContent, files, images, receiversID, select, populates, bizfullname, companyOfAuthor, isOfficial = false, ctx
    }) {
            ...
            try {
                ...
                if(checkObjectIDs(receiversID)) {
                    ...
                    if(!checkObjectIDs([parentID])) {
                        ...
                        const receivers = infoTask.accessUsers?.filter(user => user.toString() !== authorID.toString())
                        dataInsert.receivers = receivers
                    } else {
                        // console.log('=================444444444444444444=================')
                        const commentParent = await PCM_COMMENT_COLL
                            .findById(parentID)
                ...
                if(images && images.length) {
                    ...
                    if(infoTask.contract) {
                        await ctx.call(`${CF_DOMAIN_SERVICES.ITEM}.${CF_ACTIONS_ITEM.CONTRACT_UPDATE}`, {
                            contractID: infoTask.contract.toString(),
                            photos: images
                        })
                    }
                ...
                if(Object.keys(dataUpdate).length) {
                    const infoAfterUpdate = await PCM_COMMENT_COLL.findByIdAndUpdate(infoAfterInsert._id, {
                        $set: dataUpdate
                    });
                ...
                const usersUnRead = dataInsert.receivers?.filter(item => item.toString() !== authorID.toString());
                ...
                if(arrReceiver && arrReceiver.length) {
                    const sendNotiAsync = arrReceiver.map(async receiverId => {
                        const amountNoti = await PCM_PLAN_TASK_MODEL.getAmountNotification({ userID: receiverId });
                ...
                infoAfterInsert = await PCM_COMMENT_COLL
	 ...
	 * Date: 18/05/2022
	 */
    insertCommentProject({ projectID, authorID, content, files, images, select, populates, bizfullname, ctx }) {
        return new Promise(async resolve => {
            try {
                 ...
                 */
                const dataInsert    = { type: 2, project: projectID, author: authorID };
                const dataUpdate    = {};
                let dataSend        = {};
                let webUrl          = '';
                let titleMSS        = '';
                ...
                const infoProject = await ctx.call(`${CF_DOMAIN_SERVICES.ITEM}.${CF_ACTIONS_ITEM.DEPARTMENT_GET_LIST}`, {
                ...
                let { name: projectName, sign: projectSign, members, userCreate } = infoProject.data;

                let listReceivers   = [...new Set(members)];
                let arrReceiver     = listReceivers.filter(receiverID => receiverID !== authorID.toString());
                ...
                if(arrReceiver && arrReceiver.length) {
                    const amountNoti = await PCM_PLAN_TASK_MODEL.getAmountNotification({ userID: authorID });
	 ...
	 */
    insert({
        taskID, projectID, parentID, authorID, type, content, rawContent, files, images, receiversID,
        select, populates, bizfullname, isOfficial, companyOfAuthor, switchType = this.SWITCH_TYPE.TASK, ctx
    }) {
        return new Promise(async (resolve) => {
            switch (switchType) {
                // Tạo comment cho Task
                case this.SWITCH_TYPE.TASK:
                    resolve(await this.insertCommentTask({
                        taskID, parentID, authorID, type, content, rawContent, files, images, receiversID, select, populates, bizfullname, isOfficial, companyOfAuthor, ctx
                    }))
                    break;

                // Tạo comment cho Project
                case this.SWITCH_TYPE.PROJECT:
                    resolve(await this.insertCommentProject({
                        projectID, authorID, content, files, images, select, populates, bizfullname, ctx
                    }))
                    break;
                default:
                    break;
	 ...
	 * Date: 22/02/2022
	 */
    updateCommentTask({ taskID, commentID, authorID, content, files, images, isOfficial, select, populates, companyOfAuthor, bizfullname, ctx }) {
        return new Promise(async (resolve) => {
            try {
               /**
                * DECALARTION VARIABLE (1)
                ...
                */
                ...
                let infoComment = await PCM_COMMENT_COLL
                    .findOne({
                        _id: commentID,
                        task: taskID
                    }).lean();
                ...
                if(!infoComment) {
                    return resolve({
                        error: true,
                        message: "Comment không tồn tại hoặc comment không thuộc task hiện tại",
                ...
                switch (infoComment.type) {
                    case 2: { // Bình luận
                        ...
                        if(checkObjectIDs(files)){
                            const result = await this.handleAddFiles({
                                infoTask, infoComment, files, fileType: 'file', authorID, ctx
                            })
                        ...
                        if(checkObjectIDs(images)){
                            const result = await this.handleAddFiles({
                                infoTask, infoComment, files: images, fileType: 'image', authorID, ctx
                            })
                    ...
                    case 1: { // Phản hồi liên quan (Hiện tại chỉ dùng case này)
                        dataAdditional = {
                            $addToSet: {}
                        };
                        ...
                        if(checkObjectIDs(files)){
                            const result = await this.handleAddFiles({
                                infoTask, infoComment, files, fileType: 'file', authorID, companyOfAuthor, ctx
                            })
                        ...
                        if(checkObjectIDs(images)){
                            const result = await this.handleAddFiles({
                                infoTask, infoComment, files: images, fileType: 'image', authorID, companyOfAuthor, ctx
                            })
                ...
                if(accessUsers && accessUsers.length) {
                    ...
                    let listReceivers = [...new Set([author._id?.toString(), assignee._id?.toString(), ...related])];
                ...
                if(arrReceiver && arrReceiver.length) {
                    ...
                    for (let i = 0; i < arrReceiver.length; i++) {
                        const dataAmountNoti = await PCM_PLAN_TASK_MODEL.getAmountNotification({
	 ...
	 * Date: 18/05/2022
	 */
    updateCommentProject({ projectID, commentID, authorID, files, images, select, populates, ctx }) {
        return new Promise(async (resolve) => {
            try {
               /**
                * VALIDATION STEP (2)
                ...
                */
                ...
                const infoComment = await PCM_COMMENT_COLL
                    .findOne({
                        _id: commentID,
                        project: projectID,
                        author: authorID
                ...
                if(!infoComment) {
                    return resolve({
                        error: true,
                        message: "Bình luận không tồn tại (hoặc bình luận không thuộc dự án hiện tại)",
                ...
                if(checkObjectIDs(files)){
                    const result = await this.handleAddFiles({
                        infoProject: infoProject.data, infoComment, switchType: this.SWITCH_TYPE.PROJECT,
                        files, fileType: 'file', authorID, ctx
                    })
                ...
                if(checkObjectIDs(images)){
                    const result = await this.handleAddFiles({
                        infoProject: infoProject.data, infoComment, switchType: this.SWITCH_TYPE.PROJECT,
                        files: images, fileType: 'image', authorID, ctx
                    })
                ...
                const infoAfterUpdate = await PCM_COMMENT_COLL
                    .findByIdAndUpdate(commentID, dataUpdate, { new: true })
                    .select(select)
                    .populate(populates)
                    .lean();
	 ...
	 * Date: 18/05/2022
	 */
    update({ taskID, projectID, commentID, authorID, content, files, images, isOfficial, select, populates, switchType = this.SWITCH_TYPE.TASK, companyOfAuthor, ctx }) {
        return new Promise(async resolve => {
            switch (switchType) {
                case this.SWITCH_TYPE.TASK:
                    resolve(await this.updateCommentTask({
                        taskID, commentID, authorID, content, files, images, isOfficial, select, populates, companyOfAuthor, ctx
                    }))
                    break;
                case this.SWITCH_TYPE.PROJECT:
                    resolve(await this.updateCommentProject({
                        projectID, commentID, authorID, files, images, select, populates, ctx
                    }))
                    break;
                default:
                    break;
    ...
    updateMarkResponse({ taskID, commentID, official }) {
        return new Promise(async resolve => {
            try {
                ...
                const infoAfterUpdated = await PCM_COMMENT_COLL.findByIdAndUpdate(commentID, dataUpdate, { new: true });
                if(!infoAfterUpdated) {
                    return resolve({
                        error: true,
                        message: 'Là lỗi của chúng tôi không phải của bạn, chúng tôi đang tìm cách khắc phục',
    ...
    getList({
        ...
        limit = 10, lastestID, sortKey, select, populates, authorID, isGetReaction = 'true', switchType
    }) {
            ...
            try {
                ...
                if(taskID){
                    ...
                    if(infoTask.subtype === 4 && infoTask?.openedStatus === 0) {
                        ...
                        if(authorID !== infoTask.assignee?.toString() || relatedArr.includes(authorID)){
                ...
                let infoDataAfterGet = await PCM_COMMENT_COLL
                    .find(conditionObj)
                    .limit(limit + 1)
                    .sort(sortBy)
                    .select(select)
                ...
                if(infoDataAfterGet && infoDataAfterGet.length && !contractID && !isHaveImage && isGetReaction === 'true') {
                    for (const comment of infoDataAfterGet) {
                        const lastReactionComment = await PCM_COMMENT_REACTION_COLL
                            .find({ comment: comment._id })
                            .select('type')
                            .sort({ _id: -1 })
                            .limit(3)
                            .lean();
                        ...
                        if(comment.lastestReplyID && typeof comment.lastestReplyID !== 'string') {
                            const lastReactionCommentReply = await PCM_COMMENT_REACTION_COLL
                                .find({ comment: comment.lastestReplyID?._id })
                                .select('type')
                                .sort({ _id: -1 })
                                .limit(3)
                                .lean();
                            ...
                            comment.lastestReplyID['lastReactionComment'] = lastReactionCommentReply;
                        }
	 ...
	 * Date: 22/02/2022
	 */
	searchCommentById({ taskID, commentID, projectID, switchType = 'TASK', select, populates, authorID }){
		return new Promise(async resolve => {
			try {
				...
				const infoComment = await PCM_COMMENT_COLL
					.findOne(conditionObjSearchMessage)
                    .select(select)
                    .populate(populates)
					.lean();
				...
				const listCommentsFront = await PCM_COMMENT_COLL
                    .find(conditionObjFrontList)
                    .limit(5)
				...
				const listCommentsBack = await PCM_COMMENT_COLL
                    .find(conditionObjBackList)
                    .limit(5)
                    .sort({ createAt: 1, _id: 1 })
                    .select(select)
                    .populate(populates)
				...
				const listComments = [...listCommentsFront.reverse(), infoComment, ...listCommentsBack];

                const totalRecordFront = await PCM_COMMENT_COLL.countDocuments(conditionObjFrontList);
                const totalRecordBack  = await PCM_COMMENT_COLL.countDocuments(conditionObjBackList);
	 ...
	 * Date: 19/8/2022
	 */
	searchCommentBySubject({ sortKey, fromDate, toDate, authorsID, companyOfAuthorsID, projectsID, official, userID, keyword, limit = 10, unlimit, lastestID, select, populates = {} }){
     ...
     * Date: 24/9/2022
     */
    getListByProperty({ userID, option, fromDate, toDate, isParent, isGroup, projectID }){
            ...
            try {
                ...
                if(option == 1){
                    ...
                    let listDataProject = await PCM_COMMENT_COLL.aggregate([
                        {
                            $match: {
                                _id: {$in: listComments.map(item=>item._id)}
                            }
                        },
                        {
                            $group: {
                    ...
                    let listDataProjectCompanyAuthor = await PCM_COMMENT_COLL.aggregate([
                        ...
                        {
                            $group: {
                                _id: { project: '$project', companyOfAuthor: '$companyOfAuthor' },
                                amount: { $sum: 1 },
                            }
                        },
                        { $sort: sortCompany},
                    ...
                    if(listDataProjectCompanyAuthor.length){
                        await DEPARTMENT_COLL.populate(listDataProjectCompanyAuthor, conditionPopulateCompany)
                    }
                    // console.log(listDataProjectCompanyAuthor)

                    return resolve({ error: false, data: { listDataProject, listDataProjectCompanyAuthor } })
                }
