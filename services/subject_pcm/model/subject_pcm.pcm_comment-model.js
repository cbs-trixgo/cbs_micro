                    ...
                    .process(async (fileID) => {
                        ...
                        if (!infoFileCore.error) {
                            ...
                            const infoAfterInserted =
                                await PCM_FILE_MODEL.insert(dataInsert)
                            return (
            ...
            try {
                ...
                if (checkObjectIDs(receiversID)) {
                    ...
                    if (!checkObjectIDs([parentID])) {
                        ...
                        const commentParent = await PCM_COMMENT_COLL.findById(
                            parentID
                        )
                ...
                if (Object.keys(dataUpdate).length) {
                    const infoAfterUpdate =
                        await PCM_COMMENT_COLL.findByIdAndUpdate(
                            infoAfterInsert._id,
                            {
                                $set: dataUpdate,
                            }
                ...
                if (arrReceiver && arrReceiver.length) {
                    const sendNotiAsync = arrReceiver.map(
                        async (receiverId) => {
                            const amountNoti =
                ...
                infoAfterInsert = await PCM_COMMENT_COLL.findById(
                    infoAfterInsert._id
                )
                    .select(select)
                    .populate(populates)
            ...
            try {
                ...
                if (!infoComment) {
                    return resolve({
                        ...
                        keyError:
                            'comment_not_exists_or_comment_not_belong_task',
                        status: 400,
                    })
                }
            ...
            try {
                ...
                if (!infoComment) {
                    return resolve({
                        ...
                        keyError:
                            'comment_not_exists_or_comment_not_belong_project',
                        status: 400,
                    })
                }
    ...
    updateMarkResponse({ taskID, commentID, official }) {
        return new Promise(async (resolve) => {
            try {
                ...
                const infoResponse =
                    await PCM_COMMENT_COLL.findById(commentID).lean()
                if (!infoResponse) {
                    return resolve({
                        error: true,
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (taskID) {
                    ...
                    if (
                        infoTask.subtype === 4 &&
                        infoTask?.openedStatus === 0
                    ) {
                        let relatedArr = infoTask.related.map((item) =>
                            item.toString()
                        )
                ...
                if (
                    ...
                    for (const comment of infoDataAfterGet) {
                        ...
                        const infoReaction =
                            await PCM_COMMENT_REACTION_COLL.findOne({
                                comment: comment._id,
                                author: authorID,
                            })
                                .select('type')
                        ...
                        if (
                            ...
                            comment.lastestReplyID['reaction'] =
                                infoReactionReply
                ...
                return resolve({
                    error: false,
                    data: {
                        listRecords: dataResponse.length
                            ? dataResponse
                            : infoDataAfterGet,
                        limit,
                        totalRecord,
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (option == 1) {
                    ...
                    let listDataProjectCompanyAuthor =
                        await PCM_COMMENT_COLL.aggregate([
                            {
                                $match: {
                                    _id: {
                                        $in: listComments.map(
