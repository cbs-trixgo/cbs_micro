  ...
  getDynamicReport: {
    ...
    async handler(ctx) {
      try {
        ...
        let resultAfterCallHandler = await PCM_PLAN_TASK_MODEL.getDynamicReport(
          {
            companyID,
            userID,
            reportType,
  ...
  getAmountTask: {
    ...
    async handler(ctx) {
      try {
        ...
        // Bước check quyền
        let resultAfterCallHandler = await PCM_PLAN_TASK_MODEL.getAmountTask({
          userID,
          ctx,
        })
        return renderStatusCodeAndResponse({
  ...
  getBadgeBidding: {
    ...
    async handler(ctx) {
      try {
        ...
        const { _id: userID } = ctx.meta.infoUser
        // Bước check quyền
        let resultAfterCallHandler = await PCM_PLAN_TASK_MODEL.getBadgeBidding({
          userID,
          ctx,
        })
        return renderStatusCodeAndResponse({
