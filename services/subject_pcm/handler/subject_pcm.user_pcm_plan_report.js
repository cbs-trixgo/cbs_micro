        ...
        async handler(ctx) {
            try {
                ...
                for (const userID of users) {
                    let infoAfterInsert =
                        await USER_PCM_PLAN_REPORT_MODEL.insert({
                            userID,
                            subjectID,
                            role: ROLE_VIEW,
                            author,
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                ...
                // Bước check quyền
                let resultAfterCallHandler =
                    await USER_PCM_PLAN_REPORT_MODEL.update({
                        userSubjectID,
                        role,
                    })
    ...
    remove: {
        ...
        async handler(ctx) {
            try {
                ...
                // Bước check quyền
                let resultAfterCallHandler =
                    await USER_PCM_PLAN_REPORT_MODEL.remove({
                        userSubjectID,
                    })
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                ...
                if (userSubjectID) {
                    resultAfterCallHandler =
                        await USER_PCM_PLAN_REPORT_MODEL.getInfo({
                            userSubjectID,
                            select,
                            populates,
                        })
                    ...
                    resultAfterCallHandler =
                        await USER_PCM_PLAN_REPORT_MODEL.getList({
                            keyword,
                            limit,
                            lastestID,
                            userID,
