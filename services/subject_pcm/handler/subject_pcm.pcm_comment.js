 ...
 * MODELS
 */
const PCM_COMMENT_MODEL =
  require('../model/subject_pcm.pcm_comment-model').MODEL
    ...
    async handler(ctx) {
      try {
        ...
        const resultAfterCallHandler =
          await PCM_COMMENT_MODEL.updateMarkResponse({
            taskID,
            commentID,
            official,
          })
  ...
  delete: {
    ...
    async handler(ctx) {
      try {
        ...
        const resultAfterCallHandler = await PCM_COMMENT_MODEL.delete({
          commentID,
          authorID,
        })
  ...
  searchCommentById: {
    ...
    async handler(ctx) {
      try {
        await this.validateEntity(ctx.params)
        const { _id: authorID } = ctx.meta.infoUser
        const { taskID, commentID, projectID, switchType, select, populates } =
          ctx.params

        const resultAfterCallHandler =
          await PCM_COMMENT_MODEL.searchCommentById({
            taskID,
            commentID,
            projectID,
            switchType,
  ...
  searchCommentBySubject: {
    ...
    async handler(ctx) {
      try {
        ...
        const resultAfterCallHandler =
          await PCM_COMMENT_MODEL.searchCommentBySubject({
            draft,
            sortKey,
            isParent,
            isActive,
