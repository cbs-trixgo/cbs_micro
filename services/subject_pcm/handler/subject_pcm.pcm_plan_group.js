 ...
 * MODELS
 */
const PCM_PLAN_GROUP_MODEL =
    require('../model/subject_pcm.pcm_plan_group').MODEL
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler =
                    await PCM_PLAN_GROUP_MODEL.removeMany({
                        groupsID,
                    })
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await PCM_PLAN_GROUP_MODEL.downloadTemplateExcel({
    ...
    importFromExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await PCM_PLAN_GROUP_MODEL.importFromExcel({
                        projectID,
                        dataImport,
                        userID,
                        ctx,
    ...
    exportExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler =
                    await PCM_PLAN_GROUP_MODEL.exportExcel({
                        userID,
                        email,
                        companyID,
                        option,
