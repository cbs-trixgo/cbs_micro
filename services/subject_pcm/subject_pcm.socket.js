...
exports.SUBJECT_SOCKET = {
     ...
     * Tạo mới việc con
     */
    [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_ADD_TASK]:
        async function (data) {
            try {
                let socket = this
    ...
    [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_ADD_CHECKLIST]:
        async function (data) {
            try {
                ...
                if (!taskID || !checklistID) {
                    return sendDataToMultilSocketsOfListUser({
                        ...
                        data: {
                            error: true,
                            message:
                                'Tham số taskID hoặc checklistID không hợp lệ',
                        },
                        event: [
                ...
                const infoCheckList = await PCM_PLAN_TASK_COLL.findById(
                    checklistID
                )
    ...
    [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_ADD_CHILD_TASK]:
        async function (data) {
            try {
                ...
                if (!taskID || !childTaskID) {
                    return sendDataToMultilSocketsOfListUser({
                        ...
                        data: {
                            error: true,
                            message:
                                'Tham số taskID hoặc childTaskID không hợp lệ',
                        },
                        event: [
                ...
                const infoChildTask = await PCM_PLAN_TASK_COLL.findById(
                    childTaskID
                )
                    .select('_id name assignee expiredTime parent status')
                    .populate('assignee', '_id bizfullname fullname image')
                ...
                if (!taskID || !relationUsersID || !relationUsersID.length) {
                    return sendDataToMultilSocketsOfListUser({
                        ...
                        data: {
                            error: true,
                            message:
                                'Tham số taskID hoặc relationUsersID không hợp lệ',
                        },
                        event: [
    ...
    [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_ADD_SIGNATURE]:
        async function (data) {
            ...
            try {
                ...
                const amountNoti =
                    await PCM_PLAN_TASK_MODEL.getAmountNotification({ userID })
    ...
    [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_OPEN_BIDDING]:
        async function (data) {
            ...
            try {
                ...
                const listReceivers = [
                    ...new Set([author.toString(), ...related]),
                ]
                const receivers = listReceivers.filter(
                    (receiverID) => receiverID !== userID.toString()
                )
