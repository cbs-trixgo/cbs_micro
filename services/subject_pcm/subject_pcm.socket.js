 ...
 */
const {
  EVENT_SOCKET_CONSTANT_SUBJECT_PCM,
} = require('./helper/subject_pcm.events-socket-constant')
...
exports.SUBJECT_SOCKET = {
   ...
   * Tạo mới việc con
   */
  [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_ADD_TASK]: async function (
    data
  ) {
    try {
      let socket = this
      ...
      if (!taskID) {
        return sendDataToMultilSocketsOfListUser({
          ...
          event: [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_SSC_ADD_TASK],
          io,
        })
      }
      ...
      const listReceivers = [
        ...new Set([author.toString(), assignee.toString(), ...related]),
      ]
      const receivers = listReceivers.filter(
        (receiverID) => receiverID !== userID.toString()
      )
      ...
      console.info(
        '======EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_SSC_ADD_TASK====='
      )
      console.error(error)
    }
  },
   ...
   * Tạo mới checklist
   */
  [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_ADD_CHECKLIST]:
    async function (data) {
      try {
        let socket = this
        let listUserActive = socket.$service.listUserConnected
        ...
        if (!taskID || !checklistID) {
          return sendDataToMultilSocketsOfListUser({
            ...
            data: {
              error: true,
              message: 'Tham số taskID hoặc checklistID không hợp lệ',
            },
            event: [
              EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_SSC_ADD_CHECKLIST,
            ],
            io,
          })
        }
        ...
        const infoCheckList = await PCM_PLAN_TASK_COLL.findById(checklistID)
        ...
        if (!infoCheckList || !infoTask) {
          return sendDataToMultilSocketsOfListUser({
            ...
            data: {
              error: true,
              message: 'Task hoặc Checklist không tồn tại',
            },
            event: [
              EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_SSC_ADD_CHECKLIST,
            ],
   ...
   * Tạo mới việc con
   */
  [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_ADD_CHILD_TASK]:
    async function (data) {
      try {
        let socket = this
        let listUserActive = socket.$service.listUserConnected
        ...
        if (!taskID || !childTaskID) {
          return sendDataToMultilSocketsOfListUser({
            ...
            data: {
              error: true,
              message: 'Tham số taskID hoặc childTaskID không hợp lệ',
            },
            event: [
              EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_SSC_ADD_CHILD_TASK,
            ],
            io,
          })
        }
        ...
        const infoChildTask = await PCM_PLAN_TASK_COLL.findById(childTaskID)
          .select('_id name assignee expiredTime parent status')
          .populate('assignee', '_id bizfullname fullname image')
          .lean()
        ...
        if (!infoChildTask || !infoTask) {
          return sendDataToMultilSocketsOfListUser({
            ...
            data: {
              error: true,
              message: 'Task hoặc ChildTask không tồn tại',
            },
            event: [
              EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_SSC_ADD_CHILD_TASK,
            ],
   ...
   */
  [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_ADD_RELATION_USER]:
    async function (data) {
      try {
        let socket = this
        let listUserActive = socket.$service.listUserConnected
        ...
        if (!taskID || !relationUsersID || !relationUsersID.length) {
          return sendDataToMultilSocketsOfListUser({
            ...
            data: {
              error: true,
              message: 'Tham số taskID hoặc relationUsersID không hợp lệ',
            },
            event: [
              EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_SSC_ADD_RELATION_USER,
            ],
            io,
          })
        }
   ...
   * Đồng ý phê duyệt
   */
  [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_ADD_SIGNATURE]:
    async function (data) {
      let socket = this
      ...
      try {
        ...
        if (!taskID) {
          return sendDataToMultilSocketsOfListUser({
            ...
            event: [
              EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_SSC_ADD_SIGNATURE,
            ],
            io,
          })
        }
   ...
   * Mở thầu
   */
  [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_OPEN_BIDDING]:
    async function (data) {
      let socket = this
      let broker = socket.$service.broker
      let listUserActive = socket.$service.listUserConnected
      ...
      try {
        ...
        if (!taskID) {
          return sendDataToMultilSocketsOfListUser({
            ...
            event: [
              EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_SSC_OPEN_BIDDING,
            ],
            io,
          })
        }
   ...
   */
  [EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_CSS_UPDATE_NOTIFICATION]:
    async function (data) {
      try {
        let socket = this
        let listUserActive = socket.$service.listUserConnected
        ...
        if (!taskID) {
          return sendDataToMultilSocketsOfListUser({
            ...
            event: [
              EVENT_SOCKET_CONSTANT_SUBJECT_PCM.SUBJECT_PCM_SSC_UPDATE_NOTIFICATION,
            ],
            io,
          })
        }
        ...
        const listReceiversExceptMe = listReceivers.filter(
          (receiverID) => receiverID !== userID.toString()
        )

        if (isGetCountNotification) {
          await PromisePool.for(listReceivers)
            .withConcurrency(4)
