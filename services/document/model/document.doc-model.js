        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (storeSign && storeSign != '') {
                    convertStr =
                        convertStr + ' ' + stringUtils.removeAccents(storeSign)
                }
                ...
                if (approver && approver != '') {
                    convertStr =
                        convertStr + ' ' + stringUtils.removeAccents(approver)
                }
                ...
                amountDelivery &&
                    (dataInsert.amountDelivery = +amountDelivery || 0)

                contractID &&
                    checkObjectIDs(contractID) &&
                    (dataInsert.contract = contractID)
                senderID &&
                    checkObjectIDs(senderID) &&
                    (dataInsert.sender = senderID)
                propertyID &&
                ...
                if (parentID && checkObjectIDs(parentID)) {
                    let infoParent =
                        await DOCUMENT__DOC_COLL.findById(parentID).select(
                            'level'
                        )
                    if (!infoParent)
                        return resolve({
                ...
                if (!parentID) {
                    ...
                    if (packageID && checkObjectIDs(packageID)) {
                        await DOCUMENT__PACKAGE_COLL.findByIdAndUpdate(
                            packageID,
                            {
                                $inc: {
                                    numberOfDocs: 1,
            ...
            try {
                if (!checkObjectIDs(documentID, packageID, publishID, userID)) {
                    return resolve({
                        error: true,
                        message:
                            'Request params departmentID|packageID|publishID invalid',
                        keyError: 'params_invalid',
                        status: 400,
                    })
                }
                ...
                requestResponse &&
                    (dataUpdate.requestResponse = +requestResponse || 0)
                amount && (dataUpdate.amount = +amount || 1)
                amountDelivery &&
                    (dataUpdate.amountDelivery = +amountDelivery || 0)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (isShare) {
                    ...
                    infoAfterShare =
                        await DOCUMENT__USER_COLL.insertMany(listUserShared)
                } else {
                    ...
                    infoAfterShare =
                        await DOCUMENT__USER_COLL.deleteMany(
                            listUserRemoveShare
                        )
                }
    ...
    updatePermissionShare({ authorID, documentID, userID, permission }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await DOCUMENT__USER_COLL.findByIdAndUpdate(
                        infoUserShared._id,
                        {
                            $set: {
                                permission,
            ...
            try {
                ...
                if (checkObjectIDs(departmentID)) {
                    ...
                    if (!packageID && !parentID) {
                        let listPackageAccess =
                            await DOCUMENT__PACKAGE_COLL.find({
                                members: { $in: [userID] },
                            })?.select('_id')
                        if (listPackageAccess.length) {
                            conditionObj.package = {
                ...
                let infoDataAfterGet = await DOCUMENT__DOC_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await DOCUMENT__DOC_COLL.count(conditionObjOrg)
            ...
            try {
                ...
                if (listDoctypeGroupByType.length) {
                    listDoctypeGroupByType.map((doctype) => {
                        const doctypeHaveNewDocumentInWeek =
                            listDoctypeGroupByWeek.find(
    ...
    getListUserShared({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let listUserShared = await DOCUMENT__USER_COLL.find(
                    conditionObj
                )
                ...
                // GET TOTAL RECORD
                let totalRecord =
                    await DOCUMENT__USER_COLL.count(conditionObjOrg)
                let nextCursor = null
                ...
                XlsxPopulate.fromFileAsync(
                    ...
                    listDocument.map((document, index) => {
                        ...
                        if (modifyAt) {
                            currRow
                                .cell(3)
                                .value(moment(modifyAt).format(DATE_FORMAT))
                        }
                        ...
                        if (receivedDate) {
                            currRow
                                .cell(8)
                                .value(moment(receivedDate).format(DATE_FORMAT))
                        }
                        ...
                        if (type) {
                            currRow
                                .cell(20)
                                .value(DOCUMENT_TYPES[Number(type - 1)].text)
                        }
                ...
                XlsxPopulate.fromFileAsync(
                    ...
                    listPackage.map((item, index) => {
                        ...
                        for (const packageItem of listDocGroupByPackage) {
                            if (
                                packageItem._id.package.toString() ===
                                item._id.toString()
                            ) {
                                if (
                        ...
                        for (const packageItem of listDocHaveToDone) {
                            if (
                                ...
                                total2 =
                                    Number(total2) + Number(packageItem.amount)
                            }
                        }
                    ...
                    listData1.map((document, index) => {
                        ...
                        if (deadline) {
                            currRow
                                .cell(22)
                                .value(moment(deadline).format(DATE_FORMAT))
                        }
                        ...
                        currRow
                            ...
                            .value('Truy cập')
                            .hyperlink(
                                `https://app.trixgo.com/document/home/${_id}`
                            )
                        ...
                        listData2.map((subitem, subindex) => {
                            if (
                                ...
                                workbook
                                    ...
                                    .cell(16)
                                    .value(
                                        subitem.approver ? subitem.approver : ''
                                    )
                                workbook
                                    .sheet('Data')
                                    .row(indexExcel)
                                ...
                                workbook
                                    ...
                                    .cell(19)
                                    .value(
                                        subitem.field ? subitem.field.name : ''
                                    )
                                // Tính chất
                                if (subitem.type) {
                                    workbook
                                ...
                                if (subitem.deadline) {
                                    workbook
                                        ...
                                        .cell(22)
                                        .value(
                                            moment(subitem.deadline).format(
                                                DATE_FORMAT
                                            )
                                        )
                                }
                                ...
                                listData3.map((subitem2, subindex2) => {
                                    if (
                                        ...
                                        workbook
                                            ...
                                            .cell(1)
                                            .value(
                                                `${index + 1}.${subindex + 1}.${subindex2 + 1}`
                                            )
                                        workbook
                                            .sheet('Data')
                                            .row(indexExcel)
                                        ...
                                        listData4.map((subitem3, subindex3) => {
                                            if (
                                                subitem3.parent &&
                                                subitem3.parent.toString() ===
                                                    subitem2._id.toString()
                                            ) {
                                                workbook
                                                    .sheet('Data')
                                                    .row(indexExcel)
