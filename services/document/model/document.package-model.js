...
class Model extends BaseModel {
   ...
   /**
     ...
     * Date: 30/4/2022
     */
    update({ packageID, userID, ctx, fieldID, bidderID, contractID, type, name, sign, description, note, status, percentOfCompletedPackage, startTime, finishTime, actualStartTime, closingTime, actualFinishTime, form, contractType, duration, progress, packagePrice, tenderPrice, vatTenderPrice, members, membersRemove, admins, adminsRemove }) {
        const that = this
        return new Promise(async (resolve) => {
            try {
                /**
                ...
                let infoAfterUpdate = await DOCUMENT__PACKAGE_COLL.findByIdAndUpdate(packageID, dataUpdate, { new: true })
                if (!infoAfterUpdate)
    ...
    getInfo({ packageID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                if(!checkObjectIDs(packageID))
                    return resolve({ error: true, message: 'Request params packageID invalid', status: 400 });
	 ...
	 * Date: 26/10/2021
	 */
    getListPackage({ departmentID, parentID, status, userID, lastestID, isShowAll, keyword, limit = 10, select, populates = {}}){
        return new Promise(async resolve => {
            try {
                if(!checkObjectIDs(departmentID))
                    return resolve({ error: true, message: 'Request params departmentID invalid', status: 400 });

                let conditionObj = { members: { $in: [userID] }, project: departmentID };
                let sortBy;
                ...
                let listPackages = await DOCUMENT__PACKAGE_COLL
					.find(conditionObj)
					.limit(limit + 1)
					.select(select)
					.populate(populates)
