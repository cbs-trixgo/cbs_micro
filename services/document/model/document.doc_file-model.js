    ...
    return new Promise(async (resolve) => {
      try {
        ...
        const newFiles = docFiles.filter(
          (fileID) => fileID.toString() !== infoAfterInsert.file.toString()
        )
  ...
  update({ docFileID, official }) {
    return new Promise(async (resolve) => {
      try {
         ...
         * 3.2: Operation database
         */
        let infoAterUpdate = await DOCUMENT__DOC_FILE_COLL.findByIdAndUpdate(
          docFileID,
          { official },
          { new: true }
        )
  ...
  getList({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let listDocFile = await DOCUMENT__DOC_FILE_COLL.find(conditionObj)
          .limit(limit + 1)
          .sort(sortBy)
          .select(select)
          .populate(populates)
