...
class Model extends BaseModel {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(!checkObjectIDs(docFileID))
                    return resolve({ error: true, message: 'Request params docFileID invalid', status: 400 });
                 ...
                 * 3.2: Operation database
                 */
                let infoAfterInsert = await DOCUMENT__DOC_FILE_COLL.findByIdAndDelete(docFileID);
                if(!infoAfterInsert)
                    return resolve({ error: true, message: "can't_delete", status: 403 });

                const docInfo = await DOCUMENT__DOC_COLL.findById(infoAfterInsert.document).select('files').lean();

                if(!docInfo)
                    return resolve({ error: true, message: "not found document info", status: 404 });

                const docFiles = docInfo.files || [];
                const newFiles = docFiles.filter(fileID => fileID.toString() !== infoAfterInsert.file.toString());
                ...
                await DOCUMENT__DOC_COLL.findByIdAndUpdate(infoAfterInsert.document, dataUpdateDoc);
	...
	update({ docFileID, official }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 * 3.2: Operation database
                 */
                let infoAterUpdate = await DOCUMENT__DOC_FILE_COLL.findByIdAndUpdate(docFileID, { official }, { new: true });
                if(!infoAterUpdate)
                    return resolve({ error: true, message: "can't_get_info", status: 403 });

                return resolve({ error: false, data: infoAterUpdate, status: 200 });
            } catch (error) {
                return resolve({ error: true, message: error.message, status: 500 });
            }
        })
	...
	getInfo({ docFileID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 * 3.2: Operation database
                 */
                let infoAterGet = await DOCUMENT__PACKAGE_COLL.findById(docFileID)
                                    .select(select)
                                    .populate(populates)
                if(!infoAterGet)
                    return resolve({ error: true, message: "can't_get_info", status: 403 });
	 ...
	 * Date: 26/10/2021
	 */
    getList({ documentID, lastestID, keyword, limit = 10, select, populates = {} }){
        return new Promise(async resolve => {
            try {
				...
				if(lastestID && checkObjectIDs(lastestID)){
					let infoData = await DOCUMENT__DOC_FILE_COLL.findById(lastestID);
					if(!infoData)
                ...
                let listDocFile = await DOCUMENT__DOC_FILE_COLL
					.find(conditionObj)
					.limit(limit + 1)
                    .sort(sortBy)
					.select(select)
				...
				// GET TOTAL RECORD
                let totalRecord = await DOCUMENT__DOC_FILE_COLL.count(conditionObjOrg);
