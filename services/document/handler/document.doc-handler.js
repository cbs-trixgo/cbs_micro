    ...
    addFileAttachment: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.addFileAttachment({
                    authorID, documentID, filesID
                });
    ...
    insertDocument: {
        ...
        async handler(ctx) {
            try {
                ...
                const {
                    parentID, name, sign, storeSign, description, approver, date, receivedDate, type, style, status,
                    amount, amountDelivery, requestResponse, expiredResponse, direction, packageID, senderID, receiverID,
                    fieldID, propertyID, otherID, storageID, note, departmentID, value, completeStatus, contractID, datahubContactID, publishID, deadline
                } = ctx.params;

                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.insert({
                    parentID, name, sign, storeSign, description, approver, date, receivedDate, type, style, status,
                    amount, amountDelivery, requestResponse, expiredResponse, direction, packageID, senderID, receiverID,
                    fieldID, propertyID, otherID, storageID, note, departmentID, userID, value, completeStatus,contractID, datahubContactID, publishID, deadline, ctx
				});
    ...
    updateDocument: {
        ...
        async handler(ctx) {
            try {
                ...
                const {
                    documentID, name, sign, storeSign, description, approver, date, receivedDate, type, style, status,
                    amount, amountDelivery, requestResponse, expiredResponse, direction, packageID, senderID, receiverID,
                    fieldID, propertyID, otherID, storageID, note, value, completeStatus, contractID, datahubContactID, publishID, deadline
                } = ctx.params;

                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.update({
                    documentID, name, sign, storeSign, description, approver, date, receivedDate, type, style, status,
                    amount, amountDelivery, requestResponse, expiredResponse, direction, packageID, senderID, receiverID,
                    fieldID, propertyID, otherID, storageID, note, userID: userUpdate, value, completeStatus, contractID, datahubContactID, publishID, deadline, ctx
				});
    ...
    getInfoDocumment: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.getInfo({
                    userID, documentID, select, populates
                });
    ...
    getListDocument: {
        ...
        async handler(ctx) {
            try {
                ...
                let {
                    parentID, companyID, packageID, departmentID, direction, senderID, receiverID, propertyID, receivedDate, datahubContactID,
                    type, contractID, storageID, requestResponse, fieldID, fromDate, toDate, marked, keyword, limit, lastestID, select, populates, isParent
                } = ctx.params;
                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.getList({
                    parentID, companyID, packageID, departmentID, direction, senderID, receiverID, propertyID, receivedDate,
                    type, contractID, storageID, requestResponse, fieldID, fromDate, toDate, marked, publishID: datahubContactID, userID, keyword,
                    limit, lastestID, select, populates, isParent
                });
    ...
    getListByFilter: {
        ...
        async handler(ctx) {
            try {
                ...
                let {
                    parentID, companyID, packagesID, departmentsID, directions, sendersID, receiverID, propertysID, datahubContactsID,
                    types, contractsID, storagesID, requestResponse, fieldsID, fromDate, toDate, marked, keyword, limit, lastestID, select, populates, isParent
                } = ctx.params;

                // console.log(ctx.params)

                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.getListByFilter({
                    parentID, companyID, packagesID, departmentsID, directions, sendersID, receiverID, propertysID,
                    types, contractsID, storagesID, requestResponse, fieldsID, fromDate, toDate, marked, publishsID: datahubContactsID, userID, keyword,
                    limit, lastestID, select, populates, isParent
                });
    ...
    getListGroupDoctype: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.statisticalByType({
                    userID, companyID, departmentID, packageID
                });
    ...
    getListStorage: {
        ...
        async handler(ctx) {
            try {
				await this.validateEntity(ctx.params);
				const { _id: userID } = ctx.meta.infoUser;
                const { companyID, lastestID, keyword, limit, select, filter } = ctx.params;

                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.getListStorage({
                    userID, companyID, lastestID, keyword, limit, select, filter
                });
    ...
    updateMarkDocument: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.updateMarkDocument({
                    userID, documentID, isMark
                });
    ...
    updateShareDocument: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.updateShareDocument({
                    authorID, documentID, usersID, isShare
                });
    ...
    updatePermissionShare: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.updatePermissionShare({
                    authorID, documentID, userID, permission
                });
    ...
    getListUserShared: {
        ...
        async handler(ctx) {
            try {
				await this.validateEntity(ctx.params);
				const { _id: userID } = ctx.meta.infoUser;
                const { documentID, lastestID, limit, select, filter } = ctx.params;

                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.getListUserShared({
                    userID, documentID, lastestID, limit, select, filter
                });
        ...
        async handler(ctx) {
            try {
                ...
                if(isDatahubContact == 1){
                    resultAfterCallHandler = await DOCUMENT__DOC_MODEL.statisticalByDatahubContact({
                        userID, companyID: company._id, departmentID, packageID, isDatahubContact
                    });
                }else{
                    resultAfterCallHandler = await DOCUMENT__DOC_MODEL.statisticalByType({
                        userID, companyID: company._id, departmentID, packageID
                    });
                }
    ...
    getAmountDocumentByProjects: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.getAmountDocumentByProjects({
                    userID, fromDate, toDate, ctx
                });
    ...
    getDynamicReport: {
        ...
        async handler(ctx) {
            try {
                ...
                let {
                    reportType, option, companyID, projectID, status, fromDate, toDate, subtypes, upcoming,  isMilestone, year
                } = ctx.params;
                ...
                let resultAfterCallHandler = await DOCUMENT__DOC_MODEL.getDynamicReport({ companyID, userID, reportType, option, projectID, status, fromDate, toDate, subtypes, upcoming, isMilestone, year })
                return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx })
            } catch (error) {
     ...
     downloadTemplateExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                // console.log('====================HERE===================')
                // console.log('====================HERE===================')
                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.downloadTemplateExcel({
                    projectID, userID 
                })
    ...
    importFromExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.importFromExcel({
                    projectID, documentID, dataImport, userID, ctx
                })
                return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx });
            } catch (error) {
                return renderStatusCodeAndResponse({ error_message: error.message, ctx });
        ...
        async handler(ctx) {
            try {
                ...
                if(!filterParams.option){
                    const resultAfterCallHandler = await DOCUMENT__DOC_MODEL.exportDocumentOfPackage({
                        userID, filterParams, ctx
                    })
