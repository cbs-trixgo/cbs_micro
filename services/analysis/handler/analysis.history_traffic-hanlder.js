 ...
 * MODELS
 */
const ANALYSIS__HISTORY_TRAFFIC_MODEL =
  require('../model/analysis.history_traffic').MODEL
    ...
    async handler(ctx) {
      try {
        ...
        let resultAfterCallHandler =
          await ANALYSIS__HISTORY_TRAFFIC_MODEL.insert({
            appID,
            menuID,
            type,
            action,
            companyOfAuthor: company._id,
            userCreate: userID,
          })
  ...
  statisticsDeviceAccessByYear: {
    ...
    async handler(ctx) {
      try {
        ...
        const { _id: userID, company } = ctx.meta.infoUser
        let resultAfterCallHandler =
          await ANALYSIS__HISTORY_TRAFFIC_MODEL.statisticsDeviceAccessByYear({
            year,
          })
        return renderStatusCodeAndResponse({
          resultAfterCallHandler,
  ...
  statisticsByDeviceAccess: {
    ...
    async handler(ctx) {
      try {
        ...
        const { _id: userID, company } = ctx.meta.infoUser
        let resultAfterCallHandler =
          await ANALYSIS__HISTORY_TRAFFIC_MODEL.statisticsByDeviceAccess({
            fromDate,
            toDate,
          })
        return renderStatusCodeAndResponse({
  ...
  statisticsAccessByApp: {
    ...
    async handler(ctx) {
      try {
        ...
        let resultAfterCallHandler =
          await ANALYSIS__HISTORY_TRAFFIC_MODEL.statisticsAccessByApp({
            option,
            appID,
            fromDate,
            toDate,
  ...
  statisticsAccessByAppAndDevice: {
    ...
    async handler(ctx) {
      try {
        ...
        const { _id: userID, company } = ctx.meta.infoUser
        let resultAfterCallHandler =
          await ANALYSIS__HISTORY_TRAFFIC_MODEL.statisticsAccessByAppAndDeveice(
            {
              fromDate,
              toDate,
            }
  ...
  statisticsAccessByCompany: {
    ...
    async handler(ctx) {
      try {
        ...
        const { _id: userID, company } = ctx.meta.infoUser
        let resultAfterCallHandler =
          await ANALYSIS__HISTORY_TRAFFIC_MODEL.statisticsAccessByCompany({
            fromDate,
            toDate,
          })
        return renderStatusCodeAndResponse({
  ...
  getInfoAndGetList: {
    ...
    async handler(ctx) {
      try {
        ...
        if (historyTrafficID) {
          resultAfterCallHandler =
            await ANALYSIS__HISTORY_TRAFFIC_MODEL.getInfo({
              historyTrafficID,
              select,
              populates,
            })
          ...
          resultAfterCallHandler =
            await ANALYSIS__HISTORY_TRAFFIC_MODEL.getList({
              companyID,
              type,
              appID,
              menuID,
