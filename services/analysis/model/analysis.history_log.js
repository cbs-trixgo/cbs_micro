...
class Model extends BaseModel {
        ...
        return new Promise(async resolve => { 
            try {
                ...
                if(checkObjectIDs(taskID)){
                    ...
                    if(!infoTask){
                        return resolve({ error: true, message: "taskID_invalid" })
                    }
                ...
                if(!infoAfterInsert)
                    return resolve({ error: true, message: "cannot_insert_history_log" })
        ...
        return new Promise(async (resolve) => {
            try {
                if(!checkObjectIDs(historyLogID))
                    return resolve({ error: true, message: 'Request params historyLogID invalid', status: 400 });
                ...
                let infoAterGet = await ANALYSIS__HISTORY_LOG_COLL.findById(historyLogID)
                                    .select(select)
                                    .populate(populates)
                if(!infoAterGet)
     ...
     */
    getList({
        taskID, projectID, contractID, fromDate, toDate,
        ...
        return new Promise(async (resolve) => {
            try {
				...
				if(lastestID && checkObjectIDs(lastestID)){
					let infoData = await ANALYSIS__HISTORY_LOG_COLL.findById(lastestID);
					if(!infoData)
                ...
                let infoDataAfterGet = await ANALYSIS__HISTORY_LOG_COLL.find(conditionObj)
                .select(select)
                .limit(limit+1)
                .sort(sortBy)
                .populate(populates)
                ...
                let totalRecord = await ANALYSIS__HISTORY_LOG_COLL.countDocuments(conditionObj);
