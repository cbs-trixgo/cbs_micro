            ...
            try {
                ...
                if (option == 1) {
                    ...
                    if (!getNumber) {
                        ...
                        let listDataConvert = await ITEM__CONTACT_COLL.find(
                            conditionObj
                        )
                            .select(selectObj)
                            .sort({ _id: 1 })
                        ...
                        let number =
                            await ITEM__CONTACT_COLL.count(conditionObj)
                ...
                // convertDoctype
                    ...
                    if (!getNumber) {
                        let limit = Number(page * unit)
                        let listData = await ITEM__DOCTYPE_COLL.find(
                            conditionObj
                        )
                            .select('_id')
                            .sort({ _id: 1 })
                        ...
                        let listDataConvert = await ITEM__DOCTYPE_COLL.find(
                            conditionObj
                        )
                            .select(selectObj)
                            .sort({ _id: 1 })
                        ...
                        let number =
                            await ITEM__DOCTYPE_COLL.count(conditionObj)
                        return resolve({ error: false, number: number })
                    }
                }
                ...
                // convertBudget
                    ...
                    if (!getNumber) {
                        let limit = Number(page * unit)
                        let listData = await BUDGET__WORK_COLL.find(
                            conditionObj
                        )
                            .select('_id')
                            .sort({ _id: 1 })
                        ...
                        let listDataConvert = await BUDGET__WORK_COLL.find(
                            conditionObj
                        )
                            .select(selectObj)
                            .sort({ _id: 1 })
                ...
                // Convert Contact Parent
                    ...
                    if (checkObjectIDs(docID) && checkObjectIDs(parentID)) {
                        let infoContact =
                            await ITEM__CONTACT_COLL.findById(docID)
                        let infoParent =
                            await ITEM__CONTACT_COLL.findById(parentID)
                        ...
                        if (parentID && checkObjectIDs(parentID)) {
                            ...
                            for (const subitem of infoContact?.nestedChilds) {
                                let infoSubChild =
                                    await ITEM__CONTACT_COLL.findById(subitem)
                                        .populate({
                                            path: 'parent',
                                            select: 'level',
                                        })
                    ...
                    if (!getNumber) {
                        ...
                        let number =
                            await FNB_CUSTOMER_CARE_COLL.count(conditionObj)
                        return resolve({ error: false, number: number })
                    }
                }
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (info.sign && info.sign != '') {
                    convertStr =
                        convertStr + ' ' + stringUtils.removeAccents(info.sign)
                }
                if (info.description && info.description != '') {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(info.description)
                }
                if (info.note && info.note != '') {
                    convertStr =
                        convertStr + ' ' + stringUtils.removeAccents(info.note)
                }
                if (info.phone && info.phone != '') {
                    convertStr =
                        convertStr + ' ' + stringUtils.removeAccents(info.phone)
                }
                if (info.email && info.email != '') {
                    convertStr =
                        convertStr + ' ' + stringUtils.removeAccents(info.email)
                }
                if (info.identity && info.identity != '') {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(info.identity)
                }
                if (info.taxid && info.taxid != '') {
                    convertStr =
                        convertStr + ' ' + stringUtils.removeAccents(info.taxid)
                }
                if (info.bankAccount && info.bankAccount != '') {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(info.bankAccount)
                }
            ...
            try {
                ...
                if (
                    ...
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(info?.customer?.name)
                }
                if (
                    info?.customer?.sign &&
                    info?.customer?.sign != '' &&
                    ...
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(info?.customer?.sign)
                }
                if (
                ...
                if (
                    ...
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(info?.customer?.note)
                }
                if (
                    info?.customer?.phone &&
                    info?.customer?.phone != '' &&
                    ...
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(info?.customer?.phone)
                }
                if (
                    info?.customer?.email &&
                    info?.customer?.email != '' &&
                    ...
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(info?.customer?.email)
                }
                if (
                ...
                if (
                    ...
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(info?.customer?.taxid)
                }
                if (
    ...
    exportExcel({ option, isPrivate, companyID, year, fromDate, toDate }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (option == 1) {
                    let listData = await ANALYSIS__HISTORY_TRAFFIC_COLL.find(
                        conditionObj
                    )
                        .limit(5000)
                        .populate({
