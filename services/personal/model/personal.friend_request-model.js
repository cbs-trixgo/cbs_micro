...
class Model extends BaseModel {
    ...
    checkCodeValid({ code }) {
        return new Promise(async (resolve) => {
            try {
                if(!code)
                    return resolve({ error: true, message: "Vui lòng nhập code", message: "code_invalid" });
                /**
                 * BA
                ...
                let infoCode = await PERSONAL__FRIEND_REQUEST_COLL.findOne({
                    ...
                    path: "user",
                    select: "_id fullname sign image company email phone birthDay friends receiveFromFriends",
                    populate: {
                        path: "company",
                        select: "name"
                    }
                ...
                if(!infoCode)
                    return resolve({ error: true, message: "Mã code kết bạn không hợp lệ", message: "code_invalid" });
