...
class Model extends BaseModel {
    ...
    update({ noteID, name, description, members, membersRemove, trash, pin, removePin, userID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate = await PERSONAL__NOTE_COLL.findByIdAndUpdate(noteID, dataUpdate, { new: true });
                if(!infoAfterUpdate)
     ...
     * Date: 9/4/2022
     */
    getList({ keyword, pin, limit = 10, lastestID, select, populates= {}, userID }) {
        return new Promise(async (resolve) => {
            try {
                if(limit > 20){
                    limit = 20
