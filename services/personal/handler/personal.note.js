...
module.exports = {
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { noteID, name, description, members, membersRemove, trash, pin, removePin } = ctx.params;
                const { _id: userID } = ctx.meta.infoUser;

                let resultAfterCallHandler = await PERSONAL__NOTE_MODEL.update({
                    noteID, name, description, members, membersRemove, trash, pin, removePin, userID
                });
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                const { noteID, keyword, pin, limit, lastestID, select, populates } = ctx.params;
                const { _id: userID } = ctx.meta.infoUser;

                let resultAfterCallHandler;
                if(noteID){
                    resultAfterCallHandler = await PERSONAL__NOTE_MODEL.getInfo({
                        noteID, select, populates
                    });
                }else{
                    resultAfterCallHandler = await PERSONAL__NOTE_MODEL.getList({
                        keyword, pin, limit, lastestID, select, populates, userID
                    });
                }
                return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx });
            } catch (error) {
