...
class Model extends BaseModel {
	...
	insert({ companyID, parentID, name, description, userID }){
        return new Promise(async resolve => {
            try {
                ...
                if(checkObjectIDs(parentID)){
                    await ITEM__WAREHOUSE_COLL.findByIdAndUpdate(parentID, { $inc: { amountChilds: 1 }}, { new: true });
                }
        ...
        return new Promise(async resolve => { 
            try {
                if(!checkObjectIDs(warehouseID))
                    return resolve({ error: true, message: "warehouseID không hợp lệ", keyError: "Request params warehouseID invalid" });
                ...
                let infoAfterUpdate = await ITEM__WAREHOUSE_COLL.findByIdAndUpdate(warehouseID, dataUpdate, { new: true });
    ...
    getInfo({ warehouseID, select, populates }) { 
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                if(!checkObjectIDs(warehouseID))
                    return resolve({ error: true, message: 'Request params warehouseID invalid', status: 400 });
     ...
     */
    getList({
        companyID, parentID, isListParentOfListChilds,
