    ...
    getInfo({ warehouseID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                let infoAterGet = await ITEM__WAREHOUSE_COLL.findById(
                    warehouseID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await ITEM__WAREHOUSE_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await ITEM__WAREHOUSE_COLL.find(
                    conditionObj
                )
                    .limit(+limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await ITEM__WAREHOUSE_COLL.count(conditionObjOrg)
