    ...
    insert({ companyID, projectID, coll, title, note, authorID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                const checkSignature = await ITEM__SIGNATURE_COLL.findOne({
                    'coll.kind': coll.kind,
                    'coll.item': coll.item,
                    project: projectID,
                    company: companyID,
    ...
    getInfo({ signatureID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAterGet = await ITEM__SIGNATURE_COLL.findById(
                    signatureID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await ITEM__SIGNATURE_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await ITEM__SIGNATURE_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await ITEM__SIGNATURE_COLL.count(conditionObjOrg)
                let nextCursor = null
