  ...
  update({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAfterUpdate = await ITEM__DEPARTMENT_COLL.findByIdAndUpdate(
          departmentID,
          dataUpdate,
          { new: true }
        )
  ...
  getInfo({ departmentID, select, populates = {} }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let infoDepartment = await ITEM__DEPARTMENT_COLL.findById(departmentID)
          .select(select)
          .populate(populates)
          .lean()
  ...
  getList({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoDataAfterGet = await ITEM__DEPARTMENT_COLL.find(conditionObj)
          .limit(limit + 1)
          .sort(sortBy)
          .select(select)
          .populate(populates)
  ...
  getListByMembers({
    ...
    return new Promise(async (resolve) => {
      try {
         ...
         */
        if (unLimit) {
          const dataUnlimit = await ITEM__DEPARTMENT_COLL.find(conditionObj)
            .select(select)
            .lean()
        ...
        const infoDataAfterGet = await ITEM__DEPARTMENT_COLL.find(conditionObj)
          .limit(limit + 1)
          .sort(sortBy)
          .select(select)
          .populate(populates)
