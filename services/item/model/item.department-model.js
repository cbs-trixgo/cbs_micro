...
class Model extends BaseModel {
        ...
        return new Promise(async (resolve) => {
            try {
                if (!_isValid(company) | !_isValid(authorID))
                    return resolve({
                        error: true,
                        message: 'Request params company|author(*) invalid',
                        status: 400,
                    })
                if (!name)
                    return resolve({
                        error: true,
                        message: 'Request params name(*) invalid',
                        status: 400,
                    })
                ...
                if (!sign)
                    return resolve({
                        error: true,
                        message: 'Request params sign(*) invalid',
                        status: 400,
                    })

                // Lấy thông tin công ty để gán ký hiệu công ty vào tên dự án
                let infoCompany = await ctx.call(
                ...
                if (infoCompany.error)
                    return resolve({
                        error: true,
                        message: 'Request params company invalid',
                    })
                ...
                if (infoDepartment)
                    return resolve({
                        error: true,
                        message: `Tên/Mã hiệu đã tồn tại: ${infoDepartment.name}`,
                    })
                ...
                let dataInsert = {
                    ...
                    note,
                    projectCapital: !isNaN(projectCapital)
                        ? Number(projectCapital)
                        : 1,
                    projectType: !isNaN(projectType) ? Number(projectType) : 1,
                    buildingGrade: !isNaN(buildingGrade)
                        ? Number(buildingGrade)
                        : 1,
                    buildingType: !isNaN(buildingType)
                        ? Number(buildingType)
                        : 1,
                    investmentAmount: !isNaN(investmentAmount)
                ...
                if (parent && _isValid(parent)) {
                    let infoMenuParent =
                        await ITEM__DEPARTMENT_COLL.findById(parent)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                const infoDepartment =
                    await ITEM__DEPARTMENT_COLL.findById(departmentID)
                console.log(infoDepartment.members.length)

                // KIỂM TRA SIGN VÀ NAME CÓ TỒN TẠI Ở 1 DỰ ÁN KHÁC CHƯA
                if (sign || name) {
                    let checkExistsSignAndName =
                        await ITEM__DEPARTMENT_COLL.findOne({
                    ...
                    if (checkExistsSignAndName)
                        return resolve({
                            error: true,
                            message: `Tên/Mã hiệu đã tồn tại: ${checkExistsSignAndName.name}`,
                        })
                }
                ...
                if (!isNaN(numberOfUnexecutedPackage)) {
                    dataUpdate.numberOfUnexecutedPackage =
                        numberOfUnexecutedPackage
                }
                ...
                if (!isNaN(numberOfCompletedPackage)) {
                    dataUpdate.numberOfCompletedPackage =
                        numberOfCompletedPackage
                }

                if (!isNaN(amountOfUnexecutedPackage)) {
                    dataUpdate.amountOfUnexecutedPackage =
                        amountOfUnexecutedPackage
                }
                ...
                if (!isNaN(amountOfCompletedPackage)) {
                    dataUpdate.amountOfCompletedPackage =
                        amountOfCompletedPackage
                }
                ...
                if (!isNaN(contractAdvancePaymentDeduction)) {
                    dataUpdate.contractAdvancePaymentDeduction =
                        contractAdvancePaymentDeduction
                }
                ...
                if (!isNaN(contractRecommendedPayment)) {
                    dataUpdate.contractRecommendedPayment =
                        contractRecommendedPayment
                }

                if (!isNaN(contractAdvancePaymentPaid)) {
                    dataUpdate.contractAdvancePaymentPaid =
                        contractAdvancePaymentPaid
                }
                ...
                if (!isNaN(contractRemainingPayment)) {
                    dataUpdate.contractRemainingPayment =
                        contractRemainingPayment
                }

                if (!isNaN(contractAdvancePaymentOverage)) {
                    dataUpdate.contractAdvancePaymentOverage =
                        contractAdvancePaymentOverage
                }
                ...
                if (photos && photos.length) {
                    if (infoDepartment.photos?.length >= 10) {
                        await ITEM__DEPARTMENT_COLL.findByIdAndUpdate(
                            departmentID,
                            {
                                $push: {
                                    photos: {
                ...
                if (!infoAfterUpdate)
                    return resolve({
                        error: true,
                        message: 'Update department faild',
                        status: 422,
                    })
     ...
     * Date  : 22/9/2022
     */
    updateValue({ projectID, userID, option, ctx }) {
        return new Promise(async (resolve) => {
            try {
                /**
                 * BA
                ...
                if (option && Number(option) === 1) {
                    ...
                    if (listData.length) {
                        ...
                        dataUpdate.contractVatProduce = Number(
                            listData[0].vatProduce
                        )
                        dataUpdate.contractPlusProduce = Number(
                            listData[0].plusProduce
                        )
                        dataUpdate.contractVatPlusProduce = Number(
                            listData[0].vatPlusProduce
                        )
                        dataUpdate.contractRemainingProduce = Number(
                            listData[0].remainingProduce
                        )
                    }
                }
                ...
                if (option && Number(option) === 2) {
                    console.log(
                        '=============2-Cập nhật Doanh thu, tiền về DỰ ÁN===========>>>>>>>>>>>>'
                    )
                    ...
                    if (listData.length) {
                        ...
                        dataUpdate.contractVatValue = Number(
                            listData[0].vatValue
                        )
                        dataUpdate.contractPlus = Number(listData[0].plus)
                        dataUpdate.contractVatPlus = Number(listData[0].vatPlus)
                        dataUpdate.contractAcceptance = Number(
                            listData[0].acceptance
                        )
                        dataUpdate.contractVatAcceptance = Number(
                            listData[0].vatAcceptance
                        )
                        dataUpdate.contractPlusAcceptance = Number(
                            listData[0].plusAcceptance
                        )
                        dataUpdate.contractVatPlusAcceptance = Number(
                            listData[0].vatPlusAcceptance
                        )
                        dataUpdate.contractRetainedValue = Number(
                            listData[0].retainedValue
                        )
                        dataUpdate.contractAdvancePaymentDeduction = Number(
                            listData[0].advancePaymentDeduction
                        )
                        dataUpdate.contractOtherDeduction = Number(
                            listData[0].otherDeduction
                        )
                        dataUpdate.contractRecommendedPayment = Number(
                            listData[0].recommendedPayment
                        )
                        dataUpdate.contractAdvancePaymentPaid = Number(
                            listData[0].advancePaymentPaid
                        )
                        dataUpdate.contractAmountPaid = Number(
                            listData[0].amountPaid
                        )
                        dataUpdate.contractRemainingPayment =
                            Number(listData[0].recommendedPayment) -
                            Number(listData[0].advancePaymentPaid) -
                            Number(listData[0].amountPaid)
                    }
                }
     ...
     * Code: F00401
     */
    getInfo({ departmentID, select, populates = {} }) {
        return new Promise(async (resolve) => {
            try {
                /**
                ...
                let infoDepartment = await ITEM__DEPARTMENT_COLL.findById(
                    departmentID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await ITEM__DEPARTMENT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await ITEM__DEPARTMENT_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await ITEM__DEPARTMENT_COLL.count(conditionObjOrg)
                let nextCursor = null
    ...
    getListByMembers({
        ...
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                if (unLimit) {
                    const dataUnlimit = await ITEM__DEPARTMENT_COLL.find(
                        conditionObj
                    )
                        .select(select)
                        .lean()
                ...
                const infoDataAfterGet = await ITEM__DEPARTMENT_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
            ...
            let dataBlock2 = await ITEM__DEPARTMENT_COLL.aggregate([
                ...
                },
                {
                    $group: {
                        ...
                        contractAdvancePaymentPaid: {
                            $sum: '$contractAdvancePaymentPaid',
                        },
