  ...
  update({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAfterInsert = await ITEM__CONTRACT_COLL.findByIdAndUpdate(
          contractID,
          dataUpdate,
          { new: true }
        )
  ...
  updateValue({ contractID, userID, option, ctx }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAfterUpdate = await ITEM__CONTRACT_COLL.findByIdAndUpdate(
          contractID,
          dataUpdate,
          { new: true }
        )
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (!checkObjectIDs(companyID) && !checkObjectIDs(projectID))
          return resolve({
            error: true,
            message: 'Bắt buộc phải truyền companyID hoặc projectID',
          ...
          let listData = await ITEM__CONTRACT_COLL.aggregate([
            ...
            },
            {
              $project: {
                ...
                month: { $month: '$date' },
                amount: {
                  $sum: ['$value', '$vatValue', '$plus', '$vatPlus'],
                },
              },
            },
            {
  ...
  getInventory({ userID, outin, companyID, projectID, contractID, ctx }) {
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (listData && listData.length) {
          inventory =
            Number(listData[0].produce) - Number(listData[0].acceptance)
        }
  ...
  exportContract({ userID, companyID, type, filterParams = {} }) {
    return new Promise(async (resolve) => {
      try {
        ...
        const listContract = await ITEM__CONTRACT_COLL.find(conditionObj)
          .populate({
