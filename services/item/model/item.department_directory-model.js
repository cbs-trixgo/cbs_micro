...
class Model extends BaseModel {
  ...
  insert({
    ...
    return new Promise(async (resolve) => {
      try {
        let infoData = await ITEM__DEPARTMENT_DIRECTORY_COLL.findOne({
        ...
        if (infoData)
          return resolve({
            error: true,
            message: 'member_exists_in_project',
          })
    ...
    return new Promise(async (resolve) => {
      try {
        if (!checkObjectIDs(departmentDirectoryID))
          return resolve({
            error: true,
            message: 'departmentDirectoryID không hợp lệ',
            keyError: 'Request params departmentDirectoryID invalid',
          })
        ...
        let infoAfterUpdate =
          await ITEM__DEPARTMENT_DIRECTORY_COLL.findByIdAndUpdate(
            departmentDirectoryID,
            dataUpdate,
            { new: true }
          )
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (!checkObjectIDs(departmentDirectoryID))
          return resolve({
            error: true,
            message: 'Request params departmentDirectoryID invalid',
            status: 400,
          })
         ...
         */
        let infoAterGet = await ITEM__DEPARTMENT_DIRECTORY_COLL.findById(
          departmentDirectoryID
        )
          .select(select)
          .populate(populates)
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (keyword) {
          ...
          let listMemberInDepartmentDirectory =
            await ITEM__DEPARTMENT_DIRECTORY_COLL.find({
              department: projectID,
            }).select('member')
          let membersID = listMemberInDepartmentDirectory.map(
            (item) => item.member
          )
          let membersIDByKeyword = await AUTH_USER_COLL.find({
            _id: { $in: membersID },
            fullname: new RegExp(keyword, 'i'),
          }).select('_id')
          conditionObj.member = {
            $in: membersIDByKeyword.map((item) => item._id),
          }
        }
        ...
        if (lastestID && checkObjectIDs(lastestID)) {
          let infoData =
            await ITEM__DEPARTMENT_DIRECTORY_COLL.findById(lastestID)
          if (!infoData)
            return resolve({
              error: true,
        ...
        let infoDataAfterGet = await ITEM__DEPARTMENT_DIRECTORY_COLL.find(
          conditionObj
        )
          .select(select)
          .limit(limit + 1)
        ...
        let totalRecord =
          await ITEM__DEPARTMENT_DIRECTORY_COLL.count(conditionObjOrg)
