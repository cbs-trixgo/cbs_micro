    ...
    insert({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (taxid && taxid != '') {
                    convertStr =
                        convertStr + ' ' + stringUtils.removeAccents(taxid)
                }
                if (email && email != '') {
                    convertStr =
                        convertStr + ' ' + stringUtils.removeAccents(email)
                }
                if (identity && identity != '') {
                    convertStr =
                        convertStr + ' ' + stringUtils.removeAccents(identity)
                }
                if (bankAccount && bankAccount != '') {
                    convertStr =
                        convertStr +
                        ' ' +
                        stringUtils.removeAccents(bankAccount)
                }
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                for (const item of listData) {
                    if (item._id.type == 1) {
                        amountWorkHistory =
                            Number(amountWorkHistory) + Number(item.count)
                    }
                    if (item._id.type == 2) {
                    ...
                    if (item._id.type == 5) {
                        amountCertificateHistory =
                            Number(amountCertificateHistory) +
                            Number(item.count)
                    }
                }
                ...
                dataUpdate.amountEducationHistory = Number(
                    amountEducationHistory
                )
                dataUpdate.amountCertificateHistory = Number(
                    amountCertificateHistory
                )
        ...
        return new Promise(async (resolve) => {
            try {
                if (contactID) {
                    ...
                    let infoAterGet = await ITEM__CONTACT_COLL.findById(
                        contactID
                    )
                        .select(select)
                        .populate(populates)
    ...
    getList({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet = await ITEM__CONTACT_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await ITEM__CONTACT_COLL.count(conditionObjOrg)
    ...
    getListByFilter({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                genders &&
                    genders.length &&
                    (conditionObj.gender = { $in: genders })
                departmentsID &&
            ...
            try {
                ...
                if (Number(duration / 86400) > 370) {
                    return resolve({
                        error: true,
                        message:
                            'Chỉ tra cứu được trong khoảng thời gian <= 370 ngày',
                    ...
                    if (!isNaN(option) && [2, 4].includes(Number(option))) {
                        await ITEM__CONTACT_COLL.populate(
                            listData,
                            conditionPopulate
                        )
                    }
                ...
                XlsxPopulate.fromFileAsync(
                    ...
                    listContact?.forEach((item, index) => {
                        ...
                        workbook
                            ...
                            .value(
                                Number(item.type) >= 1
                                    ? CONTACT_CLASSIFY[Number(item.type - 1)]
                                          .text
                                    : ''
                            )
                        workbook
                        ...
                        workbook
                            ...
                            .value(
                                Number(item.gender) >= 1
                                    ? CONTACT_GENDER[Number(item.gender - 1)]
                                          .text
                                    : ''
                            )
                        workbook
                        ...
                        workbook
                            ...
                            .value(
                                Number(item.status) >= 1
                                    ? PAYMENT_STATUS[Number(item.status - 1)]
                                          .text
                                    : ''
                            )
                        workbook
                        ...
                        workbook
                            ...
                            .row(i)
                            .cell(23)
                            .value(item?.insuranceSign)
                        workbook
                            .sheet('Export')
                            .row(i)
                            .cell(24)
                            .value(item?.insuranceDate)
                        workbook
                            .sheet('Export')
                            .row(i)
                            .cell(25)
                            .value(item?.sallaryBasic)
                        workbook
                            .sheet('Export')
                            .row(i)
                            .cell(26)
                ...
                if (option == 1) {
                    ...
                    for (const data of dataImportJSON) {
                        ...
                        if (index > 0 && index < 51) {
                            ...
                            if (
                                data?.__EMPTY &&
                                checkObjectIDs(data?.__EMPTY)
                            ) {
                                let infoAfterInsert =
                                    await this.update(prepareData)
                                if (infoAfterInsert.error) {
                                    errorNumber++
                                }
                            } else {
                                let infoAfterInsert =
                                    await this.insert(prepareData)
                                if (infoAfterInsert.error) {
                                    errorNumber++
                                }
                            }
