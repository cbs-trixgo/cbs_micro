 ...
 * TOOLS
 */
const { checkObjectIDs, IsJsonString, validateParamsObjectID, regexPhoneNumber } = require('../../../tools/utils/utils')
...
const FNB_ORDER_VOUCHER_MODEL                       = require('../../fnb/model/fnb.voucher-model').MODEL
const CONTACT_DOCUMENT_MODEL                        = require('../../human/model/human.contact_document-model').MODEL
const ANALYSIS__HISTORY_DATA_MODEL                  = require('../.../../../analysis/model/analysis.history_data').MODEL;
...
class Model extends BaseModel {
    ...
    insert({ authorID, parent, type, name, description, note, sign, address, taxid, birthday, gender, phone, email, department, position, linkUser, status, field, property, saleID, workingStatus, realStatus, insuranceStatus, familyPolicy, contractType, contractSign, family, identity, dateProvice, place, area, area1, area2, insuranceSign, insuranceDate, sallaryFactor, sallarySubFactor, sallaryBasic, signerIn, signerOut, workStartDate, dayOff, appointDate, changeSallaryDate, contractDate, contractValid, contractExpire, nonResident, fundaID, referrerID, bankAccount, dataSource, insuranceFee, ctx
        ...
        return new Promise(async resolve => {
            try {
                 ...
                 */
                if(!name || !phone || !checkObjectIDs(fundaID) || !checkObjectIDs(authorID))
                    return resolve({ error: true, message: 'name|phone|fundaID|authorID_param_not_valid' })
                ...
                if(Number(checkPhone) < 10)
                    return resolve({ error: true, message: 'Số điện thoại phải đúng 10 ký tự' })

                let infoFunda = await ITEM__FUNDA_COLL.findById(fundaID)
                if(!infoFunda)
                    return resolve({ error: true, message: 'can_not_get_funda_info' })
                ...
                if((sign && sign != "") || (taxid && taxid != "") || (phone && phone != "")){
                    conditionObj.$or = []
                }
                if((sign && sign != "")){
                ...
                if(conditionObj.$or && conditionObj.$or.length > 0 && infoContact)
                    ...
                    return resolve({ error: true, message: `Mã hiệu/Mã số thuế/Điện thoại đã tồn tại - ${infoContact.name} - ${infoContact.phone}` });
                ...
                if(phone && phone != ""){
                    convertStr = convertStr + " " + stringUtils.removeAccents(phone.replace(/\s/g, ""))
                }
                if(taxid && taxid != ""){
                ...
                if(linkUser && checkObjectIDs(linkUser)){
                    ...
                    let checkExist = await ITEM__CONTACT_COLL.findOne({linkUser: linkUser, company: infoFunda.company})
                    if(!checkExist){
                ...
                if(parent && checkObjectIDs(parent)){
                    ...
                    if(!infoAppUser.error && infoAppUser.data.level == 0){
                        ...
                        if(infoParent.dataSource && checkObjectIDs(infoParent.dataSource)){
                ...
                if(parent && checkObjectIDs(parent)){
                    ...
                    if(!infoAppUser.error && infoAppUser.data.level == 0){
                         ...
                         * Phần tử cha cấp 1
                         */
                        await ITEM__CONTACT_COLL.findByIdAndUpdate(parent,
                            {
                                $addToSet: {
                        ...
                        for(const item of infoParent?.nestedParents){
                            ...
                            await ITEM__CONTACT_COLL.findByIdAndUpdate(item,
                                {
                                    $addToSet: { nestedChilds: infoAfterInsert._id },
                                },
                                { new: true })

                            // Thêm các phần tử cha
                            await ITEM__CONTACT_COLL.findByIdAndUpdate(infoAfterInsert._id ,
                                {
                ...
                await ITEM__FUNDA_COLL.findByIdAndUpdate(fundaID, { $inc: { numberOfCustomers: 1 }}, { new: true })
     ...
     * Code:
     */
    update({contactID, company, authorID, parent, type, name, description, note, sign, address, taxid, birthday, gender, phone, email, department, position, linkUser, status, field, property, saleID, workingStatus, realStatus, insuranceStatus, familyPolicy, contractType, contractSign, family, identity, dateProvice, place, area, area1, area2, insuranceSign, insuranceDate, sallaryFactor, sallarySubFactor, sallaryBasic, signerIn, signerOut, workStartDate, dayOff, appointDate, changeSallaryDate, contractDate, contractValid, contractExpire, nonResident, fundaID, bankAccount, dataSource, insuranceFee, union, share, image, documents, documentsRemove, purchasedValue, totalLoyaltyPoints, usedLoyaltyPoints, remainLoyaltyPoints, getVouchers, usedVouchers}){
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(linkUser && checkObjectIDs(linkUser)){
                    // Kiểm tra xem đã tồn tại hay chưa, nếu tồn tại rồi thì không cho link nữa
                    let checkExist = await ITEM__CONTACT_COLL.findOne({linkUser: linkUser, company: company})
                ...
                let infoAfterUpdate = await ITEM__CONTACT_COLL.findByIdAndUpdate(contactID, dataUpdate, { new: true })
                if(!infoAfterUpdate)
                    return resolve({ error: true, message: "Cập nhật thất bại" })

                // Cập nhật phần tử cha
                if(parent && checkObjectIDs(parent) && contactID.toString() != parent.toString()){
                    //___________Quyền truy cập ứng dụng Nhân sự
                    let infoAppUser = await AUTH__APP_USER.checkPermissionsAccessApp({
                        appID: "61e049c9fdebf77b072d1b13",
                        userID: authorID
    ...
    updateValue({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let amountWorkHistory = 0, amountProjectHistory = 0, amountContractHistory = 0, amountEducationHistory = 0, amountCertificateHistory = 0
                ...
                for(const item of listData){
                    ...
                    if(item._id.type == 2){
                        amountProjectHistory = Number(amountProjectHistory) + Number(item.count)
                    }
                    if(item._id.type == 3){
                        amountContractHistory = Number(amountContractHistory) + Number(item.count)
                    }
                    if(item._id.type == 4){
                        amountEducationHistory = Number(amountEducationHistory) + Number(item.count)
                    }
                    if(item._id.type == 5){
                        amountCertificateHistory = Number(amountCertificateHistory) + Number(item.count)
                    }
                }
            ...
            try {
                if(contactID){
                    if(!checkObjectIDs(contactID))
                        return resolve({ error: true, message: 'Request params contactID invalid', status: 400 })
                    ...
                    if(!checkObjectIDs(linkUserID))
                        return resolve({ error: true, message: 'Request params linkUserID invalid', status: 400 })

                    let info = await ITEM__CONTACT_COLL.findOne({ linkUser: linkUserID })
                    if(info){
                        if(populates && typeof populates === 'string'){
                            if(!IsJsonString(populates))
                                return resolve({ error: true, message: 'Request params populates invalid', status: 400 })
     ...
     * Code  : 30/7/2023
     */
    getList({ option, companyID, userID, uplineID, contacts, isBirthday, fundaID, voucherID, parentID, type, fromDate, toDate, keyword, limit = 5, lastestID, select, populates, month, ctx }) {
        return new Promise(async (resolve) => {
            try {
                /**
                 * BA
     ...
     */
    getListByFilter({
        parentID, companyID, receiverID, types, isExportExcel = false,
        requestResponse, fromDate, toDate, marked, userID, departmentsID,
        keyword, limit = 20, lastestID, select, populates= {}, isParent, type, genders, ctx }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                departmentsID && departmentsID.length           && (conditionObj.department     = { $in: departmentsID });
                types && types.length                           && (conditionObj.type           = { $in: types });

                // Ngày làm việc
                if(fromDate && toDate){
                    conditionObj.workStartDate = { $gte: new Date(fromDate), $lt: moment(toDate, 'YYYY-MM-DD').endOf('day')._d }
                }
     ...
     */
    getListOfSystem({
        keyword, limit = 5, lastestID, select, populates= {}, ctx }) {
        return new Promise(async (resolve) => {
            try {
                let conditionObj = { };
                let keys	     = ['createAt__-1', '_id__-1'];
     ...
     * Code: 30/7/2023
     */
    getListByProperty({ companyID, option, optionGroup, optionTime, fundasID, uplineID, year, fromDate, toDate, userID, contacts, ctx }) {
        ...
        return new Promise(async (resolve) => {
            ...
            try {
                ...
                if(Number(duration/86400) > 370){
                     return resolve({ error: true, message: 'Chỉ tra cứu được trong khoảng thời gian <= 370 ngày', keyError: KEY_ERROR.PARAMS_INVALID })
                }else{
                    let conditionObj = {
                        company: ObjectID(companyID)
                    }
                    let conditionGroup = {}, conditionProject = {}
                    ...
                    // Gom nhóm theo Tuyến dưới
                        ...
                        // Theo user đang truy cập
                        if(optionGroup && optionGroup == 1 && contacts && contacts.length){
                            let listContacts = contacts.map(item => ObjectID(item._id))
    ...
    getListAccessByContract({
        userID, option,
        keyword, limit = 10, lastestID, select, populates = {}, ctx }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let listContractAccess = await ITEM__CONTRACT_COLL.find({members: {$in: [userID]}}).select('name sign buyerInfo sellerInfo')
                ...
                if(option && option == 1){
                    // console.log('===========Bên bán================')
                    conditionObj._id = {$in: listContractAccess.map(item=>item.sellerInfo)}
                }

                if(option && option == 2){
                    // console.log('===========Bên mua================')
                    conditionObj._id = {$in: listContractAccess.map(item=>item.buyerInfo)}
                }
     ...
     downloadTemplateImportExcel({ companyID, userID, ctx }) {
        return new Promise(async resolve => {
            try {
                ...
                let listDepartment = await ITEM__DEPARTMENT_COLL.find({ company: companyID }).select('name').limit(500)
                let listPosition = await ITEM__POSITION_COLL.find({ company: companyID }).select('name').limit(500)
                let listDoctype = await ITEM__DOCTYPE_COLL.find({ company: companyID }).select('name').limit(500)
                let listUser = await USER_COLL.find({ company: companyID }).select('fullname').limit(500)
                let listFunda = await ITEM__FUNDA_COLL.find({ company: companyID }).select('name').limit(500)
                let listContract = await ITEM__CONTRACT_COLL.find({ company: companyID }).select('name sign').limit(500)
                let listArea = await ITEM__AREA_COLL.find({ level: 3 }).select('name parent')
                .populate({
                    path: 'parent',
                    select:'_id  name',
                    populate:{
                ...
                let listContact = await ITEM__CONTACT_COLL.find({ company: companyID }).limit(1000)
                .populate({
                ...
                // Modify the workbook.
                XlsxPopulate.fromFileAsync(path.resolve(__dirname, ('../../../files/templates/excels/contact_import_template.xlsm')))
                .then(async workbook => {

                    var i = 3;
                    listContact?.forEach((item, index) => {
                        workbook.sheet("Export").row(i).cell(1).value(`${item._id}`)
                        workbook.sheet("Export").row(i).cell(2).value(Number(item.type) >= 1 ? CONTACT_CLASSIFY[Number(item.type - 1)].text : '')
                        workbook.sheet("Export").row(i).cell(3).value(Number(item.subtype) >= 1 ? HUMAN_CLASSIFY[Number(item.type - 1)].text : '')
                        ...
                        workbook.sheet("Export").row(i).cell(9).value(Number(item.gender) >= 1 ? CONTACT_GENDER[Number(item.gender - 1)].text : '')
                        ...
                        workbook.sheet("Export").row(i).cell(13).value(item?.department?.name)
                        ...
                        workbook.sheet("Export").row(i).cell(17).value(Number(item.status) >= 1 ? PAYMENT_STATUS[Number(item.status - 1)].text : '')
                        ...
                        workbook.sheet("Export").row(i).cell(22).value(item?.insuranceNumber)
                        ...
                        workbook.sheet("Export").row(i).cell(27).value(Number(item.insuranceStatus) >= 1 ? INSURANCE_STATUS[Number(item.insuranceStatus - 1)].text : '')
                        ...
                        workbook.sheet("Export").row(i).cell(34).value(item?.contractType?.name)
                        workbook.sheet("Export").row(i).cell(35).value(item?.contractSign)
                        workbook.sheet("Export").row(i).cell(36).value(item?.workingStatus?.name)
                        ...
                        workbook.sheet("Export").row(i).cell(40).value(item?.changeSallaryDate)
                        ...
                        workbook.sheet("Export").row(i).cell(44).value(item?.area? `${item?.area?.name},${item?.area?.parent?.name},${item?.area?.parent?.parent?.name}` : '')
                        workbook.sheet("Export").row(i).cell(45).value(item?.area1? `${item?.area1?.name},${item?.area1?.parent?.name},${item?.area1?.parent?.parent?.name}` : '')
                        workbook.sheet("Export").row(i).cell(46).value(item?.area2?`${item?.area2?.name},${item?.area2?.parent?.name},${item?.area2?.parent?.parent?.name}` : '')
                        workbook.sheet("Export").row(i).cell(47).value(item?.sale?.fullname)
                        workbook.sheet("Export").row(i).cell(48).value(item?.linkUser?.fullname)
                        workbook.sheet("Export").row(i).cell(49).value(item?.family)
                        workbook.sheet("Export").row(i).cell(50).value(Number(item.familyPolicy) >= 1 ? CONTACT_POLICY[Number(item.familyPolicy - 1)].text : '')
                        workbook.sheet("Export").row(i).cell(51).value(Number(item.realStatus) >= 1 ? PERSONAL_STATUS[Number(item.realStatus - 1)].text : '')
                        workbook.sheet("Export").row(i).cell(52).value(Number(item.new) >= 1 ? CONTACT_NEW[Number(item.new - 1)].text : '')
                    ...
                    listArea?.forEach((item, index) => {
                        ...
                        workbook.sheet("DanhMuc").row(i).cell(26).value(`${item.name},${item?.parent.name},${item?.parent?.parent.name}`)
                        i++
                    })
                ...
                // IMPORT HỒ SƠ
                    ...
                    for (const data of dataImportJSON) {
                        ...
                        if(index > 0 && index <= 100){
                            ...
                            let infoAfterInsert = await CONTACT_DOCUMENT_MODEL.insert(dataInsert);
                            if(infoAfterInsert.error){
                                errorNumber++;
                            }
    ...
    downloadExcelFile(data) {
        return new Promise(resolve => {
            XlsxPopulate.fromFileAsync(path.resolve(__dirname, ('../../../files/templates/excels/contact_export.xlsx')))
                .then(async workbook => {

                    let i = 3;
                    data?.forEach((item, index) => {
                        workbook.sheet("NhanSu").row(i).cell(1).value(Number(index+1));
                        ...
                        workbook.sheet("NhanSu").row(i).cell(6).value(item.gender > 1 ? `${CONTACT_GENDER[Number(item.gender)-1].text}` : "");
                        ...
                        workbook.sheet("NhanSu").row(i).cell(12).value((item?.area2 != null && item?.area2 != undefined) ? (`${item?.area2?.name}, ${item?.area2?.parent?.name}, ${item?.area2?.parent?.parent?.name}`) : "");
                        workbook.sheet("NhanSu").row(i).cell(13).value((item?.area != null && item?.area != undefined) ? (`${item.address},${item?.area?.name}, ${item?.area?.parent?.name}, ${item?.area?.parent?.parent?.name}`) : "");
                        workbook.sheet("NhanSu").row(i).cell(14).value(item.taxid);
                        workbook.sheet("NhanSu").row(i).cell(15).value((!isNaN(item.insuranceNumber) && item.insuranceNumber != 0) ? Number(item.insuranceNumber) : 0);
                        workbook.sheet("NhanSu").row(i).cell(16).value(item.insuranceSign);
                        workbook.sheet("NhanSu").row(i).cell(17).value(item.insuranceDate);
                        workbook.sheet("NhanSu").row(i).cell(18).value((!isNaN(item.sallaryBasic) && item.sallaryBasic != 0) ? Number(item.sallaryBasic) : 0);
                        ...
                        workbook.sheet("NhanSu").row(i).cell(23).value(item?.contractType?.name);
