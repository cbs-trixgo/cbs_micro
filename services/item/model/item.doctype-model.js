...
class Model extends BaseModel {
     ...
     * Date: 22/02/2022
     */
    insert({ type, name, description, sign, unit, amount, alert, companyID, parent, linkItemID, level, userID, salesChannel }) {
        // console.log({ type, name, description, sign, unit, amount, companyID, parent, linkItemID, level, userID, salesChannel })
        return new Promise(async resolve => {
            try {
                if(!name || !type || !DOCTYPE_TYPE.includes(+type) || !checkObjectIDs([userID]) || !checkObjectIDs([companyID])) {
                    return resolve({
						error: true,
                ...
                let dataInsert = { name, type, description, sign, unit, company: companyID, userCreate: userID, userUpdate: userID, modifyAt: new Date() }
     ...
     * Date: 22/02/2022
     */
    update({ doctypeID, name, description, sign, unit, amount, alert, userID, salesChannel, linkItemID }) {
        ...
        return new Promise(async resolve => {
            try {
                ...
                let infoAfterUpdate = await ITEM__DOCTYPE_COLL.findByIdAndUpdate(doctypeID, dataUpdate, { new: true });
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(!checkObjectIDs(doctypeID))
                    return resolve({ error: true, message: 'Request params doctypeID invalid', status: 400 });
                 ...
                 */
                let infoAterGet = await ITEM__DOCTYPE_COLL
                    .findById(doctypeID)
                    .select(select)
                    .populate(populates)
                    .lean();
     ...
     * Code  :
     */
    getList({ companyID, projectID, parentID, type, keyword, limit = 10, lastestID, select, populates, isShowParentAndChild }) {
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet = await ITEM__DOCTYPE_COLL
                    .find(conditionObj)
                    .limit(limit+1)
                    .sort(sortBy)
                    .select(select)
