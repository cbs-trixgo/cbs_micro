        ...
        return new Promise(async resolve => {
            try {
                ...
                if(parentID && checkObjectIDs(parentID)){
                    let infoMenuParent = await ITEM__AREA_COLL.findById(parentID);
                ...
                if(parentID) {
                    await ITEM__AREA_COLL.findByIdAndUpdate(parentID, {
    ...
    update({ areaID, name, sign, userID }){
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate = await ITEM__AREA_COLL.findByIdAndUpdate(areaID, dataUpdate, { new: true });
                if(!infoAfterUpdate)
        ...
        return new Promise(async (resolve) => {
            try {
                if(!checkObjectIDs(areaID))
                    return resolve({ error: true, message: 'Request params areaID invalid', status: 400 });
    ...
    getList({ keyword, limit = 10, lastestID, select, populates= {} }) { 
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoDataAfterGet = await ITEM__AREA_COLL
                    .find(conditionObj)
                    .select(select)
                    .limit(limit+1)
                    .sort(sortBy)
