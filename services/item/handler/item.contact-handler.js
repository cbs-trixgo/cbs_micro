...
module.exports = {
    ...
    insert: {
        ...
        async handler(ctx) {
            try {
                ...
                let { parent, type, name, description, note, sign, address, taxid, birthday, province, gender, phone, email, department, position, linkUser, status, field, property, linkSale, workingStatus, realStatus, insuranceStatus, familyPolicy, contractType, contractSign, family, identity, dateProvice, place, area, area1, area2, insuranceSign, insuranceDate, sallaryFactor, sallarySubFactor, sallaryBasic, signerIn, signerOut, workStartDate, dayOff, appointDate, changeSallaryDate, contractDate, contractValid, contractExpire, nonResident, fundaID, referrerID, saleID, bankAccount, dataSource } = ctx.params;
				const { _id: userID, company } = ctx.meta.infoUser;

                let resultAfterCallHandler = await ITEM__CONTACT_MODEL.insert({ authorID: userID, parent, type, name, description, note, sign, address, taxid, birthday, province, gender, phone, email, department, position, linkUser, status, field, property, linkSale, workingStatus, realStatus, insuranceStatus, familyPolicy, contractType, contractSign, family, identity, dateProvice, place, area, area1, area2, insuranceSign, insuranceDate, sallaryFactor, sallarySubFactor, sallaryBasic, signerIn, signerOut, workStartDate, dayOff, appointDate, changeSallaryDate, contractDate, contractValid, contractExpire, nonResident, fundaID, referrerID, saleID, bankAccount, dataSource, ctx });
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { contactID, authorID, parent, type, name, description, note, sign, address, taxid, birthday, gender, phone, email, department, position, linkUser, status, field, property, saleID, workingStatus, realStatus, insuranceStatus, familyPolicy, contractType, contractSign, family, identity, dateProvice, place, area, area1, area2, insuranceSign, insuranceDate, sallaryFactor, sallarySubFactor, sallaryBasic, signerIn, signerOut, workStartDate, dayOff, appointDate, changeSallaryDate, contractDate, contractValid, contractExpire, nonResident, fundaID, bankAccount, dataSource, insuranceFee, union, share, image, amountWorkHistory, amountProjectHistory, amountContractHistory, amountEducationHistory, amountCertificateHistory, documents, documentsRemove, purchasedValue, totalLoyaltyPoints, usedLoyaltyPoints, remainLoyaltyPoints, getVouchers, usedVouchers } = ctx.params;
				const { _id: userID, company } = ctx.meta.infoUser;

                // console.log(ctx.params)

                let resultAfterCallHandler = await ITEM__CONTACT_MODEL.update({contactID, company: company._id, authorID: userID, parent, type, name, description, note, sign, address, taxid, birthday, gender, phone, email, department, position, linkUser, status, field, property, saleID, workingStatus, realStatus, insuranceStatus, familyPolicy, contractType, contractSign, family, identity, dateProvice, place, area, area1, area2, insuranceSign, insuranceDate, sallaryFactor, sallarySubFactor, sallaryBasic, signerIn, signerOut, workStartDate, dayOff, appointDate, changeSallaryDate, contractDate, contractValid, contractExpire, nonResident, fundaID, bankAccount, dataSource, insuranceFee, union, share, image, amountWorkHistory, amountProjectHistory, amountContractHistory, amountEducationHistory, amountCertificateHistory, documents, documentsRemove, purchasedValue, totalLoyaltyPoints, usedLoyaltyPoints, remainLoyaltyPoints, getVouchers, usedVouchers, ctx});
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let {
                    option, companyID, uplineID, fundaID, voucherID, contactID, type, isBirthday, parentID, month,
                    keyword, limit, lastestID, select, populates, fromDate, toDate, linkUserID
                } = ctx.params;
                ...
                if(contactID || linkUserID){
                    resultAfterCallHandler = await ITEM__CONTACT_MODEL.getInfo({
                        contactID, linkUserID, select, populates, ctx
                    });
                }else{
                    resultAfterCallHandler = await ITEM__CONTACT_MODEL.getList({
                        option, companyID, userID, uplineID, contacts, isBirthday, fundaID, voucherID, parentID, type, fromDate, toDate, month, keyword, limit, lastestID, select, populates, ctx
                    })
                }
    ...
    getListByFilter: {
        ...
        async handler(ctx) {
            try {
                ...
                let {
                    parentID, companyID, receiverID, types, isExportExcel, requestResponse, fromDate, toDate, marked, departmentsID, keyword, limit, lastestID, select, populates, isParent, type, genders
                } = ctx.params;
                ...
                const resultAfterCallHandler = await ITEM__CONTACT_MODEL.getListByFilter({
                    userID, parentID, companyID, receiverID, types, isExportExcel, requestResponse, fromDate, toDate, marked, departmentsID, keyword, limit, lastestID, select, populates, isParent, type, genders, ctx
                });
    ...
    getListByProperty: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
				const { _id: userID, contacts, company } = ctx.meta.infoUser;
                let { option, optionGroup, optionTime, fundasID, uplineID, year, fromDate, toDate,
                     keyword, limit, lastestID, select, populates, sortKey } = ctx.params;

                let resultAfterCallHandler = await ITEM__CONTACT_MODEL.getListByProperty({
                    companyID: company._id,
                    option, optionGroup, optionTime, fundasID, uplineID, year, fromDate, toDate,
    ...
    getListOfSystem: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let {
                    keyword, limit, lastestID, select, populates } = ctx.params;
                let resultAfterCallHandler = await ITEM__CONTACT_MODEL.getListOfSystem({ keyword, limit, lastestID, select, populates, ctx });
    ...
    getListAccessByContract: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await ITEM__CONTACT_MODEL.getListAccessByContract({
                    option,
                    keyword, limit, lastestID, select, populates,
                    userID, ctx
                })
    ...
    downloadTemplateImportExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await ITEM__CONTACT_MODEL.downloadTemplateImportExcel({
                    companyID: company._id, userID, ctx
                })
    ...
    importFromExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await ITEM__CONTACT_MODEL.importFromExcel({
                    companyID: company._id, userID, dataImport, option, ctx
                })
                return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx });
            } catch (error) {
                return renderStatusCodeAndResponse({ error_message: error.message, ctx });
    ...
    exportExcel: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await ITEM__CONTACT_MODEL.exportExcel({
                    userID, companyID, filterParams, ctx, queue: this.metadata.queue
                })
    ...
    exportExcelByFilter: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await ITEM__CONTACT_MODEL.exportExcelByFilter({
