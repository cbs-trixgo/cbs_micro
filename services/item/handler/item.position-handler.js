...
module.exports = {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await ITEM__POSITION_MODEL.insert({
                    name, description, companyID, parent, userID
                });
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await ITEM__POSITION_MODEL.update({
                    positionID, name, description, userID
                });
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { companyID, positionID,  keyword, limit, lastestID, select, populates } = ctx.params;
                ...
                if(positionID){
                    resultAfterCallHandler = await ITEM__POSITION_MODEL.getInfo({
                        positionID, select, populates
                    });
                }else{
                    resultAfterCallHandler = await ITEM__POSITION_MODEL.getList({
                        companyID, lastestID, keyword, select, limit, populates
                    });
                }
                return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx });
            } catch (error) {
