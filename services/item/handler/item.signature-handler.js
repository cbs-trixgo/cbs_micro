...
module.exports = {
    ...
    insert: {
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await ITEM__SIGNATURE_MODEL.insert({
                    companyID, projectID, coll, title, note, authorID
                });
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { signatureID, companyID, projectID, keyword, limit, lastestID, select, populates } = ctx.params;
				let { _id: authorID } = ctx.meta.infoUser;

                let resultAfterCallHandler;
                if(signatureID){
                    resultAfterCallHandler = await ITEM__SIGNATURE_MODEL.getInfo({
                        signatureID, select, populates
                    });
                } else{
                    resultAfterCallHandler = await ITEM__SIGNATURE_MODEL.getList({
                        companyID, projectID, lastestID, keyword, limit, select, populates, authorID
                    });
                }
