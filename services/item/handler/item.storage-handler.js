...
module.exports = {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler = await ITEM__STORAGE_MODEL.insert({
                    companyID: company._id, projectID, parentID, name, description, userID, ctx
                });
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler = await ITEM__STORAGE_MODEL.update({
                    storageID, name, description, userID
                });
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { companyID, storageID, projectID, parentID, isListParentOfListChilds, keyword, limit, lastestID, select, populates } = ctx.params;
				const { _id: userID, company } = ctx.meta.infoUser;
                if(!companyID){
                ...
                if(storageID){
                    resultAfterCallHandler = await ITEM__STORAGE_MODEL.getInfo({
                        storageID, select, populates
                    });
                }else{
                    resultAfterCallHandler = await ITEM__STORAGE_MODEL.getList({
                        companyID, projectID, parentID, isListParentOfListChilds, lastestID, keyword, limit, select, populates
                    });
                }
                return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx });
            } catch (error) {
