  ...
  getListNestedItem: {
    ...
    async handler(ctx) {
      try {
        await this.validateEntity(ctx.params)
        const { _id: userID, company } = ctx.meta.infoUser
        const { accountID, status, showInReportMonth, isNestedOfCompany } =
          ctx.params
        ...
        if (!isNestedOfCompany) {
          resultAfterCallHandler = await ITEM__ACCOUNT_MODEL.getListByCompany({
            companyID: company._id,
            status,
            showInReportMonth,
          })
        ...
        if (isNestedOfCompany) {
          resultAfterCallHandler = await ITEM__ACCOUNT_MODEL.getListNestedItem({
            accountID,
          })
        }
        return renderStatusCodeAndResponse({
  ...
  getInfoAccount: {
    ...
    async handler(ctx) {
      try {
        ...
        const resultAfterCallHandler = await ITEM__ACCOUNT_MODEL.getInfoAccount(
          { accountID }
        )
        return renderStatusCodeAndResponse({
          resultAfterCallHandler,
  ...
  getListWithKey: {
    ...
    async handler(ctx) {
      try {
        ...
        const { _id: userID, company } = ctx.meta.infoUser
        const resultAfterCallHandler = await ITEM__ACCOUNT_MODEL.getListWithKey(
          {
            companyID: company._id,
            key,
            limit,
