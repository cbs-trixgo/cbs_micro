...
module.exports = {
    ...
    insert:{
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await ITEM__WAREHOUSE_MODEL.insert({ companyID: company._id, parentID, name, description, userID });
    ...
    update:{  
        ...
        async handler(ctx) {
            try {
                ...
                const resultAfterCallHandler = await ITEM__WAREHOUSE_MODEL.update({ warehouseID, name, description, userID });
                return renderStatusCodeAndResponse({ resultAfterCallHandler, ctx });
            } catch (error) {
                return { error: true, message: error.message };
            }
    ...
    getInfoAndGetList: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params);
                let { companyID, warehouseID,  keyword, limit, lastestID, select, populates, parentID, isListParentOfListChilds, } = ctx.params;
				const { _id: userID, company } = ctx.meta.infoUser;
                ...
                if(warehouseID){
                    resultAfterCallHandler = await ITEM__WAREHOUSE_MODEL.getInfo({
                        warehouseID, select, populates
                    });
                }else{
                    resultAfterCallHandler = await ITEM__WAREHOUSE_MODEL.getList({
                        companyID, lastestID, keyword, limit, select, populates, parentID, isListParentOfListChilds,
                    });
                }
