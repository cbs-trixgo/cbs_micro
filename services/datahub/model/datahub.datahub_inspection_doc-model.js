        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!type)
                    return resolve({
                        error: true,
                        message: 'Loại datahub inspection không hợp lệ',
                        keyError: KEY_ERROR.PARAMS_INVALID,
                    })
                ...
                if (sign) {
                    let checkSign = await DATAHUB_INSPECTION_DOC_COLL.findOne({
                        sign,
                    })
                    if (checkSign)
                        return resolve({
    ...
    update({ inspectionDocID, name, sign, description, userID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await DATAHUB_INSPECTION_DOC_COLL.findByIdAndUpdate(
                        inspectionDocID,
                        dataUpdate,
                        { new: true }
                    )
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!checkObjectIDs(inspectionDocID))
                    return resolve({
                        error: true,
                        message: 'Request params inspectionDocID invalid',
                        status: 400,
                    })
                 ...
                 */
                let infoAterGet = await DATAHUB_INSPECTION_DOC_COLL.findById(
                    inspectionDocID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await DATAHUB_INSPECTION_DOC_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let listPackages = await DATAHUB_INSPECTION_DOC_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .select(select)
                ...
                // GET TOTAL RECORD
                let totalRecord =
                    await DATAHUB_INSPECTION_DOC_COLL.count(conditionObjOrg)
