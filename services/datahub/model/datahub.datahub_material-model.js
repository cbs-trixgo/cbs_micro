...
class Model extends BaseModel {
        ...
        return new Promise(async resolve => {
            try {
                if(!checkObjectIDs(company))
                    return resolve({ error: true, message: "company invalid", keyError: 'params_invalid' });
    ...
    update({ datahubProfileID, name, files, filesRemove, userID  }) {
        return new Promise(async resolve => {
            try {
                ...
                let infoAfterUpdate = await DATAHUB_MATERIAL_COLL.findByIdAndUpdate(datahubProfileID, dataUpdate, { new: true });
                if (!infoAfterUpdate)
    ...
    //             let infoAterRemove = await DATAHUB_MATERIAL_COLL.findByIdAndDelete(datahubProfileID)
    //             if(!infoAterRemove)
	...
	getInfo({ datahubProfileID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                if(!checkObjectIDs(datahubProfileID))
                    return resolve({ error: true, message: 'Request params datahubProfileID invalid', status: 400 });
                 ...
                 */
                let infoAterGet = await DATAHUB_MATERIAL_COLL.findById(datahubProfileID)
                                    .select(select)
                                    .populate(populates)
                if(!infoAterGet)
        ...
        return new Promise(async resolve => {
            try {
                if(!checkObjectIDs(userID) || !checkObjectIDs(contractorID))
                    return resolve({ error: true, message: 'Request params userID|contractorID invalid', status: 400 });
				...
				if(lastestID && checkObjectIDs(lastestID)){
					let infoData = await DATAHUB_MATERIAL_COLL.findById(lastestID);
					if(!infoData)
                ...
                let listPackages = await DATAHUB_MATERIAL_COLL
					.find(conditionObj)
					.limit(limit + 1)
					.select(select)
					.populate(populates)
				...
				// GET TOTAL RECORD
                let totalRecord = await DATAHUB_MATERIAL_COLL.count(conditionObjOrg);
