    ...
    update({ datahubProfileID, name, files, filesRemove, userID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await DATAHUB_MATERIAL_COLL.findByIdAndUpdate(
                        datahubProfileID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    getInfo({ datahubProfileID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                let infoAterGet = await DATAHUB_MATERIAL_COLL.findById(
                    datahubProfileID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await DATAHUB_MATERIAL_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let listPackages = await DATAHUB_MATERIAL_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .select(select)
                ...
                // GET TOTAL RECORD
                let totalRecord =
                    await DATAHUB_MATERIAL_COLL.count(conditionObjOrg)
