        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (info) {
                    ...
                    // Lấy giá của vật tư
                    let infoProduct =
                        await DATAHUB_PRODUCT_COLL.findById(productID)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await DATAHUB_JOBLINE_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await DATAHUB_JOBLINE_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await DATAHUB_JOBLINE_COLL.count(conditionObjOrg)
