        ...
        return new Promise(async (resolve) => {
            try {
                if (!checkObjectIDs(contractor))
                    return resolve({
                        error: true,
                        message: 'contractor Không hợp lệ',
                ...
                if (grossConstructionRevenue) {
                    dataInsert.grossConstructionRevenue =
                        grossConstructionRevenue
                }
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (grossConstructionRevenue) {
                    dataUpdate.grossConstructionRevenue =
                        grossConstructionRevenue
                }
                ...
                let infoAfterUpdate =
                    await DATAHUB_FINDREPORT_COLL.findByIdAndUpdate(
                        datahubFinreportID,
                        dataUpdate,
                        { new: true }
                    )
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!checkObjectIDs(datahubFinreportID))
                    return resolve({
                        error: true,
                        message: 'Request params datahubFinreportID invalid',
                        status: 400,
                    })
                 ...
                 */
                let infoAterGet = await DATAHUB_FINDREPORT_COLL.findById(
                    datahubFinreportID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await DATAHUB_FINDREPORT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let listPackages = await DATAHUB_FINDREPORT_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .select(select)
                ...
                // GET TOTAL RECORD
                let totalRecord =
                    await DATAHUB_FINDREPORT_COLL.count(conditionObjOrg)
