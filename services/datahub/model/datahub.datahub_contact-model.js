    ...
    insert({ name, sign, address, taxid, note, userID }) {
        return new Promise(async resolve => {
            try {
                ...
                if(info)
                    return resolve({ error: true, message: "Mã số thuế đã tồn tại", keyError: 'cannot_insert' });
    ...
    update({ datahubContactID, name, sign, address, taxid, note, userID  }) {
        return new Promise(async resolve => {
            try {
                ...
                let info = await DATAHUB_CONTACT_COLL.findOne({ taxid: taxid, _id: { $nin: [datahubContactID] } })
                if(info)
                    return resolve({ error: true, message: "Mã số thuế đã tồn tại", keyError: 'cannot_update' })
                ...
                let infoAfterUpdate = await DATAHUB_CONTACT_COLL.findByIdAndUpdate(datahubContactID, dataUpdate, { new: true });
                if (!infoAfterUpdate) 
                    return resolve({ error: true, message: "Cập nhật thất bại", keyError: "cannot_update", status: 403 });
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if(lastestID && checkObjectIDs(lastestID)){
					let infoData = await DATAHUB_CONTACT_COLL.findById(lastestID);
					if(!infoData)
                ...
                let infoDataAfterGet = await DATAHUB_CONTACT_COLL.find(conditionObj)
                    .limit(limit+1)
                    .sort(sortBy)
                    .select(select)
                    .populate(populates)
                ...
                let totalRecord = await DATAHUB_CONTACT_COLL.count(conditionObjOrg);
