    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let dataInsert = {
          ...
          unitprice:
            unitprice && !isNaN(unitprice) ? Number(unitprice).toFixed(3) : 0,
        }
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (Number(list.length) > 500)
          return resolve({
            error: true,
            message: 'the_number_is_limited_>500',
          })
          ...
          for (const item of list) {
            ...
            if (id && _isValid(id)) {
              let dataUpdate = {
                ...
                unit: item[4],
                unitprice:
                  item[5] && !isNaN(item[5]) ? Number(item[5]).toFixed(3) : 0,
                note: item[6],
                type:
                  item[7] && !isNaN(item[7]) ? Number(item[7]).toFixed(0) : 1,
              }

              await DATAHUB_PRODUCT_COLL.findByIdAndUpdate(id, dataUpdate, {
                new: true,
              })
            } else {
              let dataInsert = {
  ...
  updateJobPrice({ jobID }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAfterUpdate = await DATAHUB_JOB_COLL.findByIdAndUpdate(
          jobID,
          {
      ...
      try {
        ...
        for (const item of data) {
          if (item.id && _isValid(item.id)) {
            ...
            // Cập nhật product
            await DATAHUB_PRODUCT_COLL.findByIdAndUpdate(item.id, dataUpdate, {
              new: true,
            })
  ...
  update({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAfterUpdate = await DATAHUB_PRODUCT_COLL.findByIdAndUpdate(
          productID,
          dataUpdate,
          { new: true }
        )
