                    ...
                    for (const item of list) {
                        ...
                        if (id && _isValid(id)) {
                            let dataUpdate = {
                                ...
                                unitprice:
                                    item[5] && !isNaN(item[5])
                                        ? Number(item[5]).toFixed(3)
                                        : 0,
                                note: item[6],
                                type:
                                    item[7] && !isNaN(item[7])
                                        ? Number(item[7]).toFixed(0)
                                        : 1,
                            }

                            await DATAHUB_PRODUCT_COLL.findByIdAndUpdate(
                                id,
                                dataUpdate,
                                { new: true }
                            )
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await DATAHUB_PRODUCT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let infoDataAfterGet = await DATAHUB_PRODUCT_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .sort(sortBy)
                ...
                let totalRecord =
                    await DATAHUB_PRODUCT_COLL.count(conditionObjOrg)
