...
class Model extends BaseModel {
        ...
        return new Promise(async resolve => {
            try { 
                ...
                if(!checkObjectIDs(inspectionDocID))
                    return resolve({ error: true, message: "inspectionDocID không hợp lệ", keyError: KEY_ERROR.PARAMS_INVALID });

                let dataInsert = { inspection: inspectionDocID, name, userCreate: userID };

                if(sign){
                    let checkSign = await DATAHUB_INSPECTION_CHECKLIST_COLL.findOne({ sign });
                    if(checkSign)
    ...
    update({ inspectionChecklistID, name, sign, description, userID }) {
        return new Promise(async resolve => {
            try {
                ...
                if(sign){
                    let checkSign = await DATAHUB_INSPECTION_CHECKLIST_COLL.findOne({ _id: { $ne: inspectionChecklistID }, sign });
                    if(checkSign)
                        return resolve({ error: true, message: "Mã hiệu đã tồn tại", keyError: KEY_ERROR.ITEM_EXISTED });
                ...
                let infoAfterUpdate = await DATAHUB_INSPECTION_CHECKLIST_COLL.findByIdAndUpdate(inspectionChecklistID, dataUpdate, { new: true });
                if (!infoAfterUpdate) 
        ...
        return new Promise(async (resolve) => {
            try {
                if(!checkObjectIDs(inspectionChecklistID))
                    return resolve({ error: true, message: 'Mã datahub không hợp lệ', keyError: "inspectionChecklistID_invalid", status: 400 });
                
                let infoAterRemove = await DATAHUB_INSPECTION_CHECKLIST_COLL.findByIdAndDelete(inspectionChecklistID)
                if(!infoAterRemove)
	...
	getInfo({ inspectionChecklistID, select, populates }) { 
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                if(!checkObjectIDs(inspectionChecklistID))
                    return resolve({ error: true, message: 'Request params inspectionChecklistID invalid', status: 400 });
                 ...
                 */
                let infoAterGet = await DATAHUB_INSPECTION_CHECKLIST_COLL.findById(inspectionChecklistID)
                                    .select(select)
                                    .populate(populates)
                if(!infoAterGet) 
	 ...
	 * Date: 26/10/2021
	 */
    getList({ inspectionID, userID, lastestID, keyword, limit = 10, select, populates = {}}){
        return new Promise(async resolve => {
            try {

                if(!checkObjectIDs(inspectionID))
                    return resolve({ error: true, message: 'Request params inspectionID invalid', keyError: KEY_ERROR.PARAMS_INVALID });
				...
				if(lastestID && checkObjectIDs(lastestID)){
					let infoData = await DATAHUB_INSPECTION_CHECKLIST_COLL.findById(lastestID);
					if(!infoData)
                ...
                let listPackages = await DATAHUB_INSPECTION_CHECKLIST_COLL
					.find(conditionObj)
					.limit(limit + 1)
					.select(select)
					.populate(populates)
				...
				// GET TOTAL RECORD
                let totalRecord = await DATAHUB_INSPECTION_CHECKLIST_COLL.count(conditionObjOrg);
