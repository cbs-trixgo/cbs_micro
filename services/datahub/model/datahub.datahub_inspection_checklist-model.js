  ...
  insert({ inspectionDocID, name, sign, description, userID }) {
    return new Promise(async (resolve) => {
      try {
        ...
        if (sign) {
          let checkSign = await DATAHUB_INSPECTION_CHECKLIST_COLL.findOne({
            sign,
          })
          if (checkSign)
            return resolve({
  ...
  getInfo({ inspectionChecklistID, select, populates }) {
    return new Promise(async (resolve) => {
      try {
         ...
         */
        let infoAterGet = await DATAHUB_INSPECTION_CHECKLIST_COLL.findById(
          inspectionChecklistID
        )
          .select(select)
          .populate(populates)
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (lastestID && checkObjectIDs(lastestID)) {
          let infoData =
            await DATAHUB_INSPECTION_CHECKLIST_COLL.findById(lastestID)
          if (!infoData)
            return resolve({
              error: true,
        ...
        // GET TOTAL RECORD
        let totalRecord =
          await DATAHUB_INSPECTION_CHECKLIST_COLL.count(conditionObjOrg)
