    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let dataInsert = {
          ...
          note,
          unitprice:
            unitprice && !isNaN(unitprice) ? Number(unitprice).toFixed(0) : 0,
        }
          ...
          for (const item of list) {
            ...
            if (id && _isValid(id)) {
              let dataUpdate = {
                ...
                unit: item[4],
                unitprice:
                  item[5] && !isNaN(item[5]) ? Number(item[5]).toFixed(0) : 0,
                note: item[6],
              }

              await DATAHUB_JOB_COLL.findByIdAndUpdate(id, dataUpdate, {
                new: true,
              })
            } else {
              let dataInsert = {
      ...
      try {
        ...
        for (const item of data) {
          if (item.id && _isValid(item.id)) {
            ...
            await DATAHUB_JOB_COLL.findByIdAndUpdate(item.id, dataUpdate, {
              new: true,
            })
          } else {
            await that.insert({
