    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await DATAHUB_TEMPLATE_COLL.findByIdAndUpdate(
                        datahubProjectID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    remove({ datahubProjectID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAterRemove =
                    await DATAHUB_TEMPLATE_COLL.findByIdAndDelete(
                        datahubProjectID
                    )
                if (!infoAterRemove)
                    return resolve({
    ...
    getInfo({ datahubProjectID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                let infoAterGet = await DATAHUB_TEMPLATE_COLL.findById(
                    datahubProjectID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await DATAHUB_TEMPLATE_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let listPackages = await DATAHUB_TEMPLATE_COLL.find(
                    conditionObj
                )
                    .limit(limit + 1)
                    .select(select)
                ...
                // GET TOTAL RECORD
                let totalRecord =
                    await DATAHUB_TEMPLATE_COLL.count(conditionObjOrg)
