  ...
  update({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAfterUpdate = await DATAHUB_TEMPLATE_COLL.findByIdAndUpdate(
          datahubProjectID,
          dataUpdate,
          { new: true }
        )
  ...
  remove({ datahubProjectID }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAterRemove =
          await DATAHUB_TEMPLATE_COLL.findByIdAndDelete(datahubProjectID)
        if (!infoAterRemove)
          return resolve({
            error: true,
  ...
  getList({ userID, lastestID, keyword, limit = 10, select, populates = {} }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let listPackages = await DATAHUB_TEMPLATE_COLL.find(conditionObj)
          .limit(limit + 1)
          .select(select)
          .populate(populates)
          .sort(sortBy)
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let listCompany = await COMPANY_COLL.find({})
          ...
          .sort({ _id: 1 })
        let listProject = await DATAHUB_TEMPLATE_COLL.find({})
          .populate({
            path: 'client area3',
            select: 'name sign',
            populate: {
