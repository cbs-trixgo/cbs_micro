        ...
        return new Promise(async (resolve) => {
            try {
                if (!name)
                    return resolve({
                        error: true,
                        message: 'Tên datahub không hợp lệ',
                ...
                if (!checkObjectIDs(project, field, contractor))
                    return resolve({
                        error: true,
                        message: 'contractor || project || field invalid',
                        keyError: 'params_invalid',
                    })
                ...
                let {
                    ...
                    floorNumber,
                    floorArea,
                } = await DATAHUB_PROJECT_COLL.findById(project)
                 ...
                 * - Số hợp đồng
                 */
                let infoType = await DATAHUB_TYPE_COLL.findByIdAndUpdate(
                    field,
                    {
                        $addToSet: {
                ...
                await DATAHUB_TYPE_COLL.findByIdAndUpdate(
                    field,
                    {
    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await DATAHUB_PACKAGE_COLL.findByIdAndUpdate(
                        datahubPackageID,
                        dataUpdate,
                        { new: true }
                    )
        ...
        return new Promise(async (resolve) => {
            try {
                if (!checkObjectIDs(datahubPackageID))
                    return resolve({
                        error: true,
                        message: 'Mã datahubPackageID không hợp lệ',
                        keyError: 'datahubPackageID_invalid',
                        status: 400,
                    })

                let infoAterRemove =
                    await DATAHUB_PACKAGE_COLL.findByIdAndDelete(
                        datahubPackageID
                    )
                if (!infoAterRemove)
                    return resolve({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!checkObjectIDs(datahubPackageID))
                    return resolve({
                        error: true,
                        message: 'Request params datahubPackageID invalid',
                        status: 400,
                    })
                 ...
                 */
                let infoAterGet = await DATAHUB_PACKAGE_COLL.findById(
                    datahubPackageID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await DATAHUB_PACKAGE_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                let listPackages = await DATAHUB_PACKAGE_COLL.find(conditionObj)
                    .limit(limit + 1)
                    .select(select)
                    .populate(populates)
                    .sort(sortBy)
                ...
                // GET TOTAL RECORD
                let totalRecord =
                    await DATAHUB_PACKAGE_COLL.count(conditionObjOrg)
            ...
            try {
                ...
                if (option == 1) {
                    ...
                    if (fieldID && checkObjectIDs(fieldID)) {
                         ...
                         */
                        let infoUser =
                            await AUTH__USER_COLL.findById(userID).select(
                                'company'
                            )
                        let infoType =
                            await DATAHUB_TYPE_COLL.findByIdAndUpdate(
                    ...
                    if (listData.length) {
                        await AUTH__COMPANY_COLL.populate(
                            listData,
                            conditionPopulate
                        )
                    }
                ...
                if (option == 3) {
                    ...
                    if (listData.length) {
                        await DATAHUB_TYPE_COLL.populate(
                            listData,
                            conditionPopulate
                        )
                    }
                 ...
                 */
                if (option == 4) {
                    let number1 = await PCM_PLAN_TASK_COLL.count({
                        subtype: 14,
                    })
