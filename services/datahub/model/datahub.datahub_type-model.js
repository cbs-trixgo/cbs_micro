    ...
    update({ datahubID, name, description, popular, userID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate = await DATAHUB_TYPE_COLL.findByIdAndUpdate(
                    datahubID,
                    dataUpdate,
                    { new: true }
                )
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (!checkObjectIDs(datahubTypeID))
                    return resolve({
                        error: true,
                        message: 'Request params datahubTypeID invalid',
                        status: 400,
                    })
                 ...
                 */
                let infoAterGet = await DATAHUB_TYPE_COLL.findById(
                    datahubTypeID
                )
                    .select(select)
                    .populate(populates)
    ...
    getList({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let listPackages = await DATAHUB_TYPE_COLL.find(conditionObj)
                    .limit(limit + 1)
                    .select(select)
                    .populate(populates)
                    .sort(sortBy)
