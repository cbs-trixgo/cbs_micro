    ...
    update({ datahubProfileID, name, files, filesRemove, userID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await DATAHUB_PROFILE_COLL.findByIdAndUpdate(
                        datahubProfileID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    getInfo({ datahubProfileID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                let infoAterGet = await DATAHUB_PROFILE_COLL.findById(
                    datahubProfileID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await DATAHUB_PROFILE_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                // GET TOTAL RECORD
                let totalRecord =
                    await DATAHUB_PROFILE_COLL.count(conditionObjOrg)
