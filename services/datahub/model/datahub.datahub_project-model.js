    ...
    update({
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAfterUpdate =
                    await DATAHUB_PROJECT_COLL.findByIdAndUpdate(
                        datahubProjectID,
                        dataUpdate,
                        { new: true }
                    )
    ...
    remove({ datahubProjectID }) {
        return new Promise(async (resolve) => {
            try {
                ...
                let infoAterRemove =
                    await DATAHUB_PROJECT_COLL.findByIdAndDelete(
                        datahubProjectID
                    )
                if (!infoAterRemove)
                    return resolve({
    ...
    getInfo({ datahubProjectID, select, populates }) {
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                let infoAterGet = await DATAHUB_PROJECT_COLL.findById(
                    datahubProjectID
                )
                    .select(select)
                    .populate(populates)
        ...
        return new Promise(async (resolve) => {
            try {
                ...
                if (lastestID && checkObjectIDs(lastestID)) {
                    let infoData =
                        await DATAHUB_PROJECT_COLL.findById(lastestID)
                    if (!infoData)
                        return resolve({
                            error: true,
                ...
                // GET TOTAL RECORD
                let totalRecord =
                    await DATAHUB_PROJECT_COLL.count(conditionObjOrg)
