...
class Model extends BaseModel {
        ...
        return new Promise(async resolve => {
            try {
                if(!checkObjectIDs(field, contractor)) 
                    return resolve({ error: true, message: "Tham số không đúng định dạng", keyError: "params_invalid" });
                let dataInsert = { field, contractor, userCreate: userID, userUpdate: userID };
	 ...
	 * Code: 
	 */
    update({ datahubContractorID, field, areas, areasRemove, contractor, ranking, note, status, userID  }) {
        return new Promise(async resolve => {
            try {
                ...
                let infoAfterUpdate = await DATAHUB_CONTRACTOR_COLL.findByIdAndUpdate(datahubContractorID, dataUpdate, { new: true });
                if (!infoAfterUpdate) 
        ...
        return new Promise(async (resolve) => {
            try {
                if(!checkObjectIDs(datahubContractorID))
                    return resolve({ error: true, message: 'Mã datahubContractorID không hợp lệ', keyError: "datahubContractorID_invalid", status: 400 });
                
                let infoAterRemove = await DATAHUB_CONTRACTOR_COLL.findByIdAndDelete(datahubContractorID)
                if(!infoAterRemove)
	...
	getInfo({ datahubContractorID, select, populates }) { 
        return new Promise(async (resolve) => {
            try {
                 ...
                 */
                if(!checkObjectIDs(datahubContractorID))
                    return resolve({ error: true, message: 'Request params datahubContractorID invalid', status: 400 });
                 ...
                 */
                let infoAterGet = await DATAHUB_CONTRACTOR_COLL.findById(datahubContractorID)
                                    .select(select)
                                    .populate(populates)
                if(!infoAterGet)
        ...
        return new Promise(async resolve => {
            try {
				...
				if(lastestID && checkObjectIDs(lastestID)){
					let infoData = await DATAHUB_CONTRACTOR_COLL.findById(lastestID);
					if(!infoData)
                ...
                let listPackages = await DATAHUB_CONTRACTOR_COLL
					.find(conditionObj)
					.limit(limit + 1)
					.select(select)
					.populate(populates)
				...
				// GET TOTAL RECORD
                let totalRecord = await DATAHUB_CONTRACTOR_COLL.count(conditionObjOrg);
