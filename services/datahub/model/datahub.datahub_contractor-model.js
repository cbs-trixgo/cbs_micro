  ...
  update({
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAfterUpdate = await DATAHUB_CONTRACTOR_COLL.findByIdAndUpdate(
          datahubContractorID,
          dataUpdate,
          { new: true }
        )
  ...
  remove({ datahubContractorID }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let infoAterRemove =
          await DATAHUB_CONTRACTOR_COLL.findByIdAndDelete(datahubContractorID)
        if (!infoAterRemove)
          return resolve({
            error: true,
  ...
  getList({ userID, lastestID, keyword, limit = 10, select, populates = {} }) {
    return new Promise(async (resolve) => {
      try {
        ...
        let listPackages = await DATAHUB_CONTRACTOR_COLL.find(conditionObj)
          .limit(limit + 1)
          .select(select)
          .populate(populates)
          .sort(sortBy)
