 ...
 * MODELS
 */
const DATAHUB_CONTRACTOR_MODEL =
    require('../model/datahub.datahub_contractor-model').MODEL
    ...
    insert: {
        ...
        async handler(ctx) {
            try {
                ...
                let { field, areas, contractor, ranking, note, status } =
                    ctx.params
                const { _id: userID } = ctx.meta.infoUser

                let resultAfterCallHandler =
                    await DATAHUB_CONTRACTOR_MODEL.insert({
                        field,
                        areas,
                        contractor,
                        ranking,
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler =
                    await DATAHUB_CONTRACTOR_MODEL.update({
                        datahubContractorID,
                        field,
                        areas,
                        areasRemove,
    ...
    remove: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler =
                    await DATAHUB_CONTRACTOR_MODEL.remove({
                        datahubContractorID,
                    })
        ...
        async handler(ctx) {
            try {
                ...
                if (datahubContractorID) {
                    resultAfterCallHandler =
                        await DATAHUB_CONTRACTOR_MODEL.getInfo({
                            datahubContractorID,
                            select,
                            populates,
                        })
                    ...
                    resultAfterCallHandler =
                        await DATAHUB_CONTRACTOR_MODEL.getList({
                            userID,
                            lastestID,
                            keyword,
                            limit: +limit,
