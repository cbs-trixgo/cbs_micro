...
module.exports = {
  ...
  insert: {
    ...
    async handler(ctx) {
      try {
        ...
        let { parentID, sign, name, description, unit, note, unitprice } =
          ctx.params
  ...
  update: {
    ...
    async handler(ctx) {
      try {
        await this.validateEntity(ctx.params)
        let { jobID, sign, name, description, unit, note, unitprice } =
          ctx.params
        const { _id: userID } = ctx.meta.infoUser
  ...
  getInfoAndGetList: {
    ...
    async handler(ctx) {
      try {
        await this.validateEntity(ctx.params)
        let { jobID, parentID, lastestID, keyword, limit, select, populates } =
          ctx.params
        const { _id: userID } = ctx.meta.infoUser
