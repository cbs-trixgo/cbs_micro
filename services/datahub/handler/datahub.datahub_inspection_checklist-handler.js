 ...
 * MODELS
 */
const DATAHUB_INSPECTION_CHECKLIST_MODEL =
    require('../model/datahub.datahub_inspection_checklist-model').MODEL
...
module.exports = {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler =
                    await DATAHUB_INSPECTION_CHECKLIST_MODEL.insert({
                        inspectionDocID,
                        name,
                        sign,
                        description,
    ...
    update: {
        ...
        async handler(ctx) {
            try {
                await this.validateEntity(ctx.params)
                let { inspectionChecklistID, name, sign, description } =
                    ctx.params
                const { _id: userID } = ctx.meta.infoUser

                let resultAfterCallHandler =
                    await DATAHUB_INSPECTION_CHECKLIST_MODEL.update({
    ...
    remove: {
        ...
        async handler(ctx) {
            try {
                ...
                let resultAfterCallHandler =
                    await DATAHUB_INSPECTION_CHECKLIST_MODEL.remove({
                        inspectionChecklistID,
                    })
        ...
        async handler(ctx) {
            try {
                ...
                if (inspectionChecklistID) {
                    resultAfterCallHandler =
                        await DATAHUB_INSPECTION_CHECKLIST_MODEL.getInfo({
                            inspectionChecklistID,
                            select,
                            populates,
                        })
                    ...
                    resultAfterCallHandler =
                        await DATAHUB_INSPECTION_CHECKLIST_MODEL.getList({
                            inspectionID,
                            userID,
                            lastestID,
                            keyword,
