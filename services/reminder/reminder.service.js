...
module.exports = {
    ...
    actions: {
        ...
        [CF_ACTIONS_REMINDER.ADD_JOB_MUTE_CONVERSATION]:
            REMINDER__CORE_HANLDER.addJobMuteConversation,
        [CF_ACTIONS_REMINDER.DELETE_JOB_MUTE_CONVERSATION]:
            REMINDER__CORE_HANLDER.deleteJobMuteConversation,
        [CF_ACTIONS_REMINDER.ADD_JOB_REMINDER_CONVERSATION]:
            REMINDER__CORE_HANLDER.addJobReminderConversation,
        [CF_ACTIONS_REMINDER.DELETE_JOB_REMINDER_CONVERSATION]:
            REMINDER__CORE_HANLDER.deleteJobReminderConversation,
        [CF_ACTIONS_REMINDER.ADD_JOB_AUTO_DELETE_MESSAGE_CONVERSATION]:
            REMINDER__CORE_HANLDER.addJobAutoDeleteMessageConversation,
        [CF_ACTIONS_REMINDER.DELETE_JOB_AUTO_DELETE_MESSAGE_CONVERSATION]:
            REMINDER__CORE_HANLDER.deleteJobAutoDeleteMessageConversation,
        [CF_ACTIONS_REMINDER.ADD_JOB_REMINDER_EXPIRED_TASK]:
            REMINDER__CORE_HANLDER.addJobReminderExpiredTask,
        [CF_ACTIONS_REMINDER.DELETE_JOB_REMINDER_EXPIRED_TASK]:
            REMINDER__CORE_HANLDER.deleteJobReminderExpiredTask,
        [CF_ACTIONS_REMINDER.ADD_JOB_REFRESH_TOKEN_ZALO_OA]:
            REMINDER__CORE_HANLDER.addJobRefreshTokenZaloOA,
    },
    ...
    async started() {
        const runDefineTask = async () => {
            ...
            agenda.define('add_reminder_conversation', async (job, done) => {
                ...
                if (job.attrs.data && job.attrs.data.userID) {
                    const socket = require('socket.io-client')(
                        `${process.env.SOCKET_URL}?token=${job.attrs.data.userID}&access_key=${process.env.SOCKET_ACCESS_KEY || 'Y2JzX3Byb2plY3RAMjAyMQo='}`
                    )
                    ...
                    socket.on('connect', () => {
                        ...
                        // });
                        socket.emit(
                            'CHATTING_CSS_NOTIFICATION_REMINDER',
                            job.attrs.data
                        )
                        socket.disconnect()
                    })
            ...
            agenda.define(
                'auto_delete_message_conversation',
                async (job, done) => {
                    await this.broker.call(
            ...
            agenda.define('add_reminder_expired_task', async (job, done) => {
                try {
                    ...
                    const { taskID, userID, message, senderMail } =
                        job.attrs.data

                    // Call service để làm việc
                    const infoTask = await this.broker.call(
                    ...
                    const dataSend = {
                        ...
                        mainColl: {
                            // Bấm vào để truy cập vào chi tiết Task
                            kind: 'pcm_plan_task',
                            item: { _id: taskID },
                        },
                    }
            ...
            agenda.define('zalo_oa_refresh_token', async (job, done) => {
                try {
                    ...
                    await this.broker.call(
                        `${CF_DOMAIN_SERVICES.ZALO}.${CF_ACTIONS_ZALO.ZALO_RENEW_TOKEN}`,
                        {
                            isCache: true,
                        }
                    )
