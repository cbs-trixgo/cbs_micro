...
module.exports = {
    ...
    addJobMuteConversation: {
        ...
        async handler(ctx) {
            try {
                ...
                if (!timeScheduler) throw new Error('timeScheduler is empty')
                ...
                console.log(
                    `agenda: update_mute_conversation added - ${timeScheduler}`
                )
        ...
        async handler(ctx) {
            try {
                ...
                if (!conversationID || !userID)
                    throw new Error(
                        'Request params conversationID or userID invalid'
                    )
    ...
    addJobReminderConversation: {
        ...
        async handler(ctx) {
            try {
                ...
                if (humanTime) {
                    const reminder = agenda.create(
                        'add_reminder_conversation',
                        {
                            conversationID,
                            messageID,
                            userID,
                    ...
                    await agenda.start()
                    await reminder
                        .repeatEvery(humanTime, {
                            skipImmediate: true,
                        })
                        .save()

                    console.log(
                        `agenda: add_reminder_conversation added - ${humanTime} - ${repeat} - ${notifyFor}`
                    )
                }
                ...
                if (remindTime && repeat === 'once') {
                    ...
                    console.log(
                        `agenda: add_reminder_conversation added - ${new Date(remindTime)} - ${remindTime} - ${repeat} - ${notifyFor}`
                    )
                }
        ...
        async handler(ctx) {
            try {
                ...
                if (!conversationID || !messageID)
                    throw new Error(
                        'Request params conversationID or messageID invalid'
                    )
    ...
    addJobAutoDeleteMessageConversation: {
        ...
        async handler(ctx) {
            try {
                ...
                console.log(
                    `agenda: auto_delete_message_conversation added - ${timeScheduler}`
                )
    ...
    addJobRefreshTokenZaloOA: {
        async handler() {
            try {
                const timeScheduler = new Date(
                    moment().add(24, 'hours')
                ).getTime()

                agenda.schedule(timeScheduler, 'zalo_oa_refresh_token')
                console.log(
                    `agenda: zalo_oa_refresh_token added - ${timeScheduler}`
                )
        ...
        async handler(ctx) {
            try {
                ...
                switch (alert) {
                    case 0.4:
                        message = '15 phút'
                        timeScheduler = moment(timeScheduler).subtract(
                            15,
                            'minutes'
                        ) // .valueOf();
                        break
                ...
                console.log(
                    `agenda: add_reminder_expired_task added - ${timeScheduler}`
                )
        ...
        async handler(ctx) {
            try {
                ...
                if (!taskID)
                    return {
                        error: true,
                        message: 'Request params taskID invalid',
                    }
                ...
                if (listJob && listJob.length) {
                    ...
                    await agenda.cancel({ _id: { $in: jobsID } })
                    console.log(
                        `agenda: delete_job_reminder_expired_task canceled - taskID: ${taskID}`
                    )
                }
