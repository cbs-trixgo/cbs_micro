  ...
  addJobReminderExpiredTask: {
    ...
    async handler(ctx) {
      try {
        ...
        const { alert, actualFinishTime, status, taskID, userID, senderMail } =
          ctx.params
        let timeScheduler = '',
          message = ''

        if (status === 3) return { error: true, message: 'task is done' }
        ...
        if (!message) return { error: true, message: 'Not found alert time' }
